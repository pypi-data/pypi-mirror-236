...
class single_signal_test(object):
    ...
    def cal_turnover(self,df,ndays):
        ...
        return (holdings.diff(ndays).abs().sum(axis = 1)/2/ndays)
    ...
    def factor_ret_test_sheet(self,
        ...
        fwr = utils.compute_forward_returns(price.stack(),price,periods=(days,))
    ...
    def group_industry_strict(self,res_df,index_components,industry_info):
        # 行业成分股占比
        print('计算指数内行业占比...')
        indstpct_in_index = pd.concat([index_components,industry_info],axis=1).dropna(subset='weight').groupby(by=['date','first_industry_name']).sum().groupby(level = 'date',group_keys=False).apply(lambda x: x/x.sum())
        indst_strict = pd.concat([industry_info.reset_index('symbol').set_index('first_industry_name',append=True),indstpct_in_index],axis=1).dropna().reset_index('first_industry_name').set_index('symbol',append=True)
        # 行业限制
        print('计算组合内行业占比...')
        res_df['weighted_factor'] = res_df.groupby(by = ['date','weighted_factor_quantile'],group_keys=False)['weighted_factor'].apply(lambda x: x/x.sum())
        res_df['indst_weight'] = indst_strict['weight']
        res_df['indst'] = indst_strict['first_industry_name']
        res_df = res_df.reset_index('symbol').set_index(['weighted_factor_quantile','indst'],append = True)
        res_df['pos_indst_w'] = res_df.groupby(by = ['date','weighted_factor_quantile','indst'],group_keys=False)['weighted_factor'].sum()
        res_df = res_df.reset_index(['weighted_factor_quantile','indst']).set_index('symbol',append = True)
        print('行业偏离差异超过3%的调整该行业标的权重....')
        w_adj_loc = res_df[(res_df['pos_indst_w']-res_df['indst_weight']).abs()>0.03].index
        res_df.loc[w_adj_loc,'weighted_factor']*=res_df.loc[w_adj_loc,'indst_weight']/res_df.loc[w_adj_loc,'pos_indst_w']
        return res_df
...
class multi_factor_test(object):
    ...
    def factors_abnormal_ret(self,factordf,return_col,factorlist,days,index_price = None,pricedf = None,return_type = 'ols',demean = False):
        # 需要在当天知道历史的因子收益，所以因子整体往前挪，同时计算超额收益时也需要把cumpute_forward_return的结果挪回去
        df = factordf.apply(lambda x: x.unstack().shift(days).stack())
        if pricedf is not None:
            # 默认明收除今收
            df[str(days)+'D'] = pricedf.pct_change(days,fill_method = None).stack()/days#.shift(-days).stack()
            if index_price is not None:
                ml = mate_alphalens()
                df,pricedf = ml.index_mate(df,pricedf)
                df = ml.trans_ex_return(df,index_price,str(days)+'D',shift=True)
        ...
        if return_type == 'ols':
        ...
        elif return_type == 'factor_weighted':
            ret_k = df.apply(lambda x: x*df[return_col+str(days)+'D']).dropna().groupby(level = 0).sum()/df.groupby(level = 0).sum()
            if demean == True:
                ret_mean = df.apply(lambda x: df[return_col+str(days)+'D']).dropna().groupby(level = 0).mean()
                ret_k = ret_k - ret_mean
            del ret_k[return_col+str(days)+'D']

        elif return_type == 'long_short' :
            lscol = list(factordf)
            ...
            for col in tqdm(lscol):
                ...
                # else:
                #     direc = qreturn.rolling(20,min_periods = 1).mean()
                #     direc = direc[max(list(qreturn))]>=direc[1]
                #     LSretList.append(pd.concat([qreturn[direc][max(list(qreturn))],qreturn[~direc][1]]).sort_index())
    ...
    def in_some_pool(self,df,pool_components):
        factordf = df.copy()
        factordf['inpool']=pool_components.map(lambda x:1)
    ...
    def weighted_factor(self,factordf,weight):
        ...
        wf = (factordf*weight).sum(axis = 1)

        return self.mat_normlize(pd.DataFrame(wf,columns = ['weighted_factor']))
...
class AutoMatic(object):
    ...
    def __init__(self,
                #  Bft,
                 base_index,Price,days,
                #  qcut,demean,quantiles
                 weighted_method
                 ) -> None:
    ...
    def AutoMatic_Cal_FactorWeightedRet_WeightedFactor(self,factordf,ifrank,roll_period1,roll_period2):
        
        if ifrank:
            factors_weighted_ret = self.mft.factors_abnormal_ret(factordf,
                                                                'ret',
                                                                list(factordf),
                                                                self.days,self.base_index,
                                                                self.Price,
                                                                self.weighted_method)[list(factordf)]
            factor_weight = (factors_weighted_ret.rolling(roll_period1).mean()/factors_weighted_ret.rolling(roll_period1).std()+factors_weighted_ret.rolling(roll_period2).mean()/factors_weighted_ret.rolling(roll_period2).std()).rank(axis = 1).apply(lambda x: x-x.mean(),axis = 1) #.shift(self.days)
        else:# 因子加权收益 demean?
            factors_weighted_ret = self.mft.factors_abnormal_ret(factordf,
                                                                'ret',
                                                                list(factordf),
                                                                self.days,self.base_index,
                                                                self.Price,
                                                                self.weighted_method,
                                                                demean=True)[list(factordf)]
            factor_weight = (factors_weighted_ret.rolling(roll_period1).mean()/factors_weighted_ret.rolling(roll_period1).std()+factors_weighted_ret.rolling(roll_period2).mean()/factors_weighted_ret.rolling(roll_period2).std())#.shift(self.days)

        return self.mft.weighted_factor(factordf,factor_weight)
...
class plot_tools(object):
    ...
    def factor_plt(self,qreturn,to,ictable,fc,ndays,savedir=''):
        ...
        plt.subplot(324,title = fc+'_daily_average_turnover_bygroup')
...
class mate_alphalens(object):
    ...
    def trans_ex_return(self,clean_factor,index_price,ret_col,shift = False):
        from alphalens import utils
        index_price['factor'] = 1
        base_ret = utils.compute_forward_returns(index_price[['factor']],index_price['close'].unstack(),periods=(int(ret_col[:-1]),))
        ...
        if shift == True:
            df[ret_col]= df[ret_col]-base_ret[ret_col].unstack().shift(int(ret_col[:-1])).stack()
        else:
