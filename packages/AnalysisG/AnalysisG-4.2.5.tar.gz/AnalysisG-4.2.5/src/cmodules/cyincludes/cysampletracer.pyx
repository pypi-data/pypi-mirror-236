...
cdef class SampleTracer:
    ...
    cpdef dict makehashes(self):
        ...
        for batch in self.ptr.MakeIterable():
            if self._set.getevent: merge(&ev, &batch.event_dir, batch.hash)
            if self._set.getgraph: merge(&gr, &batch.graph_dir, batch.hash)
            if self._set.getselection: merge(&sel, &batch.selection_dir, batch.hash)
        out["event"] = map_vector_to_dict(&ev)
        out["graph"] = map_vector_to_dict(&gr)
        out["selection"] = map_vector_to_dict(&sel)
        return out

    cpdef list makelist(self):
        cdef int i
        cdef Event event
        cdef list output = []
