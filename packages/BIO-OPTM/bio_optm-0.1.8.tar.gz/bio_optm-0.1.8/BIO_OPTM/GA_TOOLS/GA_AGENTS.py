    ...
    def generateBreedingPairs(self, ):
        ...
        for i in range(int(np.ceil(self.population_size/2))):
            ...
            while True: # avoid inbreeding
                ...
                if c >= patience:
                    print("out of patience")
                    print(f"chromo pair: {ranked_chromo}")
                    reduced_chromo = list(ranked_chromo)
                    del reduced_chromo[reduced_chromo.index(kid_a)]
                    # for k in ranked_chromo
                    # while kid_b == kid_a:
                    #     print(f"OOP: A: {kid_a}, B: {kid_b}")
                    kid_b = np.random.default_rng().choice(reduced_chromo, 1, replace=False)[0]
                    break
            couplings.append([kid_a, kid_b])
        return couplings
...
class GA_TSPOptimizer(GA_OptimizerOmegaMin):
    ...
    def __init__(self, chromosome_size: int, population_size: int, chromosomeAlphabet: int, generations: int, mutation_rate: float, crossOver_rate: float, 
                 objective_func: adjacencyDistance, mode: str, 
                 DistanceMat: np.array or None,
                 **kwargs) -> None:
    ...
    def adjacencyDistance(self, chromosome, mat, **kwargs):
        # get distance from source to first node
        d = mat[chromosome[0], 0]
        for node in range(len(chromosome)-1):
            i = chromosome[node]
