...
DATA_LEVEL_METADATA_ID = {'MC0': -3, 'R0': -2, 'R1': -1,
...
def get_os_and_cpu_info():
  ...
  cpuinfo = subprocess.check_output('cat /proc/cpuinfo', shell=True, universal_newlines=True)
  model_name = subprocess.check_output('grep -m 1 "model name" /proc/cpuinfo', shell=True, universal_newlines=True).strip()
