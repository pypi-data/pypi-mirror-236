...
def desktop_info(title=None, content=None, total_time=3,
                 b=None, **leak):
...
def _browser(url='', mine=None,
    ...
    service = {
        'edge': lambda: None,
        'chrome': lambda: \
            # webdriver.ChromeService(executable_path=chromedriver_py.binary_path),
...
class Edge():
    def __init__(self, url=None, silent=False, driver=None, mine=False, mute=False):
        if not driver == None:
            ...
            self.driver = _browser(_type='edge', url='',
                                   silent=silent, mine=mine, mute=mute)
        if not url == None:
            self.get(url)
        self.silent = silent
        self.mine = mine
    ...
    def click(self, xpath=None, strict=True, depth=9,
        ...
        if type(xpath) in [str]:
            return Edge.click(self, self.element(**b))
...
if (element) {
    ...
    @manage_args(s=['xpath'])
    def elements(self, s=None, depth=9,
        ...
        try:
            if not type(s) == list:
                ret = elements(root=root, xpath=s, depth=depth,
                               silent=silent, strict=strict, complete=complete)
            else:
                for i in s:
                    ret = elements(root=root, xpath=i, depth=depth,
            ...
            if ret in [None, []]:
                if refresh:
                    self.refresh()
                    return self.elements(xpath=s, depth=depth, silent=silent, strict=strict,
                                         root=root,
                                         refresh=refresh)
                if strict:
                    self.errorscr(ret)
    ...
    error_screen=errorscr
    errscreen=errorscr
    errorscreen=errorscr
    errscr=errorscr
