...
assign ( Network const& network ) {
  ...
  for ( uint64_t d = 0; d < D; ++ d ) {
    ...
    uint64_t n_pos = 0; // Number of positive in-edges
    for ( auto source : data_ -> network_ . inputs ( d ) ) {
      if ( data_ -> network_ . interaction ( source, d ) )
        ++ n_pos;
    }
    uint64_t n_neg = n - n_pos; // Number of negative in-edges
    std::stringstream ss;
    ss << path << "/" << n <<  "_" << m;
    // for ( auto const& p : logic_struct ) ss <<  "_" << p.size();
    ss <<  "_" << n_neg << "n" <<  "_" << n_pos << "p";
