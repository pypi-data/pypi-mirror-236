...
_parse ( std::vector<std::string> const& lines ) {
  ...
  for ( auto const& logic_string : logic_strings ) {
    ...
    auto flush_factor = [&] () {
      ...
      // Compare items by values, but put negative edges first
      auto compare_items = [&](uint64_t lhs, uint64_t rhs) {
        // If lhs is negative and rhs is positive
        if ( !interaction ( lhs, target ) and interaction ( rhs, target ) ) return true;
        // If lhs is positive and rhs is negative
        if ( interaction ( lhs, target ) and !interaction ( rhs, target ) ) return false;
        // If both have the same sign, just compare values
        return lhs < rhs;
      };
      std::sort ( factor.begin(), factor.end(), compare_items );
