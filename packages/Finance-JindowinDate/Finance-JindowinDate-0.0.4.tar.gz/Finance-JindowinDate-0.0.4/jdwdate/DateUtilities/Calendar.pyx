...
from jdwdate.Enums._Weekdays cimport Weekdays
from jdwdate.Enums._TimeUnits cimport TimeUnits
from jdwdate.Enums._Months cimport Months
from jdwdate.Enums._BizDayConventions cimport BizDayConventions
from jdwdate.DateUtilities.Date cimport Date
from jdwdate.DateUtilities.Period cimport Period
...
cdef set hkex_working_weekends = {
    Date.westernStyle(4, Months.October, 1992)
}


cdef class ChinaHkexImpl(CalendarImpl):
    def __init__(self):
        pass

    cdef bint isBizDay(self, Date date):
        cdef int w = date.weekday()
        if (self.isWeekEnd(w) or date in hkex_holDays) and date not in hkex_working_weekends:
            return False
        return True
    ...
    def __richcmp__(self, right, int op):
        if op == 2:
            return isinstance(right, ChinaHkexImpl)
