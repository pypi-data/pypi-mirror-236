...
from jdwdate.Enums.TimeUnits import TimeUnits
from jdwdate.Enums.Months import Months
...
class DayCounter(object):
    ...
    def __init__(self, dcName):
        ...
        if dcName in _dcDict:
            ...
            raise ValueError(
                '{0} is not a valid day counter name'.format(dcName))
...
class Actual365NoLeap(DayCounterImpl):
    ...
    def dayCount(self, d1, d2):
        ...
        s1 = d1.dayOfMonth() + self.MonthOffset[d1.month() - 1] + (d1.year() *
                                                                   365)
        s2 = d2.dayOfMonth() + self.MonthOffset[d2.month() - 1] + (d2.year() *
                                                                   365)
    ...
    def yearFraction(self, d1, d2, d3, d4):
        ...
        if d2 <= refPeriodEnd:
            if d1 >= refPeriodStart:
                ...
                previousRef = refPeriodStart - Period(length=months,
                ...
                if d2 > refPeriodStart:
                    ...
                    return self.yearFraction(d1, d2, previousRef,
                                             refPeriodStart)

        else:
            sum = self.yearFraction(d1, refPeriodEnd, refPeriodStart,
                                    refPeriodEnd)
            i = 0
            while True:
                newRefStart = refPeriodEnd + Period(length=i * months,
                                                    units=TimeUnits.Months)
                newRefEnd = refPeriodEnd + Period(length=(i + 1) * months,
                                                  units=TimeUnits.Months)
