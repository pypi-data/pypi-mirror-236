...
def df_strikeprice_func(client, iDict, Scripmaster):
    ...
    if len(Scripmaster[(Scripmaster.ISIN == underlying_print)  & (Scripmaster.Expiry == expiry)])==0:
        print("Input expiry date not found found for", underlying_print)
        tym.sleep(2)
    ...
    margin=pd.json_normalize(margin)
    ...
    print("Margin total= ", margin.loc[0,'NetAvailableMargin']+margin.loc[0,'MarginUtilized'])
    print("Margin available= ", margin.loc[0,'NetAvailableMargin'])
    print("Margin used= ", margin.loc[0,'MarginUtilized'])

    # print("\n time till fetching data from 5paisa=", mid1-start)
    print("Time for fetching LTPs from 5paisa/above looptime=", mid2-mid1)
    # print("Time after loop=",code_completion-mid2)
    print("Code Completion seconds=",code_completion-start)
    ...
    message=str("\n"+str(underlying_print)+" LTP= "+str(round(underlying["Close"][len(underlying)-1],0))+
                "\n"+"Code Completion time= "+str(code_completion_time)+"\n"+

                "Strike CE="+str(df_strikeprice.loc[CE_indx_SELL,'Strike_Selection'])+
                "  Price="+str(df_strikeprice.loc[CE_indx_SELL,'CE_LTP'])+
                "  Scripcode="+str(df_strikeprice.loc[CE_indx_SELL,'Scripcode_CE'])+"\n"+

                "Strike PE="+str(df_strikeprice.loc[PE_indx_SELL,'Strike_Selection'])+
                "  Price="+str(df_strikeprice.loc[PE_indx_SELL,'PE_LTP'])+
                "  Scripcode="+str(df_strikeprice.loc[PE_indx_SELL,'Scripcode_PE'])+"\n"+

                "Strike CE="+str(df_strikeprice.loc[CE_indx_BUY,'Strike_Selection'])+
                "  Price="+str(df_strikeprice.loc[CE_indx_BUY,'CE_LTP'])+
                "  Scripcode="+str(df_strikeprice.loc[CE_indx_BUY,'Scripcode_CE'])+"\n"+

                "Strike PE="+str(df_strikeprice.loc[PE_indx_BUY,'Strike_Selection'])+
                "  Price="+str(df_strikeprice.loc[PE_indx_BUY,'PE_LTP'])+
                "  Scripcode="+str(df_strikeprice.loc[PE_indx_BUY,'Scripcode_PE'])
                )
