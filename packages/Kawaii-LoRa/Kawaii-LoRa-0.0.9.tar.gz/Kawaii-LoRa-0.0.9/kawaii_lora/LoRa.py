...
class LoRa(object):
    ...
    _initial_mode: MODE | None = None
    _current_mode: MODE | None = None
    ...
    def check_module(self):
        ...
        if Strawberry.Read(REG.OP_MODE) != MODE.SLEEP.value:
    ...
    def set_modem_config(self, modem_config: ModemConfig):
        ...
        #     - Tipo di modulazione
        Strawberry.Write(REG.MODEM_CONFIG_1, modem_config.value[0])
        Strawberry.Write(REG.MODEM_CONFIG_2, modem_config.value[1])
        Strawberry.Write(REG.MODEM_CONFIG_3, modem_config.value[2])
    ...
    # -------------------------------------------------------------------------------------------- #
    # --- Utils --- #
    def clear_irq_flags(self, flags: IRQ):
        Strawberry.Write(REG.IRQ_FLAGS, flags)

    def read_irq_flags(self) -> IRQ:
        return IRQ(Strawberry.Read(REG.IRQ_FLAGS))
