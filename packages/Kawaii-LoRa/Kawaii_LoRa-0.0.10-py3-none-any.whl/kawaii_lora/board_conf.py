...
from .my_types import Board, Boards
...
class Strawberry:
    ...
    @staticmethod
    def SetBoard(board: Boards):
        Strawberry.BOARD = board.value
    ...
    @staticmethod
    def AddEvents(
        callback_dio0: Callable[[int], None],
        callback_dio1: Callable[[int], None],
        callback_dio2: Callable[[int], None],
        callback_dio3: Callable[[int], None],
    ):
        # Controllo l'Injection della Board
        if Strawberry.BOARD is None:
            raise ValueError("Selezionare la Board su cui si sta lavorando.")
    ...
    # -------------------------------------------------------------------------------------------- #
    @staticmethod
    def Write(reg: int, value: int | bytes | str | Flag | list[int]):
