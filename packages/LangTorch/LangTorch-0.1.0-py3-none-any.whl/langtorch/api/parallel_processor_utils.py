...
class APIRequest(ABC):
    ...
    async def call_api(self, *args, **kwargs):
        """Abstract method for calling the API."""
        pass
...
@dataclass
class APIRequestGCP(APIRequest):
    """Represents a GCP API request."""
    letter_consumption: int
    ...
    async def call_api(
        ...
        """Calls the GCP API and saves results."""
        logging.info(f"Starting request #{self.task_id}")
        error = None
        headers = {
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    url=request_url, headers=headers, json=self.request_json
                ) as response:
                    response = await response.json()
            if "error" in response:
                logging.warning(
