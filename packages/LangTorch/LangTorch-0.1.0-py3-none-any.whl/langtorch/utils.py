...
def zeros_like(other):
    return langtorch.TextTensor(np.char.array(torch.zeros(other.shape), unicode=True).join("").astype(dtype="<U"))


def str_like(other, string):
    return langtorch.TextTensor(np.char.array(torch.zeros(other.shape), unicode=True).join("").astype(dtype="<U")) * TextTensor(string)


def zeros(*shape):
    return langtorch. TextTensor(np.char.array(torch.zeros(*shape), unicode=True).join("").astype(dtype="<U"))
...
def run_backward(tensors, grad_tensors_, retain_graph, create_graph, inputs, allow_unreachable=True,
    ...
    # 2. Prepare an empty map to store computed gradients
    gradients = {}
    ...
    for _ in range(NUM_WORKERS):
        worker_thread = Thread(target=langtorch.worker_func, args=(queue, gradients, retain_graph, create_graph, lock))
        worker_thread.start()
    ...
    for tensor in gradients.keys():
        print("gradient assignment: ", tensor)
        if tensor.requires_grad:
            if not isinstance(tensor, torch.Tensor) or (tensor.is_leaf or tensor.retain_grad):
                grad = gradients.get(tensor, TextTensor(""))
                if len(grad) == 1:
                    try:
