...
class CosineSimilarity(TextModule):
    ...
    def forward(self, t1: TextTensor, t2: TextTensor) -> Tensor:
        if not isinstance(t1, TextTensor):
            t1 = TextTensor(t1)
        if not isinstance(t2, TextTensor):
            t2 = TextTensor(t2)
        if not t1.embedding:
            t1.embed()
        if not t2.embedding:
            t2.embed()
        x1, x2 = t1.embedding, t2.embedding
        try:
            return torch.nn.functional.cosine_similarity(x1, x2, self.dim, self.eps)
        except:
            t1 = t1.reshape()
            return  torch.nn.functional.cosine_similarity(x1, x2, self.dim, self.eps)