...
from .Domain import Sensor, DownLoadFile, Country
...
class HTTPF:
    ...
    def can_visit(url):
        ...
        :return:
        """
        exCode = run("start /B ping " + url, stdout=PIPE, stderr=PIPE, universal_newlines=True, shell=True)
        return exCode.returncode == 0
        ...
        for i in tqdm(range(len(load_dict))):
            ...
            for (key, value) in load_dict[i].items():
                if dir(Sens).__contains__(key):
                    ...
                    logging.error(key + " is not in Sensor Class !! please update packages")
...
class searchData:
    ...
    def check(self):
        if not self.sens.pro_dict.__contains__(self.production):
            logging.warning("产品名称错误")
...
class search(HTTPF):
    ...
    def read_csv(self, save_dir):
        ...
        """
        data = np.array(pd.read_csv(self.file_name_path, header=None, index_col=None))
        filename_list = ["https://ladsweb.modaps.eosdis.nasa.gov" + page for page in data[:, 2].tolist()]
        pd.DataFrame(filename_list).to_csv(save_dir)
        return
