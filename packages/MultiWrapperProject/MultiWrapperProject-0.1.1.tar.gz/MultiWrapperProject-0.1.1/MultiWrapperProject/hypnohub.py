...
# Description : Wrapper for hypnohub.net on python
# -----------------------------------------------------------------------------
#
...
class Hypnohub():
    ...
    def __init__(self) -> None:
        """constructor of hypnohub class
        """
        self.url = "https://hypnohub.net/index.php"
    ...
    def paramsGenerator(self, tags: str = "") -> dict[str]:
        ...
        Args:
            tags (str, optional): tags for a search like in the hypnohub search bar. Defaults to "".
        ...
        if self.safe_mode:
            return {'tags': f"{tags}" + "rating:safe", "limit": "100","json": "1", "s": "post", "page": "dapi","q":"index"}

        elif self.explicit_mode:
            return {'tags': f"{tags}" + "-general:safe", "limit": "100","json": "1", "s": "post", "page": "dapi","q":"index"}

        else:
            return  {"tags": f"{tags}", "limit": "100","json": "1", "s": "post", "page": "dapi","q":"index"}
...
directory directory of file link
...
'''
    ...
    def dictOfAPost(self, id: int) -> dict:
        ...
        """
        url = f"https://hypnohub.net/index.php?page=dapi&s=post&q=index&id={id}"
    ...
    def download(self, url_file: str, file_name: str = "", save_path: str = "") -> NoReturn:
        """
        Function to download images or videos to hypnohub via link from hypnohub only
        ...
        Raises:
            ValueError: The URL is not authorized for downloading.
                        Link to download must start with `https://hypnohub.net/`

        Returns:
            NoReturn: no return
        """

        link_base = "https://hypnohub.net/"
    ...
    def downloadRandom(self, tags: str = "", file_name: str = "", save_path: str = "") -> NoReturn:
        ...
        Returns:
            NoReturn: no return
        """
        self.download(url_file=self.getRandomLink(tags=tags), file_name=file_name, save_path=save_path)
        
    def safeModeSwitch(self):
        """
        Function which allows you to have only post safe on hypnohub
    ...
    def explicitModeSwitch(self):
        """
        Function which allows you to have only explicit and questionable posts on hypnohub
