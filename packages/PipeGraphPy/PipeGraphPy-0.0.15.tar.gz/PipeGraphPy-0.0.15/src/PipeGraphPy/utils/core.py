...
def cal_score_by_groupby(
    ...
    if unit == "day":
        dfc.index = dfc.index.map(lambda x: datetime.datetime.strftime(x, '%Y-%m-%d'))
    elif unit == "month":
        dfc.index = dfc.index.map(lambda x: datetime.datetime.strftime(x, '%Y-%m'))
    elif unit == "year":
        dfc.index = dfc.index.map(lambda x: datetime.datetime.strftime(x, '%Y'))
    else:
