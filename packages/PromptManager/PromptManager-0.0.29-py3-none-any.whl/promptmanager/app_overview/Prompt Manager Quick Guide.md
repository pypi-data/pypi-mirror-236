...
chroma_reader= ChromaReader(connection={"url":"","username":"dcuser","password":"xxxxxx"},embedding=openai_embeddings)
```
...
```shell
curl http://localhost:9999/api/app/<appid>/run -X POST -H 'Content-Type:application/json' -d '{"variables":{"title":"","number":500,"file":"/tmp/promptmanager/upload/2023-03-08/xxxxxxxxxxxxxxxxx/knowledge_base.txt"}}'
```
