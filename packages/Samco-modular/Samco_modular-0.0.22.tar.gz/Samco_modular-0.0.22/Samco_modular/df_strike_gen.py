...
def df_strikeprice_func(token, iDict, Scripmaster):
    ...
    if len(Scripmaster[(Scripmaster.name == underlying_print) & (Scripmaster.expiryDate == expiry)])==0:
        print("Expiry date not found in Scripmaster")
        tym.sleep(3)
    ...
    margin=samco.get_limits()
    margin=json.loads(margin)
    margin=pd.json_normalize(data=margin['equityLimit'])
    ...
    print("Lot size= ", int(qty)/int(lots))
    print("Margin total= ", margin.loc[0,'grossAvailableMargin'])
    print("Margin available= ", margin.loc[0,'netAvailableMargin'])
    print("Margin used= ", margin.loc[0,'marginUsed'])
