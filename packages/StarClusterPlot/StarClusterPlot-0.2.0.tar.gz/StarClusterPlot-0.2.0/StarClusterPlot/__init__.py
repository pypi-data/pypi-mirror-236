...
def ClusterVis(data, x_scale = 6.75, y_scale = 12, min_temp = 1667, max_temp = 26000):
  ...
  RA = data['ra'].where(data['temperature'] > min_temp).where(data['temperature'] < max_temp)
  DEC = data['dec'].where(data['temperature'] > min_temp).where(data['temperature'] < max_temp)
