...
class ModelPerformance():
    ...
    def __init__(self, X_train, y_train, X_test, y_test, model):
        ...
        if len(X_train) <= sample_size:
            ...
            X_train = X_train[sample_indices].reset_index(drop=True)
            y_train = y_train[sample_indices].reset_index(drop=True)

            sample_size_test = 250
            indices_test = np.arange(len(X_test))
            np.random.shuffle(indices_test)
            sample_indices_test = indices_test[:sample_size_test]
            X_test = X_test[sample_indices_test].reset_index(drop=True)
            y_test = y_test[sample_indices_test].reset_index(drop=True)
