import os
from abstract_gui import text_to_key,make_component,ensure_nested_list,expandable
...
def get_chunked_sections():
    return [[make_component("Push"),make_component("Button",button_text="<-",key=text_to_key("chunk text back"),enable_events=True),make_component("input",default_text='0',key=text_to_key("chunk text number"),auto_size_text=True, enable_events=True),make_component("Button",button_text="->",key=text_to_key("chunk text forward"),enable_events=True),make_component("Push")],
...
def get_output_options():
    return [
        [make_component("Text",'Response Type:', size=(15, 1)), make_component("Combo",get_response_types(), default_value=get_response_types()[0], key='-RESPONSETYPE-', readonly=True),
        make_component("Button","Submit Query",key="-SUBMIT_QUERY-", disabled=False,enable_evete=True), make_component("Button",'Clear Input'), make_component("Button",'Copy Response'), make_component("Button",'Paste Input'),make_component("Button",'Clear Chunks'),make_component("Button",'Undo Chunks'),make_component("Button",'Redo Chunks')],
