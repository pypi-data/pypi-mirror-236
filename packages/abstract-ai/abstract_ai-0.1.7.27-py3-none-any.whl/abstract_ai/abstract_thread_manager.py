import threading
from abstract_utilities import T_or_F_obj_eq,create_new_name,get_last_comp_list,get_sleep
import queue
class ThreadedEvent:
...
def test_it():
    def test_thread(name):
        for i in range(10000):
            print(f'{name} is alive, countdown {i} of 10')

        return 'im dead'

    name = 'test_thread'
    thread_manager = ThreadManager()
    thread_manager.add_thread(name=name,target_function=test_thread,function_args={"name":name})
    thread_manager.start(name)
    thread_manager.stop_last(name)

    thread_manager.get_last_result()
