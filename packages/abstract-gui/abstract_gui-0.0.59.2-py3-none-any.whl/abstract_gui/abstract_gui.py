...
from abstract_utilities.compare_utils import create_new_name
class WindowGlobalBridge:
    ...
    def create_script_name(self,script_name:str='default_script_name'):
        if script_name in self.global_vars:
            script_name = create_new_name(script_name,list(self.global_vars.keys()))
        while script_name in self.global_vars:
            script_number = int(script_name.split('_')[-1])
            script_name = script_name[:-len(str(script_number))]+str(script_number+1)
        return script_name
...
class WindowManager:
    ...
    """
    def __init__(self, script_name=None, global_bridge=None):
        """
        ...
        """
        self.global_bridge=global_bridge or WindowGlobalBridge()
        self.all_windows = {'last_window': None}
        self.script_name = create_script_name(script_name)
    ...
    def register_window(self, window=None):
        ...
        return name
    def get_new_window(self, title:str=None, layout:list=None, args:dict=None, event_function=None, exit_events:(list or str)=None):
        """
        Create a new window.
        ...
        input(event_function)
        self.all_windows[name]["event_function"] = event_function  # store the actual method/function reference, not string
    ...
    def while_basic(self, window=None):
        ...
        while self.verify_window(window):
            ...
            if event_function is not None:
                # Call the event_function directly without looking it up in the global_vars
                result = event_function(self.get_event(window))
                ...
                if result in self.all_windows[self.search_global_windows(window)]["exit_events"]:
                    ...
                    return self.all_windows[self.search_global_windows(window)]["values"]  # Return the stored data instead of all_windows    
        self.close_window(window)
        return self.all_windows  # Return the stored data instead of all_win
