...
import re
from . import *
from abstract_utilities import call_functions,process_args,get_fun,mk_fun,get_closest_match_from_list
window = None
...
def get_user_agents():
    return big_user_agent_list.big_user_agent_list
...
def url_grabber_component(url=None):
    if url==None:
      url = "www.example.com"
    globals()['window'] = get_gui_fun(name='Window',args={'title':'URL Grabber', 'layout':get_gpt_layout(url),**expandable()})
