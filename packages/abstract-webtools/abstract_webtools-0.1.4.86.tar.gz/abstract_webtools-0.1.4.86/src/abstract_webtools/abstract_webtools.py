...
from abstract_utilities.string_clean import eatInner,eatAll,eatOuter
...
class UrlManager:
    ...
    def __init__(self, url='www.example.com', session=requests):
        """
...
class SafeRequest:
    ...
    def __init__(self,
        ...
        if network_manager == None:
            network_manager=NetworkManager(user_agent_manager=user_agent_manager,ssl_manager=ssl_manager, tls_adapter=tls_adapter,user_agent=user_agent,proxies=proxies,cookies=cookies,ciphers=ciphers, certification=certification, ssl_options=ssl_options)
...
class VideoDownloader:
    ...
    def monitor(self):
        while self.monitoring:
            self.pause_event.wait(self.time_interval)# check every minute
            if self.monitoring:
