...
def get_gpt_layout():
    ...
    sg.theme('LightGrey1')
    layout = [[sg.Text('URL:', size=(8, 1)), sg.Input('www.example.com', key='-URL-',enable_events=True),sg.Text('status:'),sg.Text('200',key="-STATUS_CODE-")
               ,sg.Text('success: www.example.com is valid',key="-URL_WARNING-"),sg.Button('correct url',key='-CORRECT_URL-',visible=True)],
        ...
        [sg.Text('find soup:'),[[sg.Checkbox('',default=True,key='-SOUP_TAG_BOOL-',enable_events=True),sg.Combo([], size=(15, 1),key='-SOUP_TAG-',enable_events=True)],
                                ...
                                sg.Input(key='-SOUP_VALUES_INPUT-'), sg.Button('get soup'),sg.Button('all soup')]],
...
def update_url(url_manager,request_manager,soup_manager,link_manager,values,cipher_list=get_cipher_list(),user_agent=get_user_agents()[0]):
    url=values['-URL-']
    if url != url_manager.url or url == "www.example.com":
      url_manager = UrlManager(url=url)
      ciphers = CipherManager(cipher_list=cipher_list).ciphers_string
      request_manager = SafeRequest(url_manager=url_manager,ciphers=ciphers,user_agent={'user_agent':user_agent})
    ...
    return update_source_code(url,url_manager,request_manager,soup_manager,link_manager,values)
def initialise_managers():
    return update_url("www.example.com")
def update_source_code(url,url_manager,request_manager,soup_manager,link_manager,values):
...
def url_grabber_component():
    globals()['window'] = get_gui_fun(name='Window',args={'title':'URL Grabber', 'layout':get_gpt_layout(),**expandable()})
