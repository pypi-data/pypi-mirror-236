...
def predict_abusiveness(text, decimal_places=2):
    ...
    return format_string.format(abusiveness_score[0]), format_string.format(abusiveness_score[1])
...
def predict_abusiveness_file(input_file, output_file, decimal_places=2):
    ...
    with open(output_file, 'w') as output:
        for kalimat in kalimat_list:
            ...
            output.write(f"Positive: {abusiveness_score[0]:.{decimal_places}f}, Negative: {abusiveness_score[1]:.{decimal_places}f}\n\n")
