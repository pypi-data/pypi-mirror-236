...
class EnvironmentVariables:
    ...
    @functools.cached_property
    def _remote_tags(self) -> tp.List[str]:
        out = subprocess_run(
            ["git", "ls-remote", "--tags"], check=True, capture_output=True
        )
        return list(re.findall(r"tags/(.+)\n", out.stdout.decode()))

    @functools.cached_property
    def last_release(self) -> str:
        for tag in reversed(self._remote_tags):
            regex = r"^(v)?(\d+)\.(\d+)\.(\d+)$"
    ...
    def auto_incr_process(self, tag: str) -> str:
        ...
        for candidate in reversed(self._remote_tags):
            regex = f"^{pattern}$"
            match = re.match(regex, candidate)
            if match:
                (incr,) = match.groups()
                incr_val = int(incr) + 1
                break
