...
def load_model(
    ...
    tokenizer =AutoTokenizer.from_pretrained(name_or_path,cache_dir=MODEL_CACHE_DIR, use_fast=False)
    logger.info(f"Tokenizer loaded")
    logger.info(f"Loading model from {name_or_path}")
    model = AutoModelForCausalLM.from_pretrained(name_or_path,cache_dir=MODEL_CACHE_DIR, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True, device_map="auto",load_in_8bit=load_in_8bit)
