...
@Name("invokeMethod")
public class InvokeMethodCommand extends Command<StorageClient> {
    ...
    @Description("A JSON string containing the arguments for the method invocation.")
    ...
    public void execute(CommandLine commandLine, StorageClient client) {
        ...
        client.invokeMethod(key, method, args, lockToken, flags)
                .onSuccess(response -> {
                    try {
                        System.out.println(response.getResult() == null
                                ? "null"
                                : jsonMapper.writerWithDefaultPrettyPrinter().writeValueAsString(response.getResult()));
                    } catch (JsonProcessingException e) {
