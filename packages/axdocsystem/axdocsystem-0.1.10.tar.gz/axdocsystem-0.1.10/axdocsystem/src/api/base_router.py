...
from axsqlalchemy.uow import UOWFactory, UOW

from axdocsystem.db.collection import RepoCollection
from axdocsyste.utils.auth_system import AuthSystem
from settings import JWTSettings


TUOWFactory = UOWFactory[RepoCollection]
TUOW = UOW[RepoCollection]
...
class BaseAuthDependentRouter(APIRouter):
    ...
    async def auth_dependency(self, request: Request, token: str = Depends(OAUTH2_SCHEME)):
        ...
        async with self.uowf() as uow:
            admin = await uow.repo.admin.get(email) 
