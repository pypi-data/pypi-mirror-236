...
down_revision: Union[str, None] = '781e4925ae00'
...
def upgrade() -> None:
    ...
    op.execute(
        f"""
            INSERT INTO admin (email, name, password_hash, is_active, created_at) VALUES 
            ('{settings.FIRST_USER_EMAIL}', '{settings.FIRST_USER_NAME}', '{pass_hash}', true, now());
        """
    )
...
def downgrade() -> None:
    ...
    op.execute(
        f"""
            DELETE FROM admin WHERE email = '{settings.FIRST_USER_EMAIL}'
        """
    )
    # ### end Alembic commands ###
