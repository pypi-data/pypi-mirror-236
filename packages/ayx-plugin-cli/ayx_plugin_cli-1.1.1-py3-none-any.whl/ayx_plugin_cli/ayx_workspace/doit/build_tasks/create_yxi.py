...
def task_create_yxi(omit_ui: bool, dev: bool) -> Generator[Dict, None, None]:
    ...
    if not omit_ui:
        tasks.append("generate_ui_artifacts")  # generate_ui_artifact will only happen for plugins with a ui folder
...
def create_yxi(workspace: AyxWorkspaceV1, dev_mode: bool) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for index, tool in enumerate(workspace.tools.values()):
            ...
            if dev_mode:
                manifest_path = temp_dir_path / tool.get_tool_folder_name() / "manifest.json"
...
def copy_tool(tool: ToolSettingsV1, temp_dir: Path, is_ws_root: bool, dev_mode: bool) -> None:
    ...
    if tool_ui_artifact_dir.is_dir():
        ...
        if dist_dir.is_dir():
            ...
            print(f"WARNING: {tool.backend.tool_class_name} has a ui folder, but nothing was built.")


def make_archive(
    workspace: AyxWorkspaceV1, temp_dir: Path
) -> None:
