...
def task_generate_ui_artifacts() -> Generator[Dict, None, None]:
    ...
    for tool_name in map(lambda x: str(x.backend.tool_class_name), workspace.tools.values()):
        ui_dir = Path(TEMPLATE_TOOL_UI_DIR % {"tool_name": tool_name})
        yield {
            "task_dep": ["generate_config_files"],
            "file_dep": list(
                filter(lambda p: p.is_file(), itertools.chain(ui_dir.glob("src/**/*"), ui_dir.glob("package*.json")))
            ),
...
def _build_thirdparty_deps_command(language: BackendLanguage, cache_dir: Path, dev_mode: bool) -> List[str]:
    try:
        return {
            BackendLanguage.Python: [
                sys.executable,
...
def _install_local_python_deps(cwd: Path, req_file: Path, cache_dir: Path, dev: bool) -> None:
...
def _backend_artifact_common(dev_mode: bool) -> Generator[Dict, None, None]:
    ...
    if workspace.backend_language == BackendLanguage.Python:
        tp_cmd = _build_thirdparty_deps_command(workspace.backend_language, BUILD_CACHE_DIR, dev_mode)
        yield {
            "task_dep": ["generate_config_files"],
        ...
        yield {
            ...
            "actions": [
                ...
                (_install_local_python_deps, [BACKEND_PATH, AYX_WORKSPACE_FILES[workspace.backend_language][1], BUILD_CACHE_DIR, dev_mode])
            ],
            "targets": [
