...
class Dark(Tester):
    ...
    def analyze(self):
        ...
        if not self.overscan_correct:
            ...
            for darkfile in imagelist:
                azcam.fits.sub(darkfile, bias.superbias_filename, darkfile)
    ...
    def plot(self):
        ...
        # plt.xlim(0.0, self.mean_dark_signal * self.units_scale * 10.0)
        ...
        if self.dark_limit > 0:
            plt.axhline(y=self.dark_fraction, linestyle="--", color="red", label="Fraction")
            plt.axvline(
        ...
        plt.xlim(
            [
                self.mean_dark_signal - self.sdev_dark_signal * self.units_scale * 10,
                self.mean_dark_signal + self.sdev_dark_signal * self.units_scale * 10,
            ]
        )
        plt.show()
        ...
        # plt.ylim(0.0, 1.0)
        # ax.set_yticks([0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00])
        # zcam_console.plot.plt.xlim(0.0, self.mean_dark_signal * self.units_scale * 10.0)
    ...
    def report(self):
        ...
        if "total_hist" in self.report_plots:
            lines.append(f"![Total Histogram]({os.path.abspath(self.total_hist_plot)})  ")
