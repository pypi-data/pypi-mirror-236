...
class Zone:
    ...
    def AddMember(self, member:ZoneMember):
        """ 
        ...
        Raises:
            ValueError:
                Raised if the member argument is not of type `ZoneMember`.  
                Raised if the member object DeviceId property is not set.
        ...
        if member: 
            if not isinstance(member, ZoneMember):
                raise ValueError('Zone member to be added is not a ZoneMember object: %s' % str(member))
            if member.DeviceId is None:
                raise ValueError('Zone member to be added did not specify a DeviceId: %s' % member.ToString())
            if member.DeviceId == self.MasterDeviceId:
                raise ValueError('Zone member DeviceId to be added or removed cannot be the master DeviceId: %s' % member.DeviceId)
