...
class SoundTouchClient:
    ...
    def _CheckResponseForErrors(self, element:Element):
        """
        Checks a device response for errors.  If found, a `ConnectionError`
        ...
        if (element.tag in ['errors', 'error', 'Error']):
            ...
            # find the error portion of the message and raise a ConnectionError.
    ...
    def MakeRequest(self, method:str, msg:SoundTouchMessage) -> int:
        ...
        try:
            ...
            if _logsi.IsOn(SILevel.Verbose):
                _logsi.LogSource(SILevel.Verbose, "SoundTouchClient http response: %s" % (response.status), response.data.decode("utf-8"), SISourceId.Xml)
                # if (response.headers):
                #     _logsi.LogCollection(SILevel.Verbose, "SoundTouchClient http response headers", response.headers.items())
    ...
    def PlayUrl(self, url:str, artist:str=None, album:str=None, track:str=None, 
        ...
        if not re.match(r'http://', url):
            raise SoundTouchException("url argument value does not start with 'http://'; https url's are not supported by SoundTouch devices")
    ...
    def Put(self, uri:SoundTouchUri, body:str) -> SoundTouchMessage:
        ...
        Raises:
            ConnectionError:
                When errors should not be ignored on this client, they will raise a 
                ConnectionError with all information related to that error.
