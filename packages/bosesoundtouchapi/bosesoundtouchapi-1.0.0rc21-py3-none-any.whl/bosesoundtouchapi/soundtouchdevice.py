...
class SoundTouchDevice:
    ...
    def __init__(self, host:str, connectTimeout:int=30, proxyManager:ProxyManager=None, port:int=8090) -> None:
        ...
        try:
            ...
            # # get SoundTouch device mac address; if it fails then we are done.
            # _logsi.LogVerbose("Retrieving SoundTouch device MacAddress information.")
            # reqUrl:str = f'http://{host}:{port}/netStats'
            # response:HTTPResponse = manager.request('GET', reqUrl)
            # if response.status != 200:
            #     raise Exception("Could not retrieve SoundTouch device MacAddress: (%s) - '%s'" % (response.status, reqUrl))
