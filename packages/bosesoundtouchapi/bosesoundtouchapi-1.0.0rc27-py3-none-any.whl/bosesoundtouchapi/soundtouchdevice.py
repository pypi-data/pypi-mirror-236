    ...
    def __init__(self, host:str, connectTimeout:int=30, proxyManager:ProxyManager=None, port:int=8090) -> None:
        ...
        try:
            ...
            if response.status != 200:
                raise Exception("Could not retrieve SoundTouch device info: (%s) - '%s'" % (response.status, reqUrl))
            ...
            response:HTTPResponse = manager.request('GET', reqUrl)
            if response.status != 200:
                raise Exception("Could not retrieve SoundTouch device supported urls: (%s) - '%s'" % (response.status, reqUrl))
