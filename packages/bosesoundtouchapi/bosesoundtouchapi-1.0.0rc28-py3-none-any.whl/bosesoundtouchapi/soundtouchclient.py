...
class SoundTouchClient:
    ...
    def __init__(self, device:SoundTouchDevice, raiseErrors:bool=True, manager:PoolManager=None) -> None:
        ...
        self._Manager:PoolManager = PoolManager(headers={'User-Agent': 'BoseSoundTouchApi/1.0.0'})
    ...
    def _GetMetadataFromUrl(self, url:str) -> TinyTag:
        ...
        try:
            ...
            # use TinyTag package to retrieve metadata from ID3v2 header.
            metadata = TinyTag.get(file_obj=header)
    ...
    def AddZoneMembers(self, members:list, delay:int=3) -> SoundTouchMessage:
        """
        ...
        if not members or len(members) == 0:
            raise ValueError('Members argument was not supplied, or has no members')
        ...
        if masterZone is None:
            raise SoundTouchException('Master zone status could not be retrieved')
        if len(masterZone.Members) == 0:
            raise SoundTouchException('Master zone does not exist; zone members cannot be added')
    ...
    def CreateZone(self, zone:Zone, delay:int=3) -> SoundTouchMessage:
        ...
        # validations.
        if zone is None:
            raise ValueError('Zone argument was not supplied')
        if not isinstance(zone, Zone):
            raise ValueError('Zone argument is not of type Zone')
        if len(zone.Members) == 0:
            raise ValueError('Zone argument object did not contain any members; the zone must have at least one zone member')
        delay = self._ValidateDelay(delay, 3, 10)
    ...
    def CreateZoneFromDevices(self, master:SoundTouchDevice, members:list) -> Zone:
        ...
        """
        if master is None:
            raise ValueError('Master argument was not supplied')
        if not isinstance(master, SoundTouchDevice):
            raise ValueError('Master argument is not of type SoundTouchDevice')
        if not isinstance(members, list):
            raise ValueError('Members argument is not of type list')
        ...
        for member in members:
            if not isinstance(member, SoundTouchDevice):
                raise ValueError('Member argument contained an entry in the list that is not of type SoundTouchDevice: %s' % str(member))
            zone.AddMember(ZoneMember(member.Host, member.DeviceId))
    ...
    def MakeRequest(self, method:str, msg:SoundTouchMessage) -> int:
        ...
        try:
            if msg.HasXmlMessage:
                ...
                _logsi.LogSource(SILevel.Verbose, "SoundTouchClient http request: '%s' (with body)" % (url), reqbody, SISourceId.Xml)
            ...
            if _logsi.IsOn(SILevel.Verbose):
                _logsi.LogSource(SILevel.Verbose, "SoundTouchClient http response: (%s) %s" % (response.status, url), response.data.decode("utf-8"), SISourceId.Xml)
    ...
    def PlayNotificationTTS(self, sayText:str, ttsUrl:str=None, 
        ...
        if sayText is None or len(sayText) == 0:
            _logsi.LogVerbose("sayText argument was not supplied to PlayNotificationTTS method; ignoring request")
            return
        
        if ttsUrl is None:
            ttsUrl:str = "http://translate.google.com/translate_tts?ie=UTF-8&tl=EN&client=tw-ob&q={saytext}"
        if not isinstance(ttsUrl, str):
            _logsi.LogVerbose("ttsUrl argument was not a string; ignoring PlayNotificationTTS request")
            return
        ttsUrl = ttsUrl.lstrip()
        if not re.match(r'http://', ttsUrl):
            raise SoundTouchException("ttsUrl argument value does not start with 'http://'; https url's are not supported by SoundTouch devices")
    ...
    def PlayUrl(self, url:str, artist:str=None, album:str=None, track:str=None, 
        ...
        if not re.match(r'http[s]?://', url):
           raise SoundTouchException("url argument value does not start with 'http://' or 'https://'")
    ...
    def RemoveZone(self, delay:int=1) -> SoundTouchMessage:
        ...
        if masterZone is None:
            raise SoundTouchException('Master zone status could not be retrieved')
        if len(masterZone.Members) == 0:
            raise SoundTouchException('Master zone does not exist; zone members cannot be removed')
    ...
    def RemoveZoneMembers(self, members:list, delay:int=3) -> SoundTouchMessage:
        ...
        # validations.
        if not members or len(members) == 0:
            raise ValueError('Members argument contained no zone members to remove')
    ...
    def SelectPreset(self, preset:Preset, delay:int=5) -> SoundTouchMessage:
        ...
        if not preset:
            raise ValueError('Preset argument was not supplied')
        delay = self._ValidateDelay(delay, 5, 10)
    ...
    def SelectRecent(self, recent:Recent, delay:int=5) -> SoundTouchMessage:
        ...
        if not recent:
            raise ValueError('Recent argument was not supplied')
        delay = self._ValidateDelay(delay, 5, 10)
    ...
    def SelectSource(self, source:SoundTouchSources, sourceAccount:str=None, delay:int=3) -> SoundTouchMessage:
        ...
        """
        if not source:
            raise ValueError('Source argument was not supplied')
