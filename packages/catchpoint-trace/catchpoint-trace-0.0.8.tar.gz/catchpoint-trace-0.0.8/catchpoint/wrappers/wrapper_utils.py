...
def set_error(invocation_data, error):
    ...
    if isinstance(error, Exception):
        ...
        invocation_data['tags']['error'] = True
        invocation_data['tags']['error.name'] = error_type.__name__
        invocation_data['tags']['error.message'] = str(error)
        invocation_data['tags']['error.stack'] = ''.join(traceback.format_tb(error.__traceback__))

        if hasattr(error, 'code'):
            invocation_data['errorCode'] = error.code
            invocation_data['tags']['error.code'] = error.code
        ...
        invocation_data['tags']['error'] = True
        invocation_data['tags']['error.name'] = error.get('type')
