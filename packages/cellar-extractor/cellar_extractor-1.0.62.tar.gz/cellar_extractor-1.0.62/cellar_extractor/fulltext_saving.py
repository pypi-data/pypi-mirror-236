...
def execute_sections_threads(celex, eclis, start, list_sum, list_key, list_full, list_codes, list_eurovoc, list_adv,
                             list_judge, list_affecting_id, list_affecting_str,list_citations_extra,
                             list_operative_part, progress_bar):
    ...
    operative_part = pd.Series([], dtype='string')
    ...
    list_operative_part.append(operative_part)
...
def add_sections(data, threads, json_filepath=None):
    ...
    for i in range(0, length, at_once_threads):
        ...
        t = threading.Thread(target=execute_sections_threads,
                             args=(
                                 ...
                                 list_adv, list_judge, list_affecting_id, list_affecting_str,list_citations_extra,list_operative_part, bar))
        threads.append(t)
    for t in threads:
        t.start()
    for t in threads:
    ...
    add_column_frow_list(data,'operative_part',list_operative_part)
    if json_filepath:
