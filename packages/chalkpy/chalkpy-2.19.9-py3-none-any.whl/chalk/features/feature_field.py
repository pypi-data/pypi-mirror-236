...
class Feature(Generic[_TPrim, _TRich]):
    ...
    def converter(self) -> FeatureConverter:
        ...
        if self.typ.is_document:
        ...
        elif self.typ.is_proto and ProtobufMessage is not None:
            def proto_encoder(x: _TRich):
                if isinstance(x, ProtobufMessage):
                    return x.SerializeToString()

            def proto_decoder(x: _TPrim):
                if isinstance(self.typ.proto_class, type) and issubclass(self.typ.proto_class, ProtobufMessage):
                    message = self.typ.proto_class()
                    message.ParseFromString(x)
                    return message

            self._decoder = self._decoder or proto_decoder
            self._encoder = self._encoder or proto_encoder
            self._pyarrow_dtype = self._pyarrow_dtype or pa.large_binary()
            ...
            self._pyarrow_dtype = self._pyarrow_dtype or pa.large_list(pa.struct(
                {k: v.converter.pyarrow_dtype for k, v in column_features.items()}
            ))
