...
class Post:
    ...
    def addInMemory(self, content: Union[Figure, Group]):
        ...
        if(isinstance(content, Group)):
            ...
            self.chartInfo.append({'grpId':content.grp_id, 'layout': content.layout, 'charts':  charts})
        else:
            self.chartInfo.append({'grpId':content.chart_id, 'layout': None ,'charts': [content.data.to_dict()]})

    def getInmemoryPostInfo(self):
        # get post info without any figure/group info
        tmpPostInfo = self.getPostData()
        tmpPostInfo['rows'] = self.chartInfo
        return tmpPostInfo
