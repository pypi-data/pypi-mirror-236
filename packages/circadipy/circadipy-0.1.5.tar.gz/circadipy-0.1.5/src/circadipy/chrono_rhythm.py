...
def total_activity_per_day(protocol, save_folder=None, save_suffix=""):
    ...
    if save_folder != None:
        save_file = (
            save_folder + "/total_activity_" + protocol.name.lower().replace(" ", "_") + "_" + save_suffix + ".xlsx"
        )
...
def fit_cosinor(protocol, dict=None, save_folder=None, save_suffix=""):
    ...
    dict_default = {"time_shape": "continuous", "step": 0.1, "start_time": 20, "end_time": 30, "n_components": [1]}
...
def fit_cosinor_fixed_period(protocol, best_models, save_folder=None, save_suffix=""):
    ...
    for count, label in enumerate(best_models["test"]):
        ...
        for count, day in enumerate(range(0, len(set_of_days))):
            ...
            if (
                len(best_model_day) == 0 or best_model_day["amplitude"][0] <= 0.01
            ):  # If the model can't be fitted or the amplitude is too low (threshold), the model isn't considered
    ...
    best_models_per_day = _set_significant_results(
        best_models_per_day
    )  # Create a column indicating if the results are significant
...
def fit_cosinor_per_day(protocol, dict=None, plot=False, save_folder=None, save_suffix=""):
    ...
    dict_default = {"day_window": 1, "step": 0.1, "start_time": 20, "end_time": 30, "n_components": [1]}
    ...
    if plot:  # If plot is True, create a figure
        fig, ax = plt.subplots(
            round(len(set_of_days) / 10) + 1, 10, figsize=(40, 40), sharey=True
        )  # Create a figure with a subplot for each day
    ...
    for count, day in enumerate(range(0, len(set_of_days) - day_window + 1)):  # Loop through the days
        ...
        if day_window >= 2:  # If the user want to use data from multiple days (set by day_window)
            for d in range(1, day_window):  # Loop through the days
                day_for_window_df = protocol_df[
                    protocol_df["day"] == set_of_days[day + d]
                ]  # Get the data from the next day
                day_for_window_df["x"] = day_for_window_df["x"] + 24 * d  # Add 24 hours to the time for the next day
                day_df = pandas.concat(
                    [day_df, day_for_window_df], axis=0
                )  # Concatenate the current day data with the data from the next day

        result_day = cosinor.fit_group(
            day_df, n_components=n_components, period=periods, plot=False
        )  # Fit the cosinor model to the data for the current day
        best_model_day = cosinor.get_best_fits(
            result_day, n_components=n_components
        )  # Get the best model parameters for the current day

        if (
            len(best_model_day) == 0 or best_model_day["amplitude"][0] <= 0.1
        ):  #  If the model can't be fitted or the amplitude is too low (threshold), the model isn't considered
            best_model_day.loc[0] = pandas.np.nan
            best_model_day_extended = best_model_day
        else:
        ...
        best_model_day_extended.insert(
            2, "first_hour", day_df["x"][0]
        )  # Add the first hour of the day to the best model parameters dataframe
        best_model_day_extended = _acrophase_ci_in_zt(best_model_day_extended)  # Reset the index of the dataframe
        best_models_per_day = pandas.concat(
            [best_models_per_day, best_model_day_extended], axis=0
        )  # Concatenate the best model parameters for the current day with the best model parameters for all the days

        if plot:  # If plot is True
            ax[count].bar(day_df["x"], day_df["y"], color="dimgray")
            ticks_to_plot = numpy.linspace(
                day_df["x"][0], day_df["x"][-1], num=5, endpoint=True
            )  # Create a list of time points change the ticks on the x-axis
            ...
            if not numpy.isnan(m_p_value) and m_p_value < 0.05:
                ...
                model = m_amplitude * numpy.cos(
                    numpy.multiply(2 * numpy.pi * m_frequency, day_df["x"]) + m_acrophase
                )  # Get the model
                ...
                ax[count].set_title(
                    set_of_days[day]
                    + "\n(PR: "
                    + str(round(m_period, 2))
                    + ", AC: "
                    + str(round(m_acrophase_zt, 2))
                    + ")",
                    fontsize=20,
                )
            else:
...
def derivate_acrophase(best_models_per_day):
    acrophases_zt = numpy.array(
        best_models_per_day["acrophase_zt"].interpolate(method="spline", limit_direction="both", order=3)
    )
