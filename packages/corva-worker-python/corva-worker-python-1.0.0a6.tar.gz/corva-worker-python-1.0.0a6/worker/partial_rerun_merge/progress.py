...
class MergingProgress:
    ...
    def complete_collection(self, collection_name: str) -> None:
        """
        Marks the specified collection as completed and sets the processed
        timestamp to the current time.

        Args:
            collection_name (str): The name of the collection to mark as completed.
        """
        for dataset_progress in self.dataset_progresses:
            if dataset_progress.get_trimmed_dataset_name() == collection_name:
                dataset_progress.processed_timestamp = int(time.time())
    ...
    def determine_if_cache_update_completed(self) -> bool:
        ...
        """
        return any(dataset_progress.is_completed() for dataset_progress in self.dataset_progresses)
