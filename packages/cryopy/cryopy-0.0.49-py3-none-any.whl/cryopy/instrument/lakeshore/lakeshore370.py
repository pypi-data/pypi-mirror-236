...
def update_excitation(address, channel, temperature):
    ...
    if temperature >= 1.2:
        excitation = 7
        asked_mode,asked_excitation,asked_range_value,asked_autorange,asked_autoexcitation = lakeshore370.query_resistance_range(address, channel)
        if asked_excitation != excitation:
            lakeshore370.command_resistance_range(address, channel, asked_mode, excitation, asked_range_value, asked_autorange,asked_autoexcitation)
        else:
            return

    if temperature < 1.2 and temperature >= 0.1:
        excitation = 6
        asked_mode,asked_excitation,asked_range_value,asked_autorange,asked_autoexcitation = lakeshore370.query_resistance_range(address, channel)
        if asked_excitation != excitation:
            lakeshore370.command_resistance_range(address, channel, asked_mode, excitation, asked_range_value, asked_autorange,asked_autoexcitation)
