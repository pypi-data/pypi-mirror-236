...
class Strategy:
    ...
    async def on_opened_trade(self, strategy, trade: OpenedTrade):
        logging.info(f"Received opened trade: {trade.__repr__()}")

    async def on_closed_trade(self, strategy, trade: ClosedTrade):
        logging.info(f"Received closed trade: {trade.__repr__()}")

    async def on_market_update(self, strategy, equity: FloatWithTime, available_balance: FloatWithTime):
        logging.info(f"Received market update: equity({equity.__repr__()}), available_balance({available_balance.__repr__()})")
