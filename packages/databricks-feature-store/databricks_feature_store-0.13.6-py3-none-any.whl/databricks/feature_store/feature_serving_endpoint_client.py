...
from databricks.feature_store.utils.utils_common import get_unique_list_order
...
class FeatureServingEndpointClient:
    ...
    def _create_dummy_training_df(
        ...
        feature_table_features_map = (
            self._training_scoring_client._get_feature_names_for_tables(
                req_context, table_names=table_names
            )
        )
        feature_table_metadata_map = (
            self._training_scoring_client._get_feature_table_metadata_for_tables(
                req_context, table_names=table_names
            )
        )
        feature_column_infos = self._training_scoring_client._explode_feature_lookups(
        ...
        on_demand_input_names = [
            (input_name, odci)
            for odci in on_demand_column_infos
            for input_name in odci.input_bindings.values()
        ]

        # Get missing inputs by comparing against feature columns
        missing_inputs = [
            (column_name, odci)
            for (column_name, odci) in on_demand_input_names
            if column_name not in feature_output_names
        ]

        # TODO: Add logic for missing lookup keys and add to dummy training dataframe

        uc_function_infos = self._training_scoring_client._get_uc_function_infos(
            {odci.udf_name for odci in on_demand_column_infos}
        )

        # schema is a list of human readable datatype strings such as "a int"
        schema = []

        for input_name, odci in missing_inputs:
            ...
            schema.append(f"{input_name} {input_param.type_text}")

        return self._spark.createDataFrame([], ",".join(schema))
