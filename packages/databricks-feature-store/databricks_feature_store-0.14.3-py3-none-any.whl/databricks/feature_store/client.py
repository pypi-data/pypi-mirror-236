...
from databricks.feature_store._publish_client._publish_client import PublishClient
...
from databricks.feature_store.feature_serving_endpoint_client import (
...
class FeatureStoreClient:
    ...
    def drop_online_table(
        ...
        API for dropping a table in an online store. This API first attempts to make a
        call to the online store provider to drop the table. If successful, it then
        deletes the online store from the feature catalog.
    ...
    def create_training_set(
        ...
        **kwargs,
        ...
        # Retrieve true feature_lookups, feature_functions and their indexes in the features list.
        fl_idx = []
        ff_idx = []
        feature_lookups = []
        feature_functions = []
    ...
    @staticmethod
    def _validate_endpoint_name_specified(name, legacy_name):
    ...
    def create_feature_serving_endpoint(
        ...
        **kwargs,
        ...
        """
        # Get legacy parameters from kwargs to be backward compatible.
        # the values are None if not set in kwargs.
        legacy_endpoint_name = kwargs.get("endpoint_name")
        legacy_features = kwargs.get("features")

        FeatureStoreClient._validate_endpoint_name_specified(name, legacy_endpoint_name)
        ...
        if config is not None:
            if legacy_features is not None:
                raise ValueError(
                    "features, workload_size, scale_to_zero_enabled, extra_pip_requirements "
                    "should only be specified in config"
                )
            # deepcopy the input because we may modify the object.
            config = copy.deepcopy(config)
        else:
            # fallback to legacy parameters if config is not specified.
            features = legacy_features
            if features is None:
                raise ValueError("features must be specified in config.servables.")

            _logger.warning(
                "features, workload_size, scale_to_zero_enabled, extra_pip_requirements is "
                "deprecated in favor of config. These parameters will be removed in future MLR."
            )
            config = EndpointCoreConfig(Servable(features))

        # TODO [ML-32590]: De-dupe this logic with create_training_set
        # Retrieve true feature_lookups, feature_functions and their indexes in the features list.
        fl_idx = []
        ff_idx = []
        feature_lookups = []
        ...
        for idx, feature in enumerate(servables.features):
            if isinstance(feature, FeatureLookup):
                fl_idx.append(idx)
                feature_lookups.append(feature)
            elif isinstance(feature, FeatureFunction):
                ...
                raise ValueError(
                    f"Expected a list of FeatureLookups and FeatureFunctions for 'features', but received type '{type(feature)}'."
                )
        ...
        # Restore original order of FeatureLookups, FeatureFunctions.
        for idx, feature in zip(fl_idx + ff_idx, feature_lookups + feature_functions):
            servables.features[idx] = feature
