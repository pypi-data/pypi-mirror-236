...
class LearnerBase:
    ...
    def fit(self, train_dl, val_dl=None):
        ...
        self.callbacks.trigger('before_fit',
                               learner=self,
                               total_epochs=self.epochs,
                               total_train_batchs=len(train_dl),
                               total_val_batchs=None if val_dl is None else len(val_dl)
                               )
        try:
            for epoch_idx in range(self.epochs):
                self.callbacks.trigger('before_train_epoch',
                                       learner=self,
                                       epoch_idx=epoch_idx
                                       )
                # training
                ...
                for batch_idx, (batch_x, batch_y) in enumerate(train_dl):
                    self.callbacks.trigger('before_train_batch',
                                           learner=self,
                                           batch_x=batch_x,
                                           batch_y=batch_y,
                                           batch_idx=batch_idx)
                    ...
                    self.callbacks.trigger('after_train_batch',
                                           learner=self,
                                           metrics=train_batch_metrics,
                                           batch_idx=batch_idx)
                with torch.no_grad():
                    # 计算当前epoch的指标
                ...
                if val_dl is not None and (epoch_idx + 1) % self.val_freq == 0:
                    ...
                    with torch.no_grad():
                        for batch_idx, (batch_x, batch_y) in enumerate(val_dl):
                            self.callbacks.trigger('before_val_batch',
                                           learner=self,
                                           batch_x=batch_x,
                                           batch_y=batch_y,
                                           batch_idx=batch_idx)
                            ...
                            self.callbacks.trigger('after_val_batch',
                                                   learner=self,
                                                   metrics=val_batch_metrics,
                                                   batch_idx=batch_idx)
                        # 计算当前epoch的指标
                        val_metrics = flatten_dict(exec_dicts(val_metrics), sep='')
                        progress['val'].append(val_metrics)
                    self.callbacks.trigger('after_val_epoch',
                                           learner=self,
                                           metrics=val_metrics
                                           )
                self.callbacks.trigger('after_train_epoch',
                                       learner=self,
    ...
    def test(self, test_dl):
        ...
        with torch.no_grad():
            for batch_idx, (batch_x, batch_y) in enumerate(test_dl):
                self.callbacks.trigger('before_test_batch',
                                        learner=self,
                                        batch_x=batch_x,
                                        batch_y=batch_y,
                                        batch_idx=batch_idx)
                ...
                self.callbacks.trigger('after_test_batch',
                                        learner=self,
                                        metrics=test_batch_metrics,
                                        batch_idx=batch_idx)
            # 计算当前epoch的指标
            test_metrics = flatten_dict(exec_dicts(test_metrics), sep='')
        self.callbacks.trigger('after_test_epoch',
                                learner=self,
