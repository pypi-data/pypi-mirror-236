...
class TrainerBase:
    def __init__(self, model:torch.nn.Module, loss:Callable=None,
        ...
        Args:
            model:      Pytorch模型
            loss:       损失函数
            opt:        取值可以是pytorch优化器或deepepochs.Optimizer对象，也可以是优化器列表或Optimizer列表。
            epochs:     迭代次数
            metrics:    指标
            device:     cpu或cuda
            val_freq:   验证频率
        ...
        if opt is None:
            ...
            self.opt = Optimizers([Optimizer(o) if isinstance(opt, torch.optim.Optimizer) else o for o in opt])
        else:
