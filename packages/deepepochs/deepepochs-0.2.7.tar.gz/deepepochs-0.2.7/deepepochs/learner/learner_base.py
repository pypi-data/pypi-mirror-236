...
class LearnerBase:
    def __init__(self, model:torch.nn.Module, loss:Callable=None,
                 opt:[torch.optim.Optimizer, Optimizer]=None, epochs:int=1000,
        ...
        Args:
            model:      Pytorch模型
            loss:       损失函数
            opt:        取值可以是pytorch优化器、(优化器, scheduler)元组、或deepepochs.Optimizer对象
            epochs:     迭代次数
            metrics:    指标
            device:     cpu或cuda
            val_freq:   验证频率
        ...
        try:
            for epoch_idx in range(self.epochs):
                ...
                for batch_idx, batch_data in enumerate(train_dl):
                    ...
                    train_ms = self.train_step(batch_x, batch_y)
                ...
                if val_dl is not None and (epoch_idx + 1) % self.val_freq == 0:
                    ...
                    with torch.no_grad():
                        for batch_idx, batch_data in enumerate(val_dl):
                            ...
                            val_ms = self.evaluate_step(batch_x, batch_y)
