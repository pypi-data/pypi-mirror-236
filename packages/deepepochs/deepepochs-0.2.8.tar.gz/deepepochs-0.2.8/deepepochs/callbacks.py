...
import time
from .loops import log_batch, log_epoch, check_path, StopLoopExcption
...
class DefaultCallback(Callback):
    ...
    def on_after_train_epoch(self, learner, train_metrics, val_metrics, epoch_idx):
        ...
        # 优化器学习率调度
        if learner.opt.sched_with_loss and 'loss' in val_metrics:
            learner.opt.step(at='epoch', loss=val_metrics['loss'])
        else:
