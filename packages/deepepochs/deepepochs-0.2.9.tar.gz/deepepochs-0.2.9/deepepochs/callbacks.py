...
import time
from .loops import log_batch, log_epoch, check_path, StopLoopException, LoopExcException
...
class DefaultCallback(Callback):
    ...
    def on_after_train_epoch(self, learner, train_metrics, val_metrics, epoch_idx):
        ...
        # 优化器学习率调度
        if learner.opt.sched_with_loss:
            if val_metrics and 'loss' in val_metrics:  # 优先使用验证损失
                sched_loss = val_metrics['loss']
            elif 'loss' in train_metrics:
                sched_loss = train_metrics['loss']
            else:
                raise LoopExcException("学习率调度要求损失作为参数，但`train_step`和`evaluate_step`都没有`loss`！")
            learner.opt.step(at='epoch', loss=sched_loss)
        else:
            learner.opt.step(at='epoch')
    ...
    def on_after_train_epoch(self, learner, train_metrics, val_metrics, epoch_idx):
        if val_metrics is not None:
            if not self.check(val_metrics, learner.model):
                raise StopLoopException("Early stopping triggered!")
