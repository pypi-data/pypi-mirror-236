...
class post_install(install):
    def run(self):
        ...
        path = get_python_lib() + '/deepncclplugin/'
        if os.path.exists('/etc/ld.so.conf.d/deepncclplugin.conf'):
            os.system(f'echo {path} >> "/etc/ld.so.conf.d/deepncclplugin.conf"')
        else:
            os.system(f'echo {path} > "/etc/ld.so.conf.d/deepncclplugin.conf"')
        # get index path
        if new_pip_version():
        ...
        os.system(f'echo {real_path}/deepncclplugin/ >> "/etc/ld.so.conf.d/deepncclplugin.conf"')
        ...
        os.system(f'ldconfig')
        # print('echo deep_nccl install done!')
...
setup(
    ...
    package_dir={package_name: "deepncclplugin"},
    package_data={'deepncclplugin': ['libnccl-net.so.0.0.0']},
