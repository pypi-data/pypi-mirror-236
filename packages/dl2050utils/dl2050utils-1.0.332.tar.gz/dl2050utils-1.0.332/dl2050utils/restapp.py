...
The downstream services routes include:
    ...
    /api/publish_job
    /api/get_jobs
    /api/get_pending_jobs
    /api/get_not_done_jobs
    /api/get_job
...
class App():
    ...
    def get_routes(self):
        BASE_ROUTES = [
            ...
            Route('/api/get_not_done_jobs', endpoint=self.get_not_done_jobs, methods=['POST']),
    ...
    async def get_jobs(self, request):
        """
            Gets all jobs from a user.
        """
        u = await self.auth.check_auth(request)
        uid = u['uid']
        email = await self.auth.get_email_from_uid(uid)
        jobs = await self.mq.get_jobs(email)
        return rest_ok(jobs)

    @requires('authenticated')
    async def get_pending_jobs(self, request):
        """
            Gets all pending jobs from a user.
        """
        u = await self.auth.check_auth(request)
        uid = u['uid']
        email = await self.auth.get_email_from_uid(uid)
        jobs = await self.mq.get_pending_jobs(email)
        return rest_ok(jobs)
    
    @requires('authenticated')
    async def get_not_done_jobs(self, request):
        """
            Gets all jobs except the ones done from a user.
        """
        u = await self.auth.check_auth(request)
        uid = u['uid']
        email = await self.auth.get_email_from_uid(uid)
        jobs = await self.mq.get_not_done_jobs(email)
        return rest_ok(jobs)
