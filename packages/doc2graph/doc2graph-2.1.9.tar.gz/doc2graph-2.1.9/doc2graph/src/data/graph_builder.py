...
def box_distance(box1: Tuple[int,int,int,int], 
    ...
    if intersectoin_by_axis('y',box1, box2)>0.25:
        x_dist = 0
        # if box1[0]>box2[2]:
        #     x1_center = box1[0]
        #     x2_center = box2[2]
        ...
        #     x2_center = box2[0]
        #     x_dist = abs(x2_center - x1_center)
        # elif (box1[0]<=box2[2]) and (box2[0]<=box1[2]):
        #     x_dist = 0
    else:
    ...
    if intersectoin_by_axis('x',box1, box2)>0.25:
        y_dist = 0
    else:
        # if box1[1]>box2[3]:
        #     y1_center = box1[1]
        ...
        #     y2_center = box2[1]
        #     y_dist = abs(y2_center - y1_center)
        # elif (box1[1]<=box2[3]) and (box2[1]<=box1[3]):
        #     y_dist = 0
        # else:
...
class GraphBuilder():
    ...
    def half_fully_connected3(self, 
                              bboxs : List[Tuple[int,int,int,int]], 
                              texts:  List[str], 
                              max_depth=-1,
                              kind='word_to_notword'):
        ...
        for ix, box_main in enumerate(bboxs):
            ...
            # for box in boxes_x_intersected[:1]:
            #     print(texts[box[0]])
            #     print(intersectoin_by_axis('x',box[1], box_main))
            #     print(intersectoin_by_axis('y',box[1], box_main))
            # print('---')

            #left
            for ind, box in enumerate(boxes_to_up):
                ...
                boxes_to_up = boxes_to_up[ind+1:]
                if box in boxes_x_intersected:
                    boxes_to_up = [x for x in boxes_to_up if x[1][2]+x[1][0]>2*box[1][2]]
                elif box in boxes_y_intersected:
                    boxes_to_up = [x for x in boxes_to_up if x[1][3]+x[1][1]>2*box[1][3]]
                else:
                     boxes_to_up = [x for x in boxes_to_up if max(intersectoin_by_axis('x',x[1], box[1]), intersectoin_by_axis('y',x[1], box[1]))<min_share]
                ...
                u.append(box[0])
                v.append(ix)
                #print(texts[ix],'->',texts[box[0]])
                break
            
            #up
            for box in boxes_to_up:
                target_word = find_word(texts[box[0]]) or find_words(texts[box[0]])
    ...
    def __fromWB(self, paths : list, word_boxes_list : list, max_depth = 1):
        ...
        for word_boxes in word_boxes_list:
            ...
            u, v = self.half_fully_connected3(boxes, words, max_depth, self.word_to_word)
            ...
            for i in range(len(boxes)):
                ...
                if len(dist)>1:
                    ...
                    neighbors = [v for u,v in zip(u,v) if u==i and p_dist[i,v]<=max(dist)]
            ...
            g = dgl.graph((torch.tensor(links['src']), torch.tensor(links['dst'])), num_nodes=len(boxes), idtype=torch.int32)
            graphs.append(g)
