...
class Client:
	...
	def transfer(self, data: dict = {}):
		...
		response = requests.post("https://pay.ton-rocket.com/app/transfer", headers=self.headers, json=data).json()
		return response
	...
	def withdrawal(self, data: dict = {}):
		...
		response = requests.post("https://pay.ton-rocket.com/app/withdrawal", headers=self.headers, json=data).json()
		return response
	...
	def create(self, data: dict = {}):
		...
		"""
		response = requests.post("https://pay.ton-rocket.com/tg-invoices", headers=self.headers, json=data).json()
		return response

	def get_me(self,):
		response = requests.get("https://pay.ton-rocket.com/tg-invoices?limit=100&offset=0", headers=self.headers).json()
		return response

	def check(self, id: int = 0):
		response = requests.get(f"https://pay.ton-rocket.com/tg-invoices/{id}", headers=self.headers).json()
		return response

	def delete(self, id: int = 0):
		response = requests.delete(f"https://pay.ton-rocket.com/tg-invoices/{id}", headers=self.headers).json()
		return response