...
join doppler.\"DopplerProject\" P on P.\"UserKey\"=Du.\"UserKey\" where Du.\"LoginName\"='{0}' and p.\"ProjectName\"='{1}'".format(loginName,Project_name)
        ...
        update_query = "UPDATE doppler.\"DopplerLake\" set \"ConnectionString\"='"+ConnectionString+"',"+"\"Schema\"='"+loginName+'_schema'+"',\"Source\"= 'MLStudio' ,\"CreatedTs\"=CURRENT_TIMESTAMP where \"SourceKey\"="+str(SourceKey)
        cur2=cnxn.cursor()
        ...
        for root, dirs, files in os.walk(folderPath):
            for file in files:
                ...
                if Cloud =='AWS':                    
                    client.upload_file(local_file_path, ContainerName, ConnectionString)
                    # Generate a pre-signed URL        
                    client.put_object_acl(ACL='public-read',
                                    Bucket=ContainerName,
                                    Key=ConnectionString)
