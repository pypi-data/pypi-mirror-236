...
class BrowserView(tktextext.TextFrame):
    def __init__(self, master):
        ...
        self.pdf_name = None
        get_workbench().bind("PreBrowserView", self.show_browser)
        ...
        if self.pdf_name:
            ...
            if running_on_linux() or running_on_mac_os():
                ...
                subprocess.run(["del", self.pdf_name],
                               shell=True)  # Windows
        self.unbind("PreBrowserView")
        # try:
        #     self.unbind("PreBrowserView")
        # except Exception as e:
        #     pass
        self.unbind("HideView")
        get_workbench().get_view("BrowserView").destroy()
        get_workbench().add_view(BrowserView, self.title, "se", visible_by_default=False)
    def show_browser(self, event):
        main(self.text)
        # root = tk.Tk()
        # main(root)
def init():
    get_workbench().add_view(BrowserView, '我只是一个简简单单普普通通的标题', "se", visible_by_default=False)
...
class MainFrame(tk.Frame):
    ...
    def __init__(self, root):
        ...
        # self.root.master.bind("<Configure>", self.on_root_configure)
        self.setup_icon()
...
class BrowserFrame(tk.Frame):
    ...
    def embed_browser(self):
        ...
        print('当前线程', threading.current_thread().ident)
...
class NavigationBar(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.back_button = tk.Button(self, image=self.back_image, text="不知道啥功能",
        ...
        self.forward_button = tk.Button(self, image=self.forward_image,text="不知道啥功1",
        ...
        self.reload_button = tk.Button(self, image=self.reload_image, text="不知道啥功能2",
                                       command=self.reload)
