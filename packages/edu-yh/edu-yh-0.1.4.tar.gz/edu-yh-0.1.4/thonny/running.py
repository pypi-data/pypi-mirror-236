...
class Runner:
    ...
    def cmd_run_current_script_latex(self) -> None:
        ...
        for file in os.listdir(current_dir):
            ...
            if not file.endswith('.py') and not file.endswith('.db') and not file.endswith('.ini') and not file.endswith('VERSION') and os.path.isfile(
        ...
        self.compile_latex(name)

        # thread = MyThread(target=self.compile_latex, args=(name,))
        # thread.start()
