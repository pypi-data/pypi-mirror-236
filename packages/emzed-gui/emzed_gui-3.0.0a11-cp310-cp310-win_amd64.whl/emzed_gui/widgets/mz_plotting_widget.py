...
from emzed_gui.configs import config_for_spectrum
...
class MzPlottingWidget(CurveWidget):
    ...
    def plot_peaks_iter(self, peak_data, titles=(), background=True):
        ...
        for _ in self.plot.plot_peaks(peak_data, titles, background):
            yield

    def plot_spectra(self, collected_peaks, labels):
        assert len(collected_peaks) == len(labels)
...
class MzPlot(PositiveValuedCurvePlot, ExtendedCurvePlot):
    ...
    def plot_spectra(self, collected_peaks, labels):
        self.sticks = []
        for i, (peaks, label) in enumerate(zip(collected_peaks, labels)):
            curve = make_unselectable_curve(
                [], [], title=label, curvestyle="Sticks", **config_for_spectrum(i)
            )
    ...
    def plot_peaks(self, peak_data, titles, show_background_peaks):
        visible_peaks = []
        self.sticks = []
        ...
        if show_background_peaks:
            curve = make_unselectable_curve(
                [], [], curvestyle="Sticks", title="other", linewidth=1, color="#909090"
            )

            self.add_item(curve)
            self.background_peaks_curve = curve

        for i, (pm, rtmin, rtmax, mzmin, mzmax) in enumerate(peak_data):
            ms_level = min(pm.ms_levels())
            peaks = sample_peaks(pm, rtmin, rtmax, mzmin, mzmax, 1000, ms_level)

            visible_peaks.append(peaks)

            config = config_for_spectrum(i)
            if titles:
                title = titles[i]
            else:
                title = ""
            curve = make_unselectable_curve(
                [], [], title=title, curvestyle="Sticks", **config
            )
    ...
    def update_background_curves(self, mzmin, mzmax):
        ...
        peaks = np.vstack(
            [
                ...
                for (pm, rtmin, rtmax, _, _) in self.current_peak_data
            ]
        )
