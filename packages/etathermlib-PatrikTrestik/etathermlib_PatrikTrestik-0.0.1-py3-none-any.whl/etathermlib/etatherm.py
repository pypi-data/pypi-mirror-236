...
class Etatherm:
    ...
    async def __readParams(self) -> None:
        ...
        for pos in range(1,17): 
            ...
            if used:
                ...
                name=""
            res[pos]=(used, name, shift, step)
        self._params=res

    async def getParameters(self)->(dict[int,tuple[str]]|None):
        if self._params==None:
            await self.__readParams()
        if self._params==None:
            return None
        res={pos:(p[1]) for pos,p in self._params.items() if p[0]}
        self.__closeSocket()
        return res
    ...
    async def getCurrentTemperatures(self)-> (dict[int,int] | None):
        ...
        for pos in range(1,17):
            b=data[pos-1]
            used, _, shift, step=self._params[pos]
            if used:
                res[pos]= (b+shift)*step
        self.__closeSocket()
        return res
