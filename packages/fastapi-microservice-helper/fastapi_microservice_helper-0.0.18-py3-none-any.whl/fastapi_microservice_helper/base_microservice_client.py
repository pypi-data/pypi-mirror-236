...
def serialize_uuid_to_str(data: Optional[dict[str, Any]] = None) -> Optional[dict[str, Any]]:
    if not data:
        return None

    for key, value in data.items():
        if isinstance(value, dict):
            data[key] = serialize_uuid_to_str(value)
        elif isinstance(value, list):
            data[key] = [serialize_uuid_to_str(item) for item in value]
