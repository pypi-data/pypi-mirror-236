...
class DMRGConfig(BaseConfig):
    ...
    convergence_threshold: float = Field(
        default=1e-6, description="Threshold for convergence", ge=0.0, le=1.0
    )
    target_fidelity: float = Field(
        default=1.0, description="Target fidelity to reach", ge=0.0, le=1.0
    )
    max_sweeps: int = Field(
        default=int(1e5), description="Maximum number of sweeps", ge=1, le=MAX_SWEEPS
    )
    max_subcircuit_rows: int = Field(default=1, ge=1, le=100)
