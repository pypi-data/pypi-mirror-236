...
def get_qiskit_gate_dict(
    ...
    """
    gate_dict: dict[str, str | dict[str, float] | tuple] = {}
    ...
    if gate_dict["name"] in (
        ...
        param_dict["rads"] = float(gate.operation.params[0])

    elif gate_dict["name"] in ("U3", "CU"):
        param_dict["theta"] = float(gate.operation.params[0])
        param_dict["phi"] = float(gate.operation.params[1])
        param_dict["lam"] = float(gate.operation.params[2])
        if gate_dict["name"] == "CU":
            param_dict["gamma"] = float(gate.operation.params[3])
...
def qiskit_qubit_to_str(qubit: QiskitQubit) -> str:
    return f"{qubit.register.index(qubit)}"
