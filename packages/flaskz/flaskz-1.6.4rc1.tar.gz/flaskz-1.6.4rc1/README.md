...
- **1.6.4rc1** `2023/10/20`
    ...
    - [A] `model_to_dict`函数的`option`参数添加`filter`选项, 用于过滤instance列表中的对象(适用于`flaskz.rest.register_model_*`函数的`to_json_option`参数选项)
    - [A] `flaskz.rest.register_model_query_pss_route`函数添加`get_pss_config`参数, 用于自定义pss查询参数
