...
def fpk_install(directory_name: str, session: httpx.AsyncClient):
    ...
    toml_content = asyncio.run(fpk_fetch_flatpack_toml_from_dir(directory_name))
...
async def fpk_log_to_api(message: str, api_key: Optional[str] = None, model_name: str = "YOUR_MODEL_NAME"):
...
def fpk_train(directory_name: str = None, session: httpx.AsyncClient = None):
    ...
    Args:
        ...
        session (httpx.AsyncClient): HTTP client session for making requests. Currently unused.
    """
    cache_file_path = os.path.join(os.getcwd(), 'last_flatpack.cache')
...
async def main():
    ...
    if command == "callback":
        ...
        await fpk_install(directory_name, session)  # Include the 'await' here
    elif command == "list":
        print(await fpk_list_directories(session))  # Include the 'await' here
    elif command == "ps":
        ...
        await fpk_train(directory_name, session)  # Include the 'await' here
    ...
    await session.aclose()  # Await the session close


async def logging_task(session, api_key):
    """Continuously dequeue log messages, batch them, and send to the server.
    ...
    Behavior:
        The function runs in an infinite loop, dequeuing messages from log_queue.
        It batches these messages, and once the batch reaches a pre-defined size (LOGGING_BATCH_SIZE)
        or if the queue is empty, it sends the batch to the server for logging.
    """
    batch = []
    while True:
        ...
        if len(batch) >= LOGGING_BATCH_SIZE or log_queue.empty():
            ...
            for msg, model in batch:
                await fpk_log_to_api(msg, api_key, model)
            batch.clear()
...
if __name__ == "__main__":
    ...
    # Run the event loop until the main coroutine is completed
