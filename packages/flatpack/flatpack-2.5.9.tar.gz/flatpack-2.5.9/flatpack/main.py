...
async def send_log_batch(batch):
    if not batch:
        return
    messages, model_names = zip(*batch)
    ...
    try:
        async with session.post(url, params=params, json=data, headers=headers, timeout=10) as response:
            if response.status_code != 200:
                logger.error(f"Failed to send log batch: {response.status_code}")
...
async def main():
    ...
    if command == "callback":
        ...
        print(await fpk_list_directories(session))
    elif command == "ps":
        ...
        if not args.input:
            ...
            return
        await fpk_set_api_key(args.input)
        print("API key set successfully!")
    elif command == "train":
        directory_name = args.input
        await fpk_train(directory_name, session)
