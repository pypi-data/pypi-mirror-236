...
def fpk_process_line_buffer(line_buffer, session, last_installed_flatpack):
    ...
    # Use this variable to track the last line
    last_line_had_return = False

    while line_buffer:
        line = line_buffer[0].strip()

        # If the current line ends with '\r', and the previous line had a '\r',
        # we print a new line before printing the current line
        if last_line_had_return and line.endswith('\r'):
            print()
        ...
        # Check if the line ended with '\r', update the tracker variable
        last_line_had_return = line_buffer.pop(0).endswith('\r')
