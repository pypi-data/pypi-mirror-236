...
def fpk_process_line_buffer(line_buffer, session, last_installed_flatpack):
    ...
    while line_buffer:
        print(f"Buffer size before pop: {len(line_buffer)}")  # Diagnostic print
        if not line_buffer:
            print("Warning: Buffer is empty before pop!")  # Diagnostic print
            break
        line = line_buffer.pop(0).strip()
        print(f"Buffer size after pop: {len(line_buffer)}")  # Diagnostic print
        if line:
            print(f"(*) {line}")
