...
def fpk_process_batch(batch_buffer, session, last_installed_flatpack):
    """Process a batch of lines and log them."""
    api_key = fpk_get_api_key()
    for line in batch_buffer:
        line = line.strip()
        if line:  # Check that line is not empty or just whitespace
            print(f"(*) {line}")
            if api_key:
...
def fpk_train(directory_name: str = None, session: httpx.Client = None):
    ...
    master_fd, slave_fd = pty.openpty()  # open a new pseudo-terminal

    proc = subprocess.Popen(["bash", str(training_script_path)], stdin=slave_fd, stdout=slave_fd, stderr=slave_fd)

    os.close(slave_fd)  # close the slave descriptor from the parent process

    try:
        while True:
            rlist, _, _ = select.select([master_fd, sys.stdin], [], [], 1.0)  # 1 second timeout
            if master_fd in rlist:
                ...
                if not output:
                    break
                batch_buffer.extend(output.splitlines())
                if len(batch_buffer) >= BATCH_SIZE:
                    fpk_process_batch(batch_buffer, session, last_installed_flatpack)
                    batch_buffer.clear()
            elif not rlist:  # Timeout reached
                if batch_buffer:
                    fpk_process_batch(batch_buffer, session, last_installed_flatpack)
                    batch_buffer.clear()
