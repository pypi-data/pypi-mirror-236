...
from flowi.experiment_tracking.utils import log_model
from flowi.utilities.logger import Logger
from mlflow.tracking import MlflowClient


class MLflow(Base):
    def __init__(self, flow_name_fqdn: str, version: str):
        super(MLflow, self).__init__(flow_name_fqdn=flow_name_fqdn, version=version)
        self._logger = Logger(logger_name=__name__)
        self._client = MlflowClient()
        self._version = version
        self._experiment_id = None
        self._model_name = flow_name_fqdn

        self.set_project(flow_name=flow_name_fqdn)
    ...
    def _save_artifact(self, experiment_id: str, obj: Any, file_path: str, artifact_path: str) -> str:
    ...
    def _log_model(self, experiment_id: str, model: Any, artifact_path: str) -> str:
        with mlflow.start_run(run_id=experiment_id):
            version = log_model(model=model, model_name=self._model_name, artifact_path=artifact_path, register_model=False)
            return version
    ...
    def save_model(self, experiment_id: str, obj: Any, file_path: str) -> str:
        artifact_path = "model"
        return self._log_model(experiment_id=experiment_id, model=obj, artifact_path=artifact_path)
