...
class MqttClient:
    ...
    def publish(self, topic: str, data: Any, serialize: bool = True):
        ...
        if serialize:
            logging.info(f"[PUBLISHER] {topic} - {data}")
        else:
            logging.info(f"[PUBLISHER] {topic}")
...
class MqttConsumer:
    ...
    def on_message_callback(self, serialize: bool = True):
        def on_message(client, userdata, msg):
            ...
            if serialize:
                logging.info(f"[COMMAND] {self.topic} - {payload} - {url_params}")
            else:
                logging.info(f"[COMMAND] {self.topic}")
