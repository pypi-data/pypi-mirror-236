...
class ChangeLogEntry(BaseModel):
    ...
    other_parents: Optional[List[List[ChangeLogEntry]]] = Field(default_factory=list)
    ...
    body: Optional[str] = ""
    numstat: Optional[List[FileAdditionsDeletions]] = Field(default_factory=list)
    submodule_updates: Optional[Dict[str, SubmoduleUpdate]] = Field(default_factory=dict)
    ...
    def from_log_text(cls, log_text, url_provider=None, instance_class=None, issues_provider=None):
        ...
        return instance_class(
            ...
            parent_shas=split_and_strip(extract_line_content(parents_line, "P"), splitby=" "),
            tags=tags,
            refs=refs,
            subject=subject,
            commit_date=datetime.fromisoformat(extract_line_content(date_line, "D")).astimezone(pytz.utc),
            author_name=extract_line_content(author_name_line, "A"),
            author_mail=extract_line_content(author_mail_line, "M"),
            body=body,
            cherry_pick_sha=cherry_picked_commits[0] if len(cherry_picked_commits) == 1 else None,
