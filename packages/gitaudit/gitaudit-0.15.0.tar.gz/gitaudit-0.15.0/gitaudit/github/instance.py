...
from rich.progress import Progress, TimeElapsedColumn, TimeRemainingColumn, BarColumn, MofNCompleteColumn, TaskID
...
class FileAddition(BaseModel):
    ...
    def contents_must_be_base64(cls, value):
        ...
        try:
            ...
            raise ValueError(
                (f'"{value}" is not base64 encoded! ' "For plaintext use FileAddition.from_plain_text function.")
            ) from exc
...
class GithubCacheProvider:
    ...
    def get_cached_pull_requests(self, search_query: str, current_date: date) -> List[PullRequest]:
        """
...
class Github:
    ...
    def _call(self, data: str):
        ...
        while retry:
            ...
            if "errors" in res_json:
                ...
                message += "\n".join(map(lambda x: "- " + x["message"], res_json["errors"]))
    ...
    def pull_request(self, owner: str, repo: str, number: int, querydata: str) -> PullRequest:
        ...
        """
        res = self.query(
            (f'repository(owner:"{owner}", name:"{repo}")' f"{{ pullRequest(number:{number}) {{ {querydata} }} }}")
        )
    ...
    def search_pull_requests_merged_datetime_range(
        ...
        with live:
            task = progress_day.add_task("[cyan]Processing...", total=(end_date - start_date).days + 1)
            ...
            while current_date <= end_date:
                ...
                if not pull_requests:
                    pull_requests = self._search_pull_requests_progress(
                        ...
                        task=task_pr,
                        search_query=(f"{search_query}" " " f"merged:{current_date.isoformat()}"),
    ...
    def add_comment(self, subject_id: str, body: str):
        ...
        """
        self.mutation((f'addComment(input: {{body: "{body}", subjectId: "{subject_id}"}})' "{ clientMutationId }"))
    ...
    def get_repository(self, owner: str, repo: str, querydata: str):
        ...
        """
        res = self.query((f'repository(owner:"{owner}", name:"{repo}")' f"{{ {querydata} }}"))
    ...
    def get_first_sha_before_date(
        ...
        Returns:
            str: The commit SHA of the first commit before the specified date and time, or None if
                no commit is found.
        """
        commit = self.get_first_commit_before_date(owner, repo, ref, commit_date_time, "oid")
    ...
    def get_label_id(self, owner: str, repo: str, label_name: str) -> str:
        ...
        """
        query = f'repository(owner:"{owner}", name:"{repo}") {{' f'label(name: "{label_name}") {{' "id" "}" "}"
    ...
    def create_commit_on_branch(
        ...
        additions_input = [f'{{path: "{addition.path}", contents: "{addition.contents}"}}' for addition in additions]
