        ...
        if self.opt_method_ in ['grid', 'grid_search', 'grid search']:
            if one_parameter:
                ...
                self.optimization_outputs = {
                    'opt_method' : 'grid',
                    'opt_bandwidth' : None,
                    'opt_alpha' : self.alpha_list_[opt_id[0][len(opt_id[0]) // 2]],
                    'opt_beta' : self.beta_list_[opt_id[1][len(opt_id[1]) // 2]],
                    'opt_index': (opt_id[0][len(opt_id[0]) // 2], opt_id[1][len(opt_id[1]) // 2]),
                    'kappa_cv_errors': errors}
        ...
        if (y_test is not None) and (fig_type in ['qq', 'qq-plot', 'qqplot', 'QQ-plot', 'QQplot']):
            ...
            if self.optimization_outputs['opt_method'] in ['grid', 'grid_search', 'grid search']:
                if self.one_parameter:
                    if engine == "plotly":
                        df = pd.DataFrame({
                            'bandwidth' : self.bandwidth_list_, 
                            'error' : self.optimization_outputs['kappa_cv_errors']
                        })
                        df_opt = pd.DataFrame({
                            'bandwidth' : self.optimization_outputs['opt_bandwidth'],
                        ...
                        df_dash = pd.DataFrame({
                            'bandwidth' : [self.optimization_outputs['opt_bandwidth'], 
                                           ...
                                           0],
                            'error' : [self.optimization_outputs['kappa_cv_errors'][self.optimization_outputs['opt_index']]/3,
                                       self.optimization_outputs['kappa_cv_errors'][self.optimization_outputs['opt_index']],
                                       self.optimization_outputs['kappa_cv_errors'][self.optimization_outputs['opt_index']]]
                        })
                        l1 = px.line(df, x="bandwidth", y = "error").data
                        p1 = px.scatter(df_opt, x = "bandwidth", y = "error", color_discrete_sequence=["red"]).data
                        l2 = px.line(df_dash, x="bandwidth", y = "error", color_discrete_sequence=["red"], line_dash_sequence=['dash']).data
                        fig = go.Figure(
                            data =  l1 + p1 + l2
                        )
                        fig = fig.update_layout(width = 700, 
                        ...
                        plt.vlines(x=self.optimization_outputs['opt_bandwidth'], 
                                ymin=self.optimization_outputs['kappa_cv_errors'][self.optimization_outputs['opt_index']]/5, 
                                ymax=self.optimization_outputs['kappa_cv_errors'][self.optimization_outputs['opt_index']], 
                                colors='r', 
                                linestyles='--')
                        plt.hlines(y=self.optimization_outputs['kappa_cv_errors'][self.optimization_outputs['opt_index']], 
                                xmin=0, xmax=self.optimization_outputs['opt_bandwidth'], 
                                colors='r', linestyles='--')
                        if show_fig:
                            plt.show()
                else:
                    if engine == 'plotly':
                        ...
                        opt_error = error_[self.optimization_outputs['opt_index'][0], 
                                           self.optimization_outputs['opt_index'][1]]
                        df_opt = pd.DataFrame({
                            'alpha' : [opt_alpha],
                            'beta' : [opt_beta],
                            "error" : [opt_error]
                        ...
                        fig = go.Figure(data= [go.Surface(z = error_,
                                                         x = alpha_,
                                                         y = beta_), 
                                                         p1[0]])
                        fig.update_layout(scene = dict(
                                            ...
                                            yaxis_title='Beta',
                                            zaxis_title='Error'),
                                          title_text = "Errors vs parameters (alpha, beta) for ("+ str(self.kernel) + " kernel)", 
                                          title_x = .5, 
                                          title_y = 0.925,
                                          width = 700,
                                          height = 650)
                        ...
                        opt_alpha, opt_beta = self.optimization_outputs['opt_alpha'], self.optimization_outputs['opt_beta']
                        opt_error = self.optimization_outputs['kappa_cv_errors'][self.optimization_outputs['opt_index'][0], 
                                                                              self.optimization_outputs['opt_index'][1]]
                        fig = plt.figure(figsize=(7, 4))
                        axs = fig.add_subplot(projection='3d')
                        surf = axs.plot_surface(alpha_, beta_, err, cmap=cm.coolwarm, linewidth=0, antialiased=False)
                        axs.plot([opt_alpha, opt_alpha], [opt_beta, opt_beta], [opt_error, 0], '-o', c = 'r')
                        axs.plot([opt_alpha, opt_alpha, 0], [0, opt_beta, opt_beta], [0, 0, 0], '--', c = 'r')
                        axs.set_title("Errors vs parameters (alpha, beta) for ("+ str(self.kernel)+ " kernel)")
                ...
                if self.one_parameter:
                    if engine == 'plotly':
                        df1 = pd.DataFrame({
                            'iteration' : list(range(len(self.optimization_outputs['param_collection']))),
                            'parameter' : self.optimization_outputs['param_collection']
                        })
                        df0 = pd.DataFrame({'parameter' : [self.optimization_outputs['param_collection'][-1]]})
                        ...
                        f1 = go.Figure([go.Scatter(x = df1.iteration,
                                                ...
                                                mode = 'lines',
                                                showlegend=False)])
                        f1.add_shape(type = "line", 
                                     x0 = 0, 
                                     x1 = df1.iteration.values[-1], 
                                     y0 = df0.parameter.values[0],
                                     y1 = df0.parameter.values[0],
                        ...
                        f2 = go.Figure([go.Scatter(x = df2.bandwidth, 
                                        ...
                                        go.Scatter(x = df_dash.bandwidth, 
                                                   ...
                                                   line = dict(color = "red", dash = 'dash'),
                                                   showlegend=False)])
                        fig = make_subplots(rows=1, cols=2, print_grid=False, subplot_titles=("Bandwidth at each gradient descent step", "Error vs bandwidth"))
                    ...
                    if engine == 'plotly':
                        ...
                        opt_alpha, opt_beta = self.optimization_outputs['opt_alpha'], self.optimization_outputs['opt_beta']
                        opt_error = self.kappa_cross_validation_error2(opt_alpha, opt_beta)
                        ...
                        col_path = "#169CF8" # "#37BF30"
                        col_opt = "#22F17A"
                        p1 = px.scatter_3d(df_opt, 
                                           x = "alpha", 
                                           y = "beta", 
                                           z = "error",  
                                           color_discrete_sequence=[col_opt]).data
                        l1 = go.Scatter3d(x=self.optimization_outputs['param_collection'][:,0][range(0,n, 5)].reshape(-1), 
                                        y=self.optimization_outputs['param_collection'][:,1][range(0,n, 5)].reshape(-1), 
                                        z=err_path,
                                        showlegend=False,
                                        marker=dict(
                                        size=4),
                                        line=dict(
                                                color=col_path,
                                                width=4))
                        fig = go.Figure(data = [go.Surface(z = error,
                                                         x = alpha_,
                                                         y = beta_,
                                                         opacity=0.9,
                                                         showscale=True), 
                        ...
                        err =  np.array([[self.kappa_cross_validation_error2(alpha_[i,j],beta_[i,j]) for j in range(len(alpha_[0]))] for i in range(len(alpha_))])
                        opt_alpha, opt_beta = self.optimization_outputs['opt_alpha'], self.optimization_outputs['opt_beta']
                        opt_error = self.kappa_cross_validation_error2(opt_alpha, opt_beta)
                        fig = plt.figure(figsize=(13, 5))
                        axs = fig.add_subplot(projection='3d')
                        err_path = np.array([self.kappa_cross_validation_error2(a, b) for a,b in zip(self.optimization_outputs['param_collection'][:,0][range(0,n, 5)], 
                                                                                            self.optimization_outputs['param_collection'][:,1][range(0,n, 5)])])
                        axs.plot_surface(alpha_, beta_, err, cmap=cm.coolwarm, linewidth=0, antialiased=False)
                        axs.scatter(opt_alpha, opt_beta, opt_error, c = 'r')
                        axs.plot(self.optimization_outputs['param_collection'][:,0][range(0,n,5)].reshape(-1),
