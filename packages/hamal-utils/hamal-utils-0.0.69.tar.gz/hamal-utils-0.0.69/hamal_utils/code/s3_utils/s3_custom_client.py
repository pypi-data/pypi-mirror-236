...
from hamal_utils.code.common import MONITOR_TAG_TMPL, FROM_PERCENT, TO_PERCENT, BUCKET_FILES_COUNT, MAX_WORKERS, \
    DISABLE_LOG_TAG, IGNORE_LOG_TAG
...
class CustomS3Client:
    ...
    def _validate_extensions(self, key, extensions):
    ...
    def list_objects_v2_generator(
        ...
        for file in self._list_objects_v2_generator(
            ...
            if not self._validate_extensions(key, extensions):
                continue
    ...
    def list_objects_v2_in_parallel(
            self,
            bucket,
            delimiter=None,
            encoding_type=None,
            ...
            optional_object_attributes=None,
            extensions=None,
            process_file_callback=None
    ):

        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            generator = self.list_objects_v2_generator(
                bucket, delimiter, encoding_type, max_keys, prefix, fetch_owner,
                start_after, request_payer, expected_bucket_owner,
                optional_object_attributes, extensions)

            executor.map(process_file_callback, generator)
