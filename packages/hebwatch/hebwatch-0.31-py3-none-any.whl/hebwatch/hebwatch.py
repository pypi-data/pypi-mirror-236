...
class watch(jewishcal):
    ...
    def now(self):
        ...
        if (self.current_time > self.sunset):
            ...
            if self.current_time > (self.sunset + timedelta(minutes=24)):
                self.shkiya = True
            return temp_hour
    ...
    def to_standard(self, city, relative_time=None,day_night=None, date=None):
        '''
        ...
        elif day_night == "night" or day_night == "late_night":
    ...
    def hebrew_date(self,date_str = None):
        ...
        if self.shkiya:
            date = self.current_time.date() + timedelta(days=1)
            
        elif self.dayORnight == "day" or self.dayORnight == "late_night" or self.dayORnight == "night":
        ...
        date_str = date if date_str is None else date_str

        heb_date = georgian_to_hebrew(date_str)

        return heb_date

def georgian_to_hebrew(date):

    year_g, month_g, day_g = date.split('-')

    today_abs = jewishcal().gregorian_to_absdate(int(year_g), int(month_g), int(day_g))
    year_h, month_h, day_h = jewishcal().absdate_to_hebrew(today_abs)

    heb_date = jewishcal().hebrew_date_from_num(year_h, month_h, day_h)
