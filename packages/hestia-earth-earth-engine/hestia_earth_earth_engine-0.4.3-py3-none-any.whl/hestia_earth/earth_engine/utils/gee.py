...
def _filter_image_by_years(image: ee.Image, start_date: str, end_date: str, reducer_annual: str, reducer_period: str):
    start_year = _date_year(start_date)
    end_year = _date_year(end_date)
    reducer_annual_func = getattr(ee.Reducer, reducer_annual)
    collection = [
        image.filterDate(f"{year}-01-01", f"{year}-12-31").reduce(reducer_annual_func())
...
def _reduce_features_order(features: list, orders: list):
    def process(values: list, index: int):
        ...
        order = orders[index]
        return values + [properties.get(order)]
    return process
...
def _process_batch(batch_func, collections: list, geometries: list):
    ...
    return _handle_none_values(
        [features[0].get('properties', {}).get(key) for key in order]
