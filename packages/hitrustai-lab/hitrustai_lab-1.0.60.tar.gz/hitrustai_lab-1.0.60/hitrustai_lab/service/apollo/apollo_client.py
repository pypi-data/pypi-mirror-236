...
from .util import *
from ...utils.log_handler import init_logger
...
class ApolloClient(object):
    ...
    def get_value(self, key, default_val=None, namespace='application'):
        try:
            ...
            init_logger.error("get_value has error, [key is %s], [namespace is %s], [error is %s], ",
                              key, namespace, e)
            return default_val
    ...
    def stop(self):
        self._stopping = True
        init_logger.info("Stopping listener...")
    ...
    def _long_poll(self):
        ...
        try:
            ...
            http_code = code
            if http_code == 304:
                init_logger.debug('No change, loop...')
                return
            if http_code == 200:
                data = json.loads(body)
                for entry in data:
                    ...
                    init_logger.info("%s has changes: notificationId=%d", namespace, n_id)
                    self._get_net_and_set_local(namespace, n_id, call_change=True)
                    return
            else:
                init_logger.warning('Sleep...')
        except Exception as e:
            init_logger.warning(str(e))
    ...
    def _listener(self):
        init_logger.info('start long_poll')
        while not self._stopping:
            self._long_poll()
            time.sleep(self._cycle_time)
        init_logger.info("stopped, long_poll")
