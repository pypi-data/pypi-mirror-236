...
def logtime(logger:Logger)->Callable[[Callable[P,T]],Callable[P,T]]:
    """
    Decorator function to report execution time
    """
    def decorator(func:Callable[P,T])->Callable[P,T]:
        """ decorator """
        @wraps(func)
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(
                    'Execution time(%s): [start=%s, final=%s, total=%s]',
                    func.__name__, start, end, end-start)
        return wrapper
    return decorator
