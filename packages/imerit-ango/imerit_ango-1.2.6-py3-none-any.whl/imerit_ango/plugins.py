...
class ExportPlugin(Plugin):
    ...
    def on_plugin(self, data):
        ...
        project = sdk.get_project(project_id).get('data').get('project')

        path = project.get('exportStoragePath', '%s/export'%project_id)
        file_name = path + '/' + file_name

        if project.get('exportStorageEnabled', False):
            storage_id = project.get('exportStorageId')
            bucket = project.get('exportStorageBucket')
            storage = sdk.get_storages(storage_id)
