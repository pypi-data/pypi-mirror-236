...
def generate(invoices_path, pdfs_path, image_path, product_id, product_name,
    ...
    for filepath in filepaths:
        ...
        # Add total row
        total_amount = str(df[total_price].sum())
        pdf.set_text_color(80, 80, 80)
        pdf.cell(w=30, h=8, txt='', border=1)
        pdf.cell(w=70, h=8, txt='', border=1)
        pdf.cell(w=30, h=8, txt='', border=1)
        pdf.cell(w=30, h=8, txt='', border=1)
        pdf.cell(w=30, h=8, txt=total_amount, border=1, align="R", ln=1)

        # Add total sum sentence
        pdf.set_font(family="Times", size=10, style="B")
        pdf.cell(w=30, h=8, align="L", txt=f"The total amount due is ${total_amount}. ", ln=1)
