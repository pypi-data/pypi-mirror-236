...
class DGSwap(DG):
    ...
    @property
    def depth_undo_qct(self):
        '''
        Recover the original logical circuit depth through removing all SWAPs
        We ignore the initial mapping
        '''
        ...
        mapping = list(range(len(self.ag)))
        for node in nx.topological_sort(self):
            gates = self.get_node_gates(node)
            for gate in gates:
                qubits = gate[1]
                if gate[0] in ('swap', 'SWAP'):
                    t = mapping[qubits[0]]
                    mapping[qubits[0]] = mapping[qubits[1]]
                    mapping[qubits[1]] = t
                else:
                    qubits_new = [mapping[q] for q in qubits]
                    gate_new = (gate[0], tuple(qubits_new), gate[2])
                    add_gate_to_qiskit_cir(cir_qiskit, gate_new)
        return cir_qiskit.depth()
