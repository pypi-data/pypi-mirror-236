...
class DBsDetails(BaseModel):
    """
    DBsDetails
    """
    databases: Optional[List[DBsDetailsDatabasesInner]] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DBsDetails:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DBsDetails:
        """Create an instance of DBsDetails from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DBsDetails.model_validate(obj)

        _obj = DBsDetails.model_validate({
            "databases": [DBsDetailsDatabasesInner.from_dict(_item) for _item in obj.get("databases")] if obj.get("databases") is not None else None
        })
        return _obj
