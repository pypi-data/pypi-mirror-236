...
def show(map_222):
    for i in range(SIZE):
        for j in range(SIZE):
            # 背景颜色块
            screen.blit(map_222.map[i][j] == 0 and block[(i + j) % 2]
                        or block[2 + (i + j) % 2], (PIXEL * j, PIXEL * i))
            # 数值显示
            if map_222.map[i][j] != 0:
                map_text = map_font.render(
                    str(map_2048.map[i][j]), True, (106, 90, 205))
    ...
    score_text = score_font.render((map_222.over(
    ) and "Game over with score " or "Score: ") + str(map_222.score), True, (106, 90, 205))
