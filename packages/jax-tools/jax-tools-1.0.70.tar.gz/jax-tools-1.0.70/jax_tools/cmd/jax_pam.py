...
def main() -> None:
    ...
    try:
        ...
        if arg_1 in list_sign_list:
            ...
            pm.print_account_list()
            num = int(input(u'请输入您想要更新的账号NUM(从上方获取)：'))
            account_info = pm.get_user_info(num)
            name = account_info.get(pm.NAME)
            username = account_info.get(pm.USERNAME)
            name = input(u'请输入您的要新建的账号名称(旧名称:{})：'.format(name))
            username = input(u'请输入你的登录用户名(旧用户名{}): '.format(username))
            password = getpass.getpass(u'请输入你的登录密码或验证token: ')
            if pm.delete_account(name):
                pm.add_account(name, username, password, num)
        else:
