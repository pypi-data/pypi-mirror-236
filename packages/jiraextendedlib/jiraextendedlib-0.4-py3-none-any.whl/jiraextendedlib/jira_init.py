...
class JiraInit():
    ...
    def _get_ims_token(self, new_token = False):
        ...
        if not self.auth_token:
            # self.auth_token = self.action_service.get_value(name='ipass_auth_token', decrypt=True)
            try:
        ...
        if new_token or not self.auth_token:
            config = self.config
            url = "{url}/v1?grant_type=authorization_code&client_id={client_id}".format(url=config['ims_url'],client_id=config['ims_client_id'])
            headers = {
                'content-type': 'application/json'
            }
            response = requests.post(url=url, headers=headers, data={'secret': config['ims_secret'], 'code': config['ims_code']})
            ims_reponse =  response.json()
            if "access_token" in ims_reponse:
                self.auth_token = ims_reponse['access_token']
