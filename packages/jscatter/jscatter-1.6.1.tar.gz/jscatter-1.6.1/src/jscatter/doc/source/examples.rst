...
Often for incohrent measurements only incoherent is taken into acount.
...
Normal mode relaxation as seen by NSE measuring the intemediate scattering function
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
and the Rayleigh expansion for diffusing rigid particles (:py:func:`~jscatter.bio.scatter.intScatFuncYlm`) :

Translational and rotational diffusion are corrected as :math:`D_t(Q) = D_{t,0} H(Q)/S(Q)`
and :math:`D_r = D_{r.0}H_r`.
