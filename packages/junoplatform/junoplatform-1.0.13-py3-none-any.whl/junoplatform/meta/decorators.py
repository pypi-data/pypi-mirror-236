...
class EntryPoint:
    ...
    def update_meta(self, meta:dict):
        try:
            ...
            if vdiff:
                ...
                self.storage.local.io.hset('system.ai.modules', f'{junoconfig["plant"]}.{junoconfig["module"]}', json.dumps(meta, ensure_ascii=False).encode())
    ...
    def _thread(self, func):
        ...
        while True:
            ...
            self.cfg_in = junoconfig['input_cfg']
            logging.info(f'cfg_in: {self.cfg_in}')
            self.config_lock.release()
            algo_cfg = junoconfig["algo_cfg"]
            ...
            if once:
                ...
                meta = {
                    ...
                    "username": junoconfig["author"],
                    "plant": junoconfig["plant"],
                    "module": junoconfig["module"],
                    "config": junoconfig["algo_cfg"],
                    "status": 1,
                    "enable": enable,
                }
                
                self.update_meta(meta)
                r = self.storage.local.io.set(f"system.{junoconfig['plant']}.{junoconfig['module']}.reconfig", json.dumps(junoconfig["algo_cfg"], ensure_ascii=False))
                once = False
            ...
            if not self.stop_flag:
                ...
                self.tick_key = f"system.ai.tick.{junoconfig['plant']}.{junoconfig['module']}"
    ...
    def sleep(self, secs:float):
      while secs >0:
        ...
        secs -= 1
        try:
            r = self.storage.local.io.get(f"system.{junoconfig['plant']}.{junoconfig['module']}.enable")
            if r:
                r = bool(int(r.decode()))
                r = not r
                if r != self.stop_flag:
                    ...
                    self.stop_flag = r
                    self.reconfig = True
            r = self.storage.local.io.get(f"system.{junoconfig['plant']}.{junoconfig['module']}.reconfig")
            if r:
                r = json.loads(r.decode())
                diff = dict_value_diff(junoconfig['algo_cfg'], r)
                if diff:
                ...
                if last_tick == self.tick:
                    try:
                        if not self.storage.local.io.get(self.tick_key):
                            message = {
                            "name": "junoplatform",
                            "message": f"算法模块可能挂死: ({junoconfig['plant']}, {junoconfig['module']}, {junoconfig['package_id']})" + ":" + self.tick
                            }
            ...
            if last_tick != self.tick:
                if self.storage.local.io.get(self.tick_key):
                    ...
                    try:
                        message = {
                          "name": "junoplatform",
                          "message": f"算法模块挂死恢复: ({junoconfig['plant']}, {junoconfig['module']}, {junoconfig['package_id']})" + ":" + self.tick
                        }
                        self.storage.cloud.write('juno-svc-notification', message)
                    except Exception as e:
