...
class Kazure(object):
    ...
    def create_network(self, name, cidr=None, dhcp=True, nat=True, domain=None, plan='kvirt', overrides={}):
        ...
        self.network_client.nat_gateways.begin_create_or_update(self.resource_group, f'nat-{name}',
                                                                nat_gateway_data)
        route_table_name = f'route-table-{name}'
        route_table_data = {'location': self.location}
        route_table = self.network_client.route_tables.begin_create_or_update(self.resource_group, route_table_name,
                                                                              route_table_data)
        route_table = route_table.result()
        route_data = {'address_prefix': '0.0.0.0/0', 'next_hop_type': 'VirtualAppliance',
                      'next_hop_ip_address': public_ip}
        self.network_client.routes.begin_create_or_update(self.resource_group, route_table_name, 'default',
                                                          route_data)
        if overrides.get('create_subnet', True):
            ...
            if not nat:
                subnet_data['route_table'] = {'id': route_table.id}
    ...
    def create_subnet(self, name, cidr, dhcp=True, nat=True, domain=None, plan='kvirt', overrides={}):
        ...
        if not nat:
            ...
            nat_gateway = self.network_client.route_tables.get(self.resource_group, nat_gateway)
            data['nat_gateway'] = {'id': nat_gateway.id}
            route_table_name = f'route-table-{network}'
            route_table = self.network_client.route_tables.get(self.resource_group, route_table_name)
            data['route_table'] = {'id': route_table.id}
