...
from kfsd.apps.endpoints.serializers.utils.tbl import ClearTblInputReqSerializer
...
class UtilsViewSet(viewsets.ViewSet):
    ...
    def getClearTblInputData(self, request):
        inputSerializer = ClearTblInputReqSerializer(data=request.data)
    ...
    @extend_schema(**UtilsDoc.clear_tbl_view())
    @decorators.action(
        detail=False,
        methods=["post"],
        renderer_classes=[KubefacetsJSONRenderer],
    )
    def clear_tbl(self, request):
        serializer = self.getClearTblInputData(request)
        return response.Response(serializer.exec(serializer.data), status.HTTP_200_OK)
