...
class AWSRequestsSigner(object):
    """
    ...
    def __init__(self,
        ...
        self.aws_token = secret_token
        self.r = r
        self.aws_host = str(urlparse(r.url).hostname)
    ...
    def __call__(self):
        ...
        Adapted from https://docs.aws.amazon.com/general/latest/gr/sigv4-signed-request-examples.html
        """
        aws_headers = self.get_aws_request_headers_handler()
        self.r.headers.update(aws_headers)
        return self.r

    def get_aws_request_headers_handler(self):
        """
    ...
    def get_aws_request_headers(self):
        """
        ...
        canonical_uri = self.get_canonical_path()

        canonical_querystring = self.get_canonical_querystring()
        ...
        if self.secret_token:
            canonical_headers += 'x-amz-security-token:' + self.secret_token + '\n'
        ...
        body = self.r.data if self.r.data else bytes()
        try:
        ...
        if self.r.headers.get('content-md5'):
            payload_hash = hashlib.sha256(body).hexdigest()
        else:
            payload_hash = EMPTY_PAYLOAD_HASH

        # Combine elements to create create canonical request
        canonical_request = (self.r.method + '\n' + canonical_uri + '\n' +
        ...
        signing_key = getSignatureKey(self.secret_key,
                                      datestamp,
                                      self.aws_region,
                                      self.service)
        ...
        authorization_header = (algorithm + ' ' + 'Credential=' + self.secret_key_id +
        ...
        if self.secret_token:
            headers['X-Amz-Security-Token'] = self.secret_token
        return headers
    ...
    def get_canonical_querystring(self):
        ...
        q = str(urlparse(self.r.url).query)
        querystring_sorted = '&'.join(sorted(q.split('&')))
