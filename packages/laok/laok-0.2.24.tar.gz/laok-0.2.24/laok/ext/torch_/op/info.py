...
'''
import torch
from laok.base.str import s_fmt_size
# ===============================================================================
r'''
'''
# ===============================================================================
...
def _print_tensor(data, title=None, show_data=True):
    ...
    msg += f'type[{_type}]'
    if isinstance(data, torch.Tensor):
        msg += f' shape{s_fmt_size(data.shape)} dim[{data.ndim}] dtype[{_fmt_dtype(data.dtype)}] device[{data.device}]'
...
def _print_model(model):
    ...
    for i, (name, param) in enumerate(model.named_parameters()):
        print( f"<{i}> {name}")
        print( f"    requires_grad:{param.requires_grad}  size={s_fmt_size(param.size())}")
    return model
