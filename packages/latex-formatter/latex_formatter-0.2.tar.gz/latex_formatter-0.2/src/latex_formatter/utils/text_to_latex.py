...
def to_root(data):
    ...
    if data["type"] == "root":
        ...
        if index:
            ...
            numerator  = f"[{index}]"
            denominator = converter(radicand)
        else:

            print(radicand[0], type(radicand[0]))
            numerator = converter(radicand[0])

            denominator = converter(radicand[1])
