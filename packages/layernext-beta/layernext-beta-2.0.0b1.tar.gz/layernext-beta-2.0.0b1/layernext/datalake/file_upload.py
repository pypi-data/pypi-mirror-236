...
from layernext.datalake.constants import MAX_UPLOAD_RETRY_COUNT, MediaType, FILE_UPLOAD_THREADS, SUB_FILE_LENGTH, MULTI_PART_UPLOAD_CHUNK_SIZE
...
class FileUpload:
    ...
    def parallel_upload(self, sublist):
        _upload = StorageUpload(self._client, self.storage_type, self.collectionName, self.uploadId, self.upload_type, self.write_progress, self.add_fail_files, self.application_code)
    ...
    def file_upload_initiate(self, path, collection_type, collection_name, meta_data_object, meta_data_override, storage_prefix_path):
        ...
        #if self.progress != 0:
        complete_res = self._client.datalake_interface.complete_collection_upload(self.uploadId, is_single_file_upload)
        ...
        if is_single_file_upload:
            ...
            return_obj["unique_name"] = uploaded_file_list[0] if len(uploaded_file_list) > 0 else ""
