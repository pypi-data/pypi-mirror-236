...
LeanServerResult = namedtuple('LeanServerResult', ['error', 'warning', 'info', 'open_states', 'context', 'core_context', 'tactic_state_id'])
...
class ProvingSearchAgent:
    ...
    def init_search_raw(self, context: Optional[str] = None, filename: Optional[str] = None) -> Dict[str, Any]:
        ...
        results['core_context'] = results['context'][len(self.pre_lines) : -len(self.post_lines) if len(self.post_lines) > 0 else len(results['context'])]
        results['tactic_state_id'] = len(self.ordered_context) - 1
        self.search_history.add_node(results['context'], **{k : v for k, v in results.items() if k != 'context'})
        return LeanServerResult(**results)
    ...
    def run_tac_plain(self, context: str, tactic_state_id: Optional[int] = None, **kwargs: Any) -> Dict[str, Any]:
        ...
        **kwargs: Additional parameters.
        ...
        dict: The verification result of the modified context.
        """
        self.pre_lines = '\n'.join([i for i in context.split('\n') if i.startswith('import')] + [self.pre_lines])
        context = '\n'.join([self.pre_lines] + [i for i in context.split('\n') if not i.startswith('import')] + [self.post_lines])
...
def add_mathlib_data():
    from leaven.src.proof_search_agent import ProvingSearchAgent
