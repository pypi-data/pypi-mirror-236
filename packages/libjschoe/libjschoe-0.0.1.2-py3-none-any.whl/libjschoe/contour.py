...
def contour_from_csv(
    ...
    color_style='seaborn-v0_8-white',
):
    """
    ...
    with plt.style.context(color_style):
...
def contour_slice(
    ...
    if file is None:
        warnings.warn("File is not assigned. Please pass a file as an argument.")
        return
    try:
        df = pd.read_csv(file)
        ...
        warnings.warn("File not found. Please make sure the file exists.")
