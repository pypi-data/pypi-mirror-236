...
from .mixins import ProjectRelated, VoucherNumber, PeriodRange, PeriodRangeObservable, PaymentRelated, Payable, MovementBase
...
class RegistrableVoucher(Registrable, Voucher):
    ...
    def register_voucher(self, ar=None, do_clear=True):
        ...
        def doit(partners, products):
            ...
            if True: # settings.SITE._history_aware_logging:
        ...
        if dd.is_installed('storage'):
            for em in obj.storage_movement_set_by_voucher.order_by('seqno'):
                ...
                for k in ('partner_id', 'product_id', 'qty', 'value_date'):
                    ...
                    wmv = getattr(wm, k)
                    if emv != wmv:
                        diffs.append("{} (found {!r} != expected {!r})".format(k, emv, wmv))
                if len(diffs) > 0:
