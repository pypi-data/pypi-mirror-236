...
def create_log_group_sample():
    ...
    target_aws_secret = ""
    boto3Client = Boto3Client(target_region_name, target_aws_key, target_aws_secret)
    response = boto3Client.create_log_group_stream(target_log_group, 
                            target_log_stream)
...
def set_log_message_sample():
    ...
    if response is None:
        ...
        if debug_log_response is not None:
            print("log inserted successfully")
            
        if fatal_log_response is not None:
            print("log inserted successfully")
            
        if info_log_response is not None:
            print("log inserted succssfully")
