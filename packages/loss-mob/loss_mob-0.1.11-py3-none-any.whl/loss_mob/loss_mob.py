...
def rank_gini(rank, freq, loss):
  """
  It is an utility function to calculate gini based on the defined rank order
  and not callable by end users.
  Parameters:
    rank : A numeric vector for the rank in gini calculation. 
    freq : A numeric vector for the frequency.
    loss : A numeric vector for the loss.
  Returns:
    A scalar of gini-coefficient value.
  """

  fsum = sum([_ for _ in freq])
  lsum = sum([_ for _ in loss])

  _l1 = sorted([[_[0], _[1] / fsum, _[2] / lsum] for _ in zip(rank, freq, loss)],
               key = lambda x: x[0])

  lcum = [_ for _ in numpy.cumsum([_[2] for _ in _l1])]

  _l2 = [[*_[0], _[1], _[2], (_[1] + _[2]) * _[0][1] / 2]
         for _ in zip(_l1, [0] + lcum[:-1], lcum)]
...
def screen(x, y):
  ...
  return({"total records"           : len(_data),
          ...
          "gini coefficient"        : round(num_gini(_x, _y), 8)})
...
def num_gini(x, y):
  """
  It calculates the gini-coefficient between raw values of x and y.
  Parameters:
    x : A numeric vector for the model predictor;
    y : A numeric vector for the model response.
  Returns:
    The gini-coefficient value.
  """
  ...
  _d1 = sorted([_ for _ in zip(x, y) if ~numpy.isnan(_[0]) and ~numpy.isnan(_[1])],
               key = lambda x: x[0])

  _d2 = [{"x": i[0], "freq": len(i[1]), "ysum": sum([_[1] for _ in i[1]])}
         for i in list(cytoolz.groupby(0, _d1).items())]

  _r = False if spearmanr([_[0] for _ in _d1], [_[1] for _ in _d1])[0] > 0 else True

  _d3 = [{"rank": 1 + _[0], **_[1]} for _ in enumerate(sorted(_d2, key = lambda x: x["x"], reverse = _r))]

  if (len(x) - len(_d1)) > 0:
    _m1 = {"rank": 0, "x": numpy.nan, "freq": len(x) - len(_d1),
           "ysum": sum([_[1] for _ in zip(x, y) if numpy.isnan(_[0]) and ~numpy.isnan(_[1])])}
    _d4 = sorted([_m1] + _d3, key = lambda x: x["rank"])
  else:
    _d4 = sorted(_d3.copy(), key = lambda x: x["rank"])

  _d5 = [[_[v] for _ in _d4] for v in ["rank", "freq", "ysum"]]

  return(rank_gini(_d5[0], _d5[1], _d5[2]))
...
def kmn_bin(x, y):
  ...
  _n = numpy.arange(2, max(3, min(50, round(len(set(_x)) / 2, 0))))
