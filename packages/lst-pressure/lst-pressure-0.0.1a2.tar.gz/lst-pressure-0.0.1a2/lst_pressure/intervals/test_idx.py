...
def create_idx(calendar):
    ...
    for i, entry in enumerate(calendar[:-1]):
        ...
        idx.insert(
            Interval(
                today_sunrise_lst,
                today_sunset_lst + LST_DAY_DEC if today_sunset_lst < today_sunrise_lst else today_sunset_lst,
                {
                    "sun": sun,
                    "dt": today_start,
...
def test_observations(test):
    ...
    for observation in observations:
        ...
        filtered_intervals = [i for i in candidate_intervals if interval_type is None or i[2]["type"] == interval_type]
