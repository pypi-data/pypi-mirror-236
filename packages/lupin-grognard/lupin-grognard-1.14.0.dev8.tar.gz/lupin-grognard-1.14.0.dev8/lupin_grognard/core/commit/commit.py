...
class Commit:
    def __init__(self, commit: str):
        ...
        self.parents = None
        self.mr_approvers_same_author_child_commit = False
        self.author_child_commit = ""

    @classmethod
    def get_mr_approvers_same_author_child_commit(
        cls, commits: List["Commit"]
    ) -> List["Commit"]:
        commits = cls._get_parents_for_commits(commits=commits)
        for i, commit in enumerate(commits):
            if len(commit.parents) == 2:  # It's a merge commit
                mr_approvers_mail = [
                    approver.split(" ")[-1] for approver in commit.approvers
                ]

                for j in range(i + 1, len(commits)):
                    if (
                        commits[j].hash == commit.parents[1]
                        and commits[j].author_mail in mr_approvers_mail
                    ):
                        commit.mr_approvers_same_author_child_commit = True
                        commit.author_child_commit = commits[j].author_mail
                    break
        return commits
