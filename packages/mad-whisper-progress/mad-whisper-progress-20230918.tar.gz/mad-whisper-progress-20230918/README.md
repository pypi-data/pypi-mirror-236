...
**this project support progress callback programmatically in transcribe module, this module version just follow original whisper version**
...
Progress Callback Transcription can also be performed within Python:

```python
import whisper

def progress_transcribe(segments, segment_size, left_segment, rate):
    print(segments, segment_size, left_segment, rate)

model = whisper.load_model("base")
result = model.transcribe("audio.mp3", progress=progress_transcribe)
print(result["text"])
```

- progress param is optional
- segments: list about current segment information, start, end, text info dict
- segment_size: audio current transcribed segment size
- left_segment: remained segment size
- rate: current rate 0~1, 1 is 100%
...
jseo@madowl.kr, http://www.madowl.kr
