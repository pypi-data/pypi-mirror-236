...
class RestaurantMixin:
    def search_restaurants(self, keyword, start_index, page_size, proxy=None):
        url = f"https://stage.mangoplate.com/api/v5/search/by_keyword.json"
        data = {
            "language": "kor",
    ...
    def search_restaurants_count(self, keyword, proxy=None):
        url = f"https://stage.mangoplate.com/api/v5/search/by_keyword/count.json"
        data = {
            "language": "kor",
            "keyword": keyword,
        }
    ...
    def get_restaurant(self, code, proxy=None):
        url = f"https://stage.mangoplate.com/api/v5/restaurants/{code}.json"
        data = {
            "language": "kor",
        }
        response = requests.get(url, data=data, proxy={"http":proxy})
    ...
    def _parse_related_tags(self, code, proxy=None):
        ...
        """
        url = f"https://www.mangoplate.com/restaurants/{code}"
        response = requests.get(url, headers=HEADERS, proxy={"http":proxy})
