...
for log_file in log_files:
    ...
    result = df.groupby(["active_students", "success"]).agg(
        {
    ...
    for student_number in student_numbers:
        ...
        print(
            f"Percentage of successful API calls for {student_number} students: {percentage.__round__(2)}"
        )
    ...
    for index, student_number in enumerate(student_numbers):
        ...
        axs[index].hist(
            x=data["time_line"], bins=bins_dict[student_number]
        )  # axs[row][column]
        # axs[index][1].set_title(f'Histogram for {student_number} students')
