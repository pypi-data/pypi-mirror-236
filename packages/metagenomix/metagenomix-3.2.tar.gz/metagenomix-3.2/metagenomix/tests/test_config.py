...
class TestIO(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.config_input = {
            'meta_fp': '',
            'fastq_dirs': '%s/fastqs_illumina' % FOLDER,
        ...
        for fastq in ['A_1.fastq', 'A_2.fastq', 'B_1.fastq', 'B_2.fastq']:
            fastq_path = '%s/fastqs_illumina/%s' % (FOLDER, fastq)
    ...
    def test_get_fastq_paths(self):
        ...
        conf.fastq_dirs = ('%s/fastqs_illumina' % FOLDER,)
        fastq_fps = conf.get_fastq_paths()
        self.assertEqual(sorted(fastq_fps), sorted(self.fastqs1))

        conf.fastq_dirs = ('%s/fastqs_illumina' % FOLDER, '%s/output' % FOLDER)
    ...
    def test_get_fastq_samples(self):
        ...
        conf.set_metadata()
        conf.fastq_dirs = (FOLDER, '%s/fastqs_illumina' % FOLDER,)
        conf.get_fastq_samples()
        self.assertEqual(conf.fastq, self.fastqs)

        conf.set_metadata()
        conf.fastq_dirs = (FOLDER, '%s/fastqs_illumina' % FOLDER, '%s/output' % FOLDER,)
        conf.get_fastq_samples()
        self.assertEqual(conf.fastq, self.fastqs)
