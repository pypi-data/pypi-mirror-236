...
def main():
    ...
    if len(kraken_reports_all['sample'].unique()) > 5:
        ...
        # Group
        grouped = kraken_reports_all.groupby('ncbi_taxa') 
        ...
        for name, group in grouped:
            ...
            if len(group) > 1:
                ...
                # 直接赋值
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'corr_reads_min'] = corr_reads_min
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'pval_reads_min'] = pval_reads_min
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'corr_reads_uniq'] = corr_reads_uniq 
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'pval_reads_uniq'] = pval_reads_uniq
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'corr_min_uniq'] = corr_min_uniq
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'pval_min_uniq'] = pval_min_uniq

            else:
                # 处理只有一行的情况
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'corr_reads_min'] = np.nan
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'pval_reads_min'] = np.nan
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'corr_reads_uniq'] = np.nan
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'pval_reads_uniq'] = np.nan
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'corr_min_uniq'] = np.nan
                kraken_reports_all.loc[kraken_reports_all['ncbi_taxa'] == name, 'pval_min_uniq'] = np.nan
        ...
        for col in pval_columns:
            ...
            if kraken_reports_all[col].max() < 1:
                _, pvals_corrected, _, _ = multipletests(kraken_reports_all[col], alpha=0.05, method='fdr_bh')
