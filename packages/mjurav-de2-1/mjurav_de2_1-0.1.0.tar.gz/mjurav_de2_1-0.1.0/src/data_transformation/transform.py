...
from pydantic import BaseModel, PositiveInt, conlist, ValidationError


class Transpose2DInput(BaseModel):
    input_matrix: conlist(conlist(float, min_length=1), min_length=1)
...
class Convolution2DInput(BaseModel):
    input_matrix: conlist(conlist(float, min_length=1), min_length=1)
    kernel: conlist(conlist(float, min_length=1), min_length=1)
    stride: PositiveInt = 1
