...
state_dc = {19: 'Deactivating', 130: 'Slew', 120: 'Stop', 0: 'Undefined', 110: 'SIP', 220: 'Jog', 1: 'Standby', 2: 'Parked', 3: 'Locked', 300: 'Track', 4: 'E-Stop', 6: 'Stowed', 9: 'Locked & Stowed', 10: 'Activating', -1: "UNKNOWN"}
band_dc = {7: 'Band 5c', 8: 'Maintenance Position 1', 9: 'Maintenance Position 2', 0: 'Not in Position', 1: 'Band 1', 2: 'Band 2', 3: 'Band 3', 4: 'Band 4', 5: 'Band 5a', 6: 'Band 5b'}
stow_pin_controller_dc = {0: 'Unknown', 1: 'Retracted', 2: 'Retracting', 3: 'Deployed', 4: 'Deploying'}
authority_dc = {0: 'Authority free to take over', 1: 'Authority actual provided to LMC', 2: 'Authority actual provided to "Engineering Interface" (MTM\'s local M&C)', 3: 'Authority actual provided to MTMSCU', 4: 'Authority actual provided to HHD (Hand-held device)'}

mappers = {
    'acu.azimuth.state': state_dc,
    'acu.general_management_and_controller.feed_indexer_pos': state_dc,
    'acu.elevation.state': state_dc,
    'acu.feed_indexer.state': state_dc,
    'acu.command_arbiter.act_authority': authority_dc,
    'acu.stow_pin_controller.azimuth_status': stow_pin_controller_dc,
    'acu.stow_pin_controller.elevation_status': stow_pin_controller_dc,
}
    ...
    def get_log(self, id:int, interval_ms:float=None) -> pd.DataFrame:
        with self.dataLock:
            if id == 'history':
                ...
                if len(self.datalog) == 1 and len(self.datalog[self.data["datalogging.lastSession"]]) == 0:
                    # first call no data available -> edge case... fall back to history instead
                    _log.info("call to get_log with no logs available: -> edge case... fall back to history instead")
                    data = self.history
                assert data is not None and len(data) > 0, 'datalog requested is empty, or this telescope has no datalog yet!'
        ...
        for c in df2.columns:
            if 'ds_software_version' in c:
                df2[c] = df2[c].astype(int)
            if '.p_act' in c or '.p_set' in c:
                df2[c] = df2[c].astype(float)
            if c in mappers and not pd.api.types.is_string_dtype(df2[c]):
                df2[c] = df2[c].replace(mappers[c])
