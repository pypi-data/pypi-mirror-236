...
class TestAcuSim(unittest.TestCase):
    ...
    def test_tracking_table(self):
        ...
        self.assertFalse(df.empty)
        self.assertFalse(df.isnull().values.any())
        chans = 'acu.elevation.state acu.azimuth.state acu.feed_indexer.state acu.general_management_and_controller.state acu.stow_pin_controller.azimuth_status'
        chans = chans.split()
        failed_chans = {c:df[c].dtype for c in chans if not pd.api.types.is_string_dtype(df[c])}

        self.assertFalse(list(failed_chans.values()), 'fail on channels status: ' + ', '.join(failed_chans.keys()))
