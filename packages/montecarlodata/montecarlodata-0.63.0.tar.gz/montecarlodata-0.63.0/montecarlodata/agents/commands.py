...
def register_s3_store(ctx, bucket_name, **kwargs):
    AgentService(config=ctx["config"]).create_agent(
...
def deregister(ctx, agent_id):
    ...
    """
    AgentService(config=ctx["config"]).delete_agent(agent_id)


@agents.command(help="Perform a health check of the Agent.")
@click.pass_obj
@click.option("--agent-id", help="UUID of Agent.", required=False)
@add_common_options(DISAMBIGUATE_DC_OPTIONS)
def health(ctx, **kwargs):
    """
    Check the health of an Agent
    """
    AgentService(config=ctx["config"]).check_agent_health(**kwargs)
...
def upgrade(ctx, **kwargs):
    ...
    """
    AgentService(config=ctx["config"]).upgrade_agent(**kwargs)
...
def list_agents(ctx):
    ...
    """
    AgentService(config=ctx["config"]).echo_agents()
