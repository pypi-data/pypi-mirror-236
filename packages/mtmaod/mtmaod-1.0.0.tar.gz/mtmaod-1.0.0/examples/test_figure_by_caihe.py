...
#     threshold = tf.fill([1, 1], 0.3)
#     mae = abs(y_pred - y_actual)
#     mse = (y_pred - y_actual) ** 2
#     return tf.where(y_actual > threshold, mse, mae)
# model=tf.keras.models.load_model('logs',custom_objects={'custom_loss':custom_loss})

##自定义损失函数huberloss
# def huber_loss(y_true, y_pred, clip_delta = 0.3):
#     error = y_true - y_pred
#     cond = tf.keras.backend.abs(error) <= clip_delta
#     squared_loss = 0.5 * tf.keras.backend.square(error)
#     linear_loss = clip_delta * (tf.keras.backend.abs(error) - 0.5 * clip_delta ** 2)
#     return tf.where(cond, squared_loss, linear_loss)
# model=tf.keras.models.load_model('logs',custom_objects={'huber_loss':huber_loss})
...
# input1_test,input2_test,label_test = db_all
# #model.evaluate([input1_test, input2_test], [label_test])
# label_predict=model.predict([input1_test, input2_test])
