...
def load(run_id = None, compute_results=False):
    ...
    if compute_results == True:
        if response_dict['dataset_library'] == 'PyTorch':
            print('Getting test_loader object at: ', response_dict['test_dataloader'])
            if 'test_dataloader' in response_dict:
              with open(response_dict['test_dataloader'], "rb") as file:
                  test = dill.load(file)
        if response_dict['model_library'] == 'PyTorch':
            print('Getting model network at: ', response_dict['best_model_network'])
            Netx = output_dict['best_model_network']
            print('Getting model state_dict at: ', response_dict['best_model_statedict'])
            Netx.load_state_dict(output_dict['best_model_statedict'])
