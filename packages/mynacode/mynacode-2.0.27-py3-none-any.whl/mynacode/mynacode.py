...
def torch_model(model=None, model_name='best_model_chkpt', metric_name=None, metric_value=None, goal='maximize', current_epoch=None, track={}, run_id=None):
    ...
    if model:
      ...
      if goal == 'maximize':
        if metric_value > prev_max:
          ...
          torch.save(model.state_dict(), run_dir_global+'/model/'+model_name+'.pt')
          ...
          model_dict = {'library':'PyTorch', 'metric_values': metric_values,
                              'best_model_statedict_path': run_dir_global+'/model/'+model_name+'.pt',
...
def torch_model_save(model=None, model_name='best_model_chkpt', metric_name=None, metric_value=None, current_epoch=None, track={}, run_id=None):
...
try:
  class MynacodeCallback(keras.callbacks.Callback):
      ...
      def __init__(self, metric_name, goal='maximize', model_name='best_model_chkpt', track={}, run_id=None):
          ...
          if self.metric_name not in logs.keys():
            ...
            if self.goal == 'maximize':
              if logs.get(self.metric_name) > prev_max:
                ...
                self.model.save(run_dir_global+'/model/'+self.model_name+'.hdf5')
                ...
                model_dict = {'library':'Keras', 'metric_values': metric_values,
                                    'best_model_path': run_dir_global+'/model/'+self.model_name+'.hdf5',
                                    'best_model_logs_path': run_dir_global+'/model/best_model_logs.txt',}
...
def keras_model_save(model=None, model_name='best_model_chkpt', metric_name=None, metric_value=None, current_epoch=None, track={}, run_id=None):
    ...
    if model:
        ...
        self.model.save(run_dir_global+'/model/'+model_name+'.hdf5')
...
def results(y_true = [], y_pred = [], threshold=0.5, results_dict = {}, problem_type = 'binary classification', run_id = None):
    ...
    if results_dict:
        ...
        yaml.dump({'precision': round(prec, 4), 'recall': round(rec, 4), 'specificity': round(spec, 4),
              'f1': round(f1, 4), 'accuracy': round(acc, 4), 'npv': round(npv_val, 4), 'c_matrix': c_matrix,
              'test_auc': roc_auc, 'threshold': round(threshold, 4)},file)
        file.close()
...
def load(run_id = None, compute_results=False):
    ...
    if compute_results == True:
        ...
        print("\nLoading in test dataloader and model...")
