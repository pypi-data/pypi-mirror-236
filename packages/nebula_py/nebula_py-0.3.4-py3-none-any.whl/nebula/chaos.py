...
class derive:
    def __init__(self: Self, /, *derives: Callable[[type], type]) -> None:
        self.derives = derives

    def __call__(self: Self, /, cls: T) -> T:
...
def ψηαοσ() -> bool:
    try:
        ...
        @fishhook.hook.cls(dict)
        class dict_hook:
            setget = setget
            σγ = σγ

        @fishhook.hook.cls(int | float | complex)
        class int_hook:
            def __call__(self: Self, other):
                return self * other

        @fishhook.hook.cls(FunctionType)
        class function_hook:
        ...
        @fishhook.hook.cls(object)  # scary
        class obj_hook:
            for_each = for_each
            ϟ = ϟ
