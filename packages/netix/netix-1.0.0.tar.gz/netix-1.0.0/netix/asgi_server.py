    ...
    async def send(self, data):
        try:
            if data['type'] in ('http.response.start', 'websocket.accept'):
                ...
                status_code, status_message = self.response.get_status()
                status_message_decoded = status_message.decode('latin-1')

                log_message = f"{log_date()} {BOLD}{self._scope['client'][0]}:{self._scope['client'][1]}{RESET} - "

                # Determine the color for HTTP method
                if self._scope['method'] in {'GET', 'POST'}:
                    method_color = GREEN
                elif self._scope['method'] in {'PUT', 'DELETE'}:
                    method_color = RED
                else:
                    method_color = YELLOW

                # Determine the color for status code
                success_status_codes = {200, 201, 302}  # Add more as needed
                if status_code in success_status_codes:
                    status_color = GREEN
                elif status_code in {404, 400, 405, 401, 403, 402}:  # Add more error status codes as needed
                    status_color = RED
                else:
                    status_color = YELLOW

                # Determine the path color based on the status color
                path_color = status_color

                # Format the log message
                log_message += f"\"{BOLD}{method_color}{self._scope['method']}{RESET} {path_color}{self._scope['path']}{RESET} {BOLD}{self._scope['http_version']}{RESET}\" {status_color}{status_code}, {status_message_decoded}{RESET}"
                print(log_message)
                ...
                if 'headers' in data:
                    for header in data['headers']:
                        ...
                        self.response.append_header(b'%s: %s\r\n' % (header[0].encode('utf-8'), header[1].encode('utf-8')))
