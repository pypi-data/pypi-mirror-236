...
@jitclass(spec)
class _Graph:
    def __init__(self, n_children_max: int64, n_nodes: int64, edges: int64[:], node_types):

        self.n_children_max = n_children_max+1
    ...
    def make_children_map(self):
        ...
        for i, e in enumerate(self.edges):
            ...
            row[0] += 1
            if row[0] > self.n_children_max:
                raise ValueError('More children than allowed!')
            row[row[0]] = e[1]

            row_pe = self.parent_edges[e[1], :]
            row_pe[0] += 1
            if row_pe[0] > self.n_children_max :
                raise ValueError('More parents than allowed!')
            row_pe[row_pe[0]] = i

            row_ce = self.children_edges[e[0], :]
            row_ce[0] += 1
            if row_ce[0] > self.n_children_max:
                raise ValueError('More parents than allowed!')
            row_ce[row_ce[0]] = i

    def make_edges_map(self, edges, nodes):
        n_nodes = len(nodes)
        parent_edges = np.zeros((n_nodes, self.n_children_max), np.int64)
        children_edges = np.zeros((n_nodes, self.n_children_max), np.int64)
    ...
    def get_zero_indegree(self):
        n_zero_indegree = 0
        zero_indegree = np.zeros(self.n_nodes, dtype=np.int64)
    ...
    def topological_sort(self):
        ...
        sorted_nodes = np.zeros(self.n_nodes, np.int64) * -1
        n_sorted = 0
    ...
    def get_dependants_graph_subgraph(self, nodes, subedges):
        ...
        edges_visited = np.zeros(len(subedges), dtype=np.int64)
        n_visited = int64(0)
        for n in nodes:
        ...
        nodes = np.zeros(2 * len(edges), dtype=np.int64)
    ...
    def detect_cyclic_graph(self):
        ...
        for i in range(n_zero_indegree):
            zid = zero_indegree[i]
            cyclic_dependency, cyclic_path = depth_first_search(zid, np.zeros((0,), dtype=np.int64), self.children)
            if cyclic_dependency >= 0:
                print('cyclic dependency: ', cyclic_dependency)
                print('cyclic path: ', cyclic_path)
                return cyclic_dependency, cyclic_path

        return int64(-1), np.zeros((0,), dtype=np.int64)
