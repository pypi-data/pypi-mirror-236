...
def docker_status_review(ssh, stop_running_containers, remove_volumes, remove_images):
    """
...
def manage(
    ...
    cache_logs_from_vpu: str = "/home/oem/share/logs>~/o3r_logs/From_VPUs",
    set_vpu_name: str = "o3r_docker_manager_test_000",

    stop_running_containers: bool = False,
    ...
    if USING_IPYTHON:
        ...
        # TODO copy arguments from above into here so that ipython testing works
        ...
        setup_volume: str = ""
        load_image: str = ""
        items_to_cleanup: str = ""
        ...
        # # format for boolean arguments
        # feature = True
        # parser.add_argument('--feature', action='store_true', help = f"Does a thing, Defaults to {feature}")
        ...
        parser.add_argument(
            "--cache_logs_from_vpu", type=str, default=cache_logs_from_vpu, help=f"specifies how to cache logs from VPU and won't cache anything if blank, Defaults to {cache_logs_from_vpu}"
        )
        parser.add_argument(
        ...
        parser.add_argument('--stop_running_containers', action='store_true',
                            help=f"Defaults to {stop_running_containers}")
        parser.add_argument('--dont_stop_running_containers',
                            dest='stop_running_containers', action='store_false')
        parser.set_defaults(stop_running_containers=stop_running_containers)

        parser.add_argument('--remove_volumes', action='store_true',
                            help=f" Defaults to {remove_volumes}")
        parser.add_argument('--dont_remove_volumes',
                            dest='remove_volumes', action='store_false')
        parser.set_defaults(remove_volumes=remove_volumes)

        parser.add_argument('--remove_images', action='store_true',
                            help=f"Defaults to {remove_images}")
        parser.add_argument('--dont_remove_images',
                            dest='remove_images', action='store_false')
        parser.set_defaults(remove_images=remove_images)

        parser.add_argument('--mount_usb_volumes', action='store_true',
                            help=f"Defaults to {mount_usb_volumes}")
        parser.add_argument('--dont_mount_usb_volumes',
                            dest='mount_usb_volumes', action='store_false')
        ...
        parser.add_argument(
            "--setup_volume", type=str, default=setup_volume, help=f"Path/to/directory/to/mount and volume_name separated by comma. If empty, will not create a volume, Defaults to {setup_volume}"
        )
        parser.add_argument(
            "--load_image", type=str, default=load_image, help=f"Path/to/docker_image.tar and image_name separated by comma. If empty, will not load an image, Defaults to {load_image}"
        )
        parser.add_argument(
        ...
        parser.add_argument('--dont_stop_upon_exit',
                            dest='stop_upon_exit', action='store_false')
        ...
        cache_logs_from_vpu = args.cache_logs_from_vpu
        set_vpu_name = args.set_vpu_name

        stop_running_containers = args.stop_running_containers
        ...
        setup_volume = args.setup_volume
    ...
    if USING_IFM3DPY and cache_logs_from_vpu:
        vpu_log_dir, local_log_cache = cache_logs_from_vpu.split(">")
    ...
    docker_status_review(ssh, stop_running_containers,
                         remove_volumes, remove_images)

    # %%#####################################
    # TODO: mount any usb devices
    #########################################
    ...
    if load_image:
        # load image
        docker_image_fname = Path(load_image).name
        logger.info("loading image into vpu docker storage")
        cmd = f"cat {load_image}| docker import - {docker_image_fname[:-4]}"
        _stdin, _stdout, _stderr = ssh.exec_command(cmd)
        logger.info(_stdout.read().decode().strip() +
                    _stderr.read().decode().strip())
    ...
    if setup_volume:
        ...
        path_for_volume_to_mount, volume_name = setup_volume.split(",")
    ...
    initialized = False
    if initialize:
        docker_compose_fname, service_name = initialize.split(":")
    ...
    if attach_to:
        ...
        if not (initialize) or initialized:
