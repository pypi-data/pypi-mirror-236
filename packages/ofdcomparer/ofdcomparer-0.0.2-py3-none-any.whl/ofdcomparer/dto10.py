...
from ofdcomparer.kkt_libs.libfptr10 import IFptr

from ofdcomparer.kkt_libs.dto_error_descriptions import get_dto_error_code_description

DTO10_LOG_ENVIRONMENT_NAME = "DTO10_LOG_CONFIG_FILE"
DTO10_LOG_LEVEL_ERROR = "ERROR"
DTO10_LOG_LEVEL_INFO = "INFO"
DTO10_LOG_LEVEL_DEBUG = "DEBUG"
DTO10_DEFAULT_LOG_PATH = join(os.path.expanduser('~'), "AppData", "Roaming", "ATOL", "drivers10", "logs")
DTO10_DRIVER_PROPERTIES_FILE_NAME = "fptr10_log.properties"
DTO10_DRIVER_PROPERTIES_FILE = join(DTO10_DEFAULT_LOG_PATH, "../..", DTO10_DRIVER_PROPERTIES_FILE_NAME)
...
class DTO10Helper:
    ...
    def request_error_code(self, flag_all_kkt_settings=False):
        ...
        if self.ErrorCode != IFptr.LIBFPTR_OK:
            if flag_all_kkt_settings and self.ErrorCode == IFptr.LIBFPTR_ERROR_NOT_SUPPORTED:
                pass
            else:
    ...
    def get_last_fiscal_document_number(self):
        ...
        self.request_error_code()
        return int(self.fptr.getParamInt(IFptr.LIBFPTR_PARAM_DOCUMENT_NUMBER))
    ...
    def set_connection_settings_to_ethernet(self, ip, port):
        self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_PORT, str(IFptr.LIBFPTR_PORT_TCPIP))
        self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_IPADDRESS, str(ip))
        self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_IPPORT, str(port))
    ...
    def is_connection_type_ethernet(self):
        if not self.fptr.isOpened():
            return False
        connection_type = self.fptr.getSingleSetting(IFptr.LIBFPTR_SETTING_PORT)
        if not int(connection_type) == IFptr.LIBFPTR_PORT_TCPIP:
    ...
    def set_connection_settings_to_usb(self, usb_device_path):
        ...
        self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_USB_DEVICE_PATH, str(usb_device_path))
        self.fptr.applySingleSettings()
        return self.request_error_code()
    ...
    def is_connection_type_usb(self):
        ...
        connection_type = self.fptr.getSingleSetting(IFptr.LIBFPTR_SETTING_PORT)
        self.request_error_code()
        if not int(connection_type) == IFptr.LIBFPTR_PORT_USB:
    ...
    def connect_to_kkt_by_rs(self, com="COM1", baudrate=115200):
        ...
        if self.set_connection_settings_to_rs(com, baudrate) != IFptr.LIBFPTR_OK:
    ...
    def set_connection_settings_to_rs(self, com, baudrate):
        ...
        self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_COM_FILE, com)
        self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_BAUDRATE, str(baudrate))
        self.fptr.applySingleSettings()
        return self.request_error_code()
    ...
    def is_connection_type_rs(self):
        ...
        connection_type = self.fptr.getSingleSetting(IFptr.LIBFPTR_SETTING_PORT)
        self.request_error_code()
        if not int(connection_type) == IFptr.LIBFPTR_PORT_COM:
    ...
    def get_model_name(self):
        ...
        self.fptr.queryData()
        model_name = str(self.fptr.getParamString(IFptr.LIBFPTR_PARAM_MODEL_NAME))
        return model_name

    def set_no_print_flag(self, no_print):
        self.fptr.setParam(IFptr.LIBFPTR_PARAM_REPORT_ELECTRONICALLY, no_print)
        return True
    ...
    def get_fatal_errors_status(self):
        ...
        counter_fault = self.fptr.getParamBool(IFptr.LIBFPTR_PARAM_COUNTERS_FAULT)
        ...
        script_fault = self.fptr.getParamBool(IFptr.LIBFPTR_PARAM_SCRIPTS_FAULT)
    ...
    def set_setting_id(self, _id, _value):
        self.fptr.setParam(IFptr.LIBFPTR_PARAM_SETTING_ID, _id)
        self.fptr.setParam(IFptr.LIBFPTR_PARAM_SETTING_VALUE, _value)
    ...
    def get_setting_id(self, _id):
        ...
        self.request_error_code()
        return self.fptr.getParamString(IFptr.LIBFPTR_PARAM_SETTING_VALUE)
    ...
    def print_string(self, text=None, alignment=None, wrap=None, font=None, double_width=None, double_height=None,
        ...
        if font is not None:
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_FONT, font)
        if double_width is not None:
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_FONT_DOUBLE_WIDTH, double_width)
        if double_height is not None:
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_FONT_DOUBLE_HEIGHT, double_height)
        if linespacing is not None:
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_LINESPACING, linespacing)
        if brightness is not None:
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_BRIGHTNESS, brightness)
        if store_in_journal is not None:
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_STORE_IN_JOURNAL, store_in_journal)
        self.fptr.printText()
    ...
    def get_fn_info(self):
        ...
        serial = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_SERIAL_NUMBER)
        version = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_FN_VERSION)
        type = self.fptr.getParamInt(IFptr.LIBFPTR_PARAM_FN_TYPE)
        state = self.fptr.getParamInt(IFptr.LIBFPTR_PARAM_FN_STATE)
        flags = self.fptr.getParamInt(IFptr.LIBFPTR_PARAM_FN_FLAGS)

        need_replacement = self.fptr.getParamBool(IFptr.LIBFPTR_PARAM_FN_NEED_REPLACEMENT)
        exhausted = self.fptr.getParamBool(IFptr.LIBFPTR_PARAM_FN_RESOURCE_EXHAUSTED)
        memory_overflow = self.fptr.getParamBool(IFptr.LIBFPTR_PARAM_FN_MEMORY_OVERFLOW)
        ofd_timeout = self.fptr.getParamBool(IFptr.LIBFPTR_PARAM_FN_OFD_TIMEOUT)
        critical_error = self.fptr.getParamBool(IFptr.LIBFPTR_PARAM_FN_CRITICAL_ERROR)
    ...
    def get_last_doc_with_template(self, tempalate, variable_settings=None):
        ...
        for _ in range(3):
            ...
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_JSON_DATA, request_last_fn_doc)
            self.fptr.processJson()
            last_fn_doc_str = (self.fptr.getParamString(IFptr.LIBFPTR_PARAM_JSON_DATA))
            if last_fn_doc_str != "":
                break
    ...
    def execute_json(self, json_task, timeout=30):
        ...
        try:
            ...
            self.fptr.open()
            self.fptr.setParam(IFptr.LIBFPTR_PARAM_JSON_DATA, json.dumps(json_task))
            self.fptr.processJson()
            response_raw = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_JSON_DATA)
    ...
    def get_fn_number(self):
        self.fptr.setParam(IFptr.LIBFPTR_PARAM_DATA_TYPE, IFptr.LIBFPTR_DT_CACHE_REQUISITES)
        self.fptr.queryData()

        serialNumber    = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_FN_SERIAL_NUMBER)
        ecrRegNumber    = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_ECR_REGISTRATION_NUMBER)
        ofdVatin        = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_OFD_VATIN)
        fnsUrl          = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_FNS_URL)
        ffdVersion      = self.fptr.getParamInt(IFptr.LIBFPTR_PARAM_FFD_VERSION)
        return serialNumber
    ...
    def process_json(self, json_task, answer_indent=4, flag_all_kkt_settings=False):
        self.fptr.setParam(IFptr.LIBFPTR_PARAM_JSON_DATA, json_task)
        self.fptr.processJson()
        error = self.request_error_code(flag_all_kkt_settings=flag_all_kkt_settings)
        json_str_answer = self.fptr.getParamString(IFptr.LIBFPTR_PARAM_JSON_DATA)
        if json_str_answer != "":
            self.JsonAnswer = None
            try:
