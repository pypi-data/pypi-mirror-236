...
from ogmios.model.model_map import Method, Direction
...
class NextBlock:
    ...
    def receive(self) -> (Direction, Tip, Point | Origin | Block, Optional[Any]):
        ...
        return _parse_NextBlock_response(response)


def _parse_NextBlock_response(
    response: dict,
) -> (Direction, Tip, Point | Origin | Block, Optional[Any]):
    if response.get("method") != Method.nextBlock.value:
    ...
    if result := response.get("result"):
        if result.get("direction") and result.get("tip"):
            direction: Direction = _parse_direction(result.get("direction"))
            tip: Tip | Origin = rh.parse_TipOrOrigin(result.get("tip"))
            id: Optional[Any] = response.get("id")
            if block_resp := result.get("block"):
    ...
    ID = {id}"""
            ...
            elif point_resp := result.get("point"):
                point: Point | Origin = rh.parse_PointOrOrigin(point_resp)
                logger.info(
                    f"""Parsed NextBlock response:
    Direction = {direction}
    Tip = {tip}
...
def _parse_direction(value: str) -> Direction:
    ...
    raise InvalidResponseError(f"next_block response contains invalid direction parameter: {value}")
