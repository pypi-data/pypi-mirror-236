...
def _set_creator(omoospace):
    ...
    class URLValidator(Validator):
        ...
        def validate(self, document):
            # Allow empty
            if len(document.text) > 0 and not is_url(document.text):
                raise ValidationError(
    ...
    if creator_id >= 0:
        ...
        creator_profile = find_first(
            registered_creators, "email", creator_email)
    ...
    creator_role = inquirer.fuzzy(
        ...
        choices=["*NotInclude", *registered_roles],
        style=inquirer_style
    ).execute()
    ...
    creator_profile["role"] = creator_role or None
    ...
    if is_confirm:
        registered_roles.append(creator_role)
        Setting().registered_roles = registered_roles
        omoospace.set_creator(creator_profile)
...
def _set_software(omoospace):
    ...
    software_name = inquirer.fuzzy(
        ...
        choices=["*NotInclude", *registered_softwares],
        style=inquirer_style
    ).execute()
    ...
    software_profile: Software = {
        "name": software_name,
        "version": software_version or None,
        "plugins": None,
    }
    console.print(software_profile)
    is_confirm = inquirer.confirm(
        message="Confirm",
        default=True,
        style=inquirer_style
    ...
    if is_confirm:
        registered_softwares.append(software_name)
        Setting().registered_softwares = registered_softwares
        omoospace.set_software(software_profile)
...
def _set_work(omoospace):
    ...
    if is_confirm:
        omoospace.set_work(work_profile)
