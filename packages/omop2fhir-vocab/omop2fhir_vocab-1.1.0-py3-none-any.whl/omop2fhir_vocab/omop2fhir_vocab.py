...
def omop2fhir(
    ...
    exclude_codesystem=False, exclude_conceptmap=False, caching: List[str] = None
):
    """Run program"""
    # OMOP2OWL
    ...
    d: Dict = omop2owl(
        ...
        skip_semsql=True, retain_general_cache=omop2owl_cache, use_cache=omop2owl_cache)
...
def cli_parser(title: str = PROG, description: str = DESC) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        help='Which relationship types from the concept_relationship table\'s relationship_id field to include? '
             'Default is "Is a" only. Passing "ALL" includes everything. Usage: --realationships "Is a" "Maps to"')
    ...
    parser.add_argument(
        ...
        help='Caching options. Warning: Developer option. Use at your own risk. Currently this option is in development'
             ' and it will only work if you don\'t change your options after running it for the first time. If you use '
             'this and then want to change your options, particularly --vocabs and --relationships, you will have to '
             '(a) not use caching for subsequent runs, or (b) uninstall and reinstall the program. '
             '`all`: Turns on all cache options. `omop2fhir`: Saves any intermediate files at the top '
