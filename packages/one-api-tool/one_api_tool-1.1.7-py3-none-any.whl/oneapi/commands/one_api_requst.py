...
def main():
    ...
    if args.prompt and args.config_file:
        ...
        rprint(Markdown(f"\nWelcome to **One API**.\n"))
        questions = [inquirer.List('api_type', message="Please select the API type you want to use", choices=['open_ai', 'azure', 'claude', 'custom'])]
        ...
        if f'{key_word}_API_KEY' in os.environ:
            ...
            chat_template = os.environ.get(f"{key_word}_CHAT_TEMPELATE").strip()
            if api_base.strip() == "":
            ...
            if api_type == "custom":
                questions = [
                    inquirer.Text("api_base", message=f"{key_word.title()} Base URL and port.", validate=lambda _ , c: len(c) > 0),
                    inquirer.Text("chat_template", message="Please enter the chat template you want to use in Jinja2 format. Enter to use the default chat template."),
                ...
                questions = [inquirer.Text("api_key", message=f"{key_word.title()} API key", validate=lambda _ , c: len(c) > 10 if api_type != "custom" else True),
        ...
        if api_type == 'custom' and chat_template:
