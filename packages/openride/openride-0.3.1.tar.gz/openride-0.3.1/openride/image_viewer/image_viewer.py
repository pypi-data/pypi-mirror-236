...
class ImageViewer:
    ...
    def labelize(self, label: str, pos: Point, color: tuple = (1,1,1), fontsize: float = 1.0, thickness:int=1):
        point2d = self.__project_points(np.array((pos.x, pos.y, pos.z))).astype(int)
        if point2d.size == 0:
            return
        self.display = cv2.putText(self.display, label, point2d[0], cv2.FONT_HERSHEY_SIMPLEX, fontsize, color, thickness)


    def draw_rectangle(self, xmin:float, ymin:float, xmax:float, ymax:float, color:tuple=(1,1,1), thickness:int=1, fill:bool=False, alpha:float=1.0):
        _thickness = thickness if not fill else -1 * thickness
        if alpha >= 1.0:
            self.display = cv2.rectangle(self.display, (int(xmin), int(ymin)), (int(xmax), int(ymax)), color, _thickness)
        elif alpha > 0.0:
            tmp = np.zeros_like(self.display)
            tmp = cv2.rectangle(tmp, (int(xmin), int(ymin)), (int(xmax), int(ymax)), color, _thickness)
            mask = tmp.astype(bool)
            self.display[mask] = cv2.addWeighted(self.display, 1-alpha, tmp, alpha, 0)[mask]
    ...
    def draw_bounding_box(
        ...
        if alpha >= 1.0:
            ...
            tmp = np.zeros_like(self.display)
            for line in self.__get_box_projection(bounding_box):
                cv2.line(tmp, line[0], line[1], color, linewidth)
            mask = tmp.astype(bool)
            self.display[mask] = cv2.addWeighted(self.display, 1-alpha, tmp, alpha, 0)[mask]
    ...
    def draw_polyline(
        ...
        if alpha >= 1.0:
            ...
            tmp = np.zeros_like(self.display)
            for i in range(mask.shape[0] - 1):
                cv2.line(tmp, points2d[mask][i], points2d[mask][i + 1], color, linewidth)
            mask = tmp.astype(bool)
            self.display[mask] = cv2.addWeighted(self.display, 1-alpha, tmp, alpha, 0)[mask]
