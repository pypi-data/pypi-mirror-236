...
def _recover_project(prj_path: str):
    ...
    click.secho(
        f"Project [{project_name}] with namespace [{namespace}] was successfully created from [{prj_path}].", fg="bright_green"
    )
...
@click.option("--prj_path", help="If set, project will be created according to config file of this path.")
def create_project(name: str, namespace: str, desc: Optional[str], prj_path: Optional[str]):
    """
    ...
    click.secho(
        ...
        + f"  cd {project_dir}", fg="bright_green"
    )
