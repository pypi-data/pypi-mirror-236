...
"""Helper functions for java class loader"""
...
class ClassLoader:
    def __init__(self):
        ...
        searchEngineConnectionInfo = findClass(
            'com.alipay.openkgengine.common.model.datasource.connection.SearchEngineConnectionInfo')
        global_class['SearchEngineConnectionInfo'] = searchEngineConnectionInfo

        elasticSearchEngineClientDriver = findClass(
            "com.alipay.openkgengine.cloudext.impl.searchengine.elasticsearch.ElasticSearchEngineClientDriver")
        global_class['SearchEngineConnectionInfo'] = searchEngineConnectionInfo
...
def SearchRequest(query, idx_name, start, size):
    ...
    searchRequest = findClass(
        "com.alipay.openkgengine.cloudext.interfaces.searchengine.model.request.SearchRequest")
...
def MatchQuery(name: str, value):
    from pemja import findClass
    matchQuery = findClass('com.alipay.openkgengine.cloudext.interfaces.searchengine.model.request.query.MatchQuery')
    return matchQuery(name, value)


def TermQuery(term: str, value):
    from pemja import findClass
    termQuery = findClass('com.alipay.openkgengine.cloudext.interfaces.searchengine.model.request.query.TermQuery')
    return termQuery(term, value)


def QueryGroup(queries: list, op):
    from pemja import findClass
    queryGroup = findClass('com.alipay.openkgengine.cloudext.interfaces.searchengine.model.request.query.QueryGroup')
...
def OperatorType():
    ...
    operatorType = findClass(
        'com.alipay.openkgengine.cloudext.interfaces.searchengine.model.request.query.OperatorType')
    return operatorType
