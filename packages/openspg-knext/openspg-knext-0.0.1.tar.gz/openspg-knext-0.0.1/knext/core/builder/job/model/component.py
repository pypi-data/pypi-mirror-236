...
from knext.core.builder.operator import Operator, BaseOp
...
class Component(ABC):
    ...
    def __rshift__(self, other: Union[T, List[T]]):
        last = [self]
        while len(last) > 0 and last[0].next:
            last = last[0].next
...
class KnowledgeExtractComponent(Component):
    ...
    def _to_rest(self):
        ...
        """
        client = Operator()
        operator_config = client._generate_op_config(op_name=self.operator, params=self.params)
...
class RelationMappingComponent(Component):
    ...
    def __init__(self, subject_name: Union[Type, str], predicate_name: str, object_name: Union[Type, str]):
...
class SPGMappingComponent(Component):
    ...
    def set_operator(self, extract_op: str, params: Dict[str, str]):
        self.operator = extract_op
        self.operator = extract_op
        return self
    ...
    def _translate(self):
        ...
        """
        extract = KnowledgeExtractComponent([]).set_operator(self.operator)

        subject_mapping = EntityMappingComponent(self.spg_type_name) \
            .add_filter("__vertex_type__", self.spg_type_name)
        subject_type = self.schema_session.get(self.spg_type_name)
        ...
        for prop in subject_type.properties:
            ...
            object_mapping = EntityMappingComponent(object_type_name) \
                .add_filter("__vertex_type__", object_type_name) \
                .add_field("id", "id").add_field("name", "name")
