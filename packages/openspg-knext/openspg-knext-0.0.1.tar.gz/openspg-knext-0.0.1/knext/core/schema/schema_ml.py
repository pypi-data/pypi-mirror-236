...
from knext.core.schema.model import EntityType, ConceptType, EventType, StandardType, Property, Relation
from knext.core.schema.model.base import HypernymPredicateEnum, BasicTypeEnum, ConstraintTypeEnum, \
    AlterOperationEnum, SpgTypeEnum, PropertyGroupEnum
...
class SPGSchemaMarkLang:
    ...
    parsing_register = {RegisterUnit.Type: None, RegisterUnit.Property: None, RegisterUnit.Relation: None, RegisterUnit.SubProperty: None}
    ...
    def __init__(self, filename):
        ...
        for type_name in session._spg_types:
            ...
            if session.get(type_name).spg_type_enum in [SpgTypeEnum.Basic, SpgTypeEnum.Standard]:
        ...
        if step < 0:
            ...
            if self.current_parsing_level == IndentLevel.PredicateMeta.value:
                ...
                if self.parsing_register[RegisterUnit.Property] is not None:
                    self.save_register(RegisterUnit.Property, Property(name="_", object_type_name="Thing"))
                elif self.parsing_register[RegisterUnit.Relation] is not None:
                    self.save_register(RegisterUnit.Relation, Relation(name="_", object_type_name="Thing"))
        elif step == 1:
            assert self.current_parsing_level + 1 < len(IndentLevel), self.error_msg("Invalid indentation (too many levels?)")
    ...
    def parse_type(self, expression):
        ...
        if namespace_match:
            assert self.namespace is None, self.error_msg("Duplicated namespace define, please ensure define it only once")

            self.namespace = namespace_match.group(1)
            return

        type_match = re.match(r"^([a-zA-Z0-9\.]+)\((\w+)\):\s*?([a-zA-Z0-9,]+)$", expression)
        if type_match:
            assert self.namespace is not None, self.error_msg("Missing namespace, please define namespace at the first")

            type_name = type_match.group(1)
            type_name_zh = type_match.group(2).strip()
            type_class = type_match.group(3).strip()
            assert type_class in self.keyword_type, self.error_msg(f'{type_class} is illegal, please define it before current line')

            spg_type = None
            if type_class == "EntityType":
                spg_type = EntityType(name=f'{self.namespace}.{type_name}', name_zh=type_name_zh)
            elif type_class == "ConceptType":
                spg_type = ConceptType(name=f'{self.namespace}.{type_name}', name_zh=type_name_zh,
                                       hypernym_predicate=HypernymPredicateEnum.IsA)
            elif type_class == "EventType":
                spg_type = EventType(name=f'{self.namespace}.{type_name}', name_zh=type_name_zh)
            ...
            assert ns_type_name not in self.types, self.error_msg(f'Type "{type_name}" is duplicated in the schema')

            self.types[ns_type_name] = spg_type
            self.save_register(RegisterUnit.Type, spg_type)
            return

        sub_type_match = re.match(r"^([a-zA-Z0-9]+)\((\w+)\)\s*?->\s*?([a-zA-Z0-9\.]+):$", expression)
        if sub_type_match:
            assert self.namespace is not None, self.error_msg("Missing namespace, please define namespace at the first")
            ...
            assert type_class not in self.keyword_type, self.error_msg(f'{type_class} is not a valid inheritable type')
            assert ns_type_class in self.types, self.error_msg(f'{type_class} not found, please define it first')

            parent_spg_type = self.types[ns_type_class]
            spg_type = copy.deepcopy(parent_spg_type)
            spg_type.name = type_name
            spg_type.name_zh = type_name_zh
            spg_type.parent_spg_type = parent_spg_type
        ...
        raise Exception(self.error_msg("unrecognized expression, expect namespace A or A(B):C or A(B)->C"))
    ...
    def parse_type_meta(self, expression):
        ...
        match = re.match(r"^(desc|properties|relations|hypernymPredicate):\s*?(.*)$", expression)
        assert match, self.error_msg("Unrecognized expression, expect desc:|properties:|relations:")
        ...
        if type_meta == "desc" and len(meta_value) > 0:
            ...
            assert meta_value in ["isA", "locateAt"], self.error_msg("Invalid hypernym predicate, expect isA or locateAt")
            assert self.parsing_register[
                       RegisterUnit.Type].spg_type_enum == SpgTypeEnum.Concept, self.error_msg("Hypernym predicate is available for concept type only")

            if meta_value == "isA":
                self.parsing_register[RegisterUnit.Type].hypernym_predicate = HypernymPredicateEnum.IsA
            else:
                self.parsing_register[RegisterUnit.Type].hypernym_predicate = HypernymPredicateEnum.LocateAt
        return
    ...
    def parse_predicate(self, expression):
        ...
        match = re.match(r"^([a-zA-Z0-9]+)\((\w+)\):\s*?([a-zA-Z0-9,\.]+)$", expression)
        assert match, self.error_msg("Unrecognized expression, expect pattern like english(Chinese):Type")
        ...
        assert f'{self.namespace}.{predicate_class}' in self.types or predicate_class in self.internal_type, \
            self.error_msg(f'{predicate_class} is illegal')
        assert predicate_name not in self.entity_internal_property, self.error_msg(f'property {predicate_name} is the default property of type')
        if self.parsing_register[RegisterUnit.Relation] is not None:
            assert predicate_name not in self.parsing_register[RegisterUnit.Relation].sub_properties, \
                self.error_msg(
                    f'Property "{predicate_name}" is duplicated under the relation {self.parsing_register[RegisterUnit.Relation].name}')
        else:
            assert predicate_name not in self.parsing_register[RegisterUnit.Type].properties, \
                self.error_msg(
                    f'Property "{predicate_name}" is duplicated under the type {type_name[type_name.index(".") + 1:]}')
        if predicate_class == "ConceptType":
            assert not self.is_internal_property(predicate_name, SpgTypeEnum.Concept), \
                self.error_msg(f'property {predicate_name} is the default property of ConceptType')
        if predicate_class == "EventType":
            assert not self.is_internal_property(predicate_name, SpgTypeEnum.Event), \
                self.error_msg(f'property {predicate_name} is the default property of EventType')

        if "." not in predicate_class and predicate_class not in BasicTypeEnum.__members__:
        ...
        if self.parsing_register[RegisterUnit.SubProperty]:
            ...
            predicate = Property(name=predicate_name, object_type_name=predicate_class)
            if self.parsing_register[RegisterUnit.Type].spg_type_enum == SpgTypeEnum.Event and predicate_name == "subject":
                assert predicate_class not in self.internal_type,\
                    self.error_msg(f'The subject of event type only allows entity/concept type')
                ...
                if "," in predicate_class:
                    ...
                    for subject_type in subject_types:
                        ...
                        assert subject_type not in BasicTypeEnum.__members__, self.error_msg(f'{predicate_class} is illegal for subject in event type')

                        if "." not in subject_type:
                            subject_type = f'{self.namespace}.{predicate_class}'
                        assert subject_type in self.types, self.error_msg(f'{predicate_class} is illegal')

                        subject_predicate = Property(name=f'subject{subject_type}', object_type_name=subject_type)
                        subject_predicate.property_group = PropertyGroupEnum.Subject
                        self.parsing_register[RegisterUnit.Type].add_property(subject_predicate)
            ...
            assert predicate_class in self.types, self.error_msg(f'{predicate_class} is illegal')
            assert f'{predicate_name}_{predicate_class}' not in self.parsing_register[RegisterUnit.Type].relations, \
                self.error_msg(f'Relation "{match.group()}" is duplicated under the type {type_name[type_name.index(".") + 1:]}')
    ...
    def parse_property_meta(self, expression):
        ...
        assert match, self.error_msg("Unrecognized expression, expect desc:|properties:|constraint:|rule:")
        ...
        if property_meta == "desc" and len(meta_value) > 0:
            ...
            if self.parsing_register[RegisterUnit.SubProperty] is not None:
                self.parse_constraint_for_property(meta_value, self.parsing_register[RegisterUnit.SubProperty])
            elif self.parsing_register[RegisterUnit.Property] is not None:
                self.parse_constraint_for_property(meta_value, self.parsing_register[RegisterUnit.Property])

        elif property_meta == "properties":
            self.save_register(RegisterUnit.SubProperty, Property(name="_", object_type_name="Thing"))
    ...
    def parse_relation_meta(self, expression):
        ...
        assert match, self.error_msg("Unrecognized expression, expect desc:|properties:|rule:")
    ...
    def complete_rule(self, rule):
        ...
        if not match:
            ...
            head = f'Define (s:{subject_name})-[p:{predicate.name}]->(o:{predicate.object_type_name})' + " {\n"
            rule = head + rule
            rule += "\n}"
        ...
        if matches:
            for group in matches:
                if "." in group[1] or group[1].lower() in ["integer", "text", "float"]:
                    continue
                replace_list.append((f'({group[0]}{group[1]})', f'({group[0]}{self.namespace}.{group[1].strip()})'))
            ...
            if self.rule_quote_open:
                ...
                if strip_line.endswith("]]"):
                    ...
                    if len(right_strip_line) > 2:
                        self.rule_quote_predicate.logical_rule += right_strip_line[:len(right_strip_line) - 2]
                    self.rule_quote_predicate.logical_rule = self.complete_rule(self.rule_quote_predicate.logical_rule)
            ...
            if indent_count == 0:
                ...
                for i in range(0, len(self.indent_level_pos)):
                    if indent_count == self.indent_level_pos[i]:
                        ...
                        break
                assert backward_step, self.error_msg(f'Invalid indentation, please align with the previous definition')
    ...
    def diff_sub_property(self, new, old, old_type_name, old_property, new_property):
        ...
        for prop in old:
            if not old_property.inherited and prop not in new:
                ...
                need_update = True
                print(f'Delete sub property: [{old_type_name}] {old_property.name}.{prop}')

        for prop, o in new.items():
            if prop not in old and not new_property.inherited:
                old_property.add_sub_property(new[prop])
                need_update = True
                print(f'Create sub property: [{old_type_name}] {old_property.name}.{prop}')

            elif old[prop].object_type_name != new[prop].object_type_name:
                assert not old_property.inherited, \
                    self.error_msg(f'{old_type_name}] {old_property.name}.{prop} is inherited sub property, deny modify')

                old[prop].alter_operation = AlterOperationEnum.Delete
                old_property.add_sub_property(new[prop])
                need_update = True
                print(f'Recreate sub property: [{old_type_name}] {old_property.name}.{prop}')

            elif old[prop] != new[prop]:
                assert not old_property.inherited, \
                    self.error_msg(f'{old_type_name}] {old_property.name}.{prop} is inherited property, deny modify')
    ...
    def diff_and_sync(self, print_only):
        ...
        for spg_type in self.types:
            ...
            if spg_type not in session._spg_types:
                ...
                if new_type.spg_type_enum != old_type.spg_type_enum or new_type.parent_type_name != old_type.parent_type_name:
                ...
                if new_type.spg_type_enum == SpgTypeEnum.Concept \
                    ...
                    old_type.relations[new_type.hypernym_predicate].alter_operation = AlterOperationEnum.Create
                    old_type.relations[old_type.hypernym_predicate].alter_operation = AlterOperationEnum.Delete
                    need_update = True
                    print(f'Recreate hypernym predicate: [{new_type.name}] {new_type.hypernym_predicate}')

                for prop in old_type.properties:
                    if not old_type.properties[prop].inherited and prop not in new_type.properties \
                            and not self.is_internal_property(prop, new_type.spg_type_enum):
                        assert prop != "subject" and old_type.properties[prop].property_group != PropertyGroupEnum.Subject,\
                            self.error_msg("The subject property of event type cannot be deleted")

                        old_type.properties[prop].alter_operation = AlterOperationEnum.Delete
                        need_update = True
                        print(f'Delete property: [{new_type.name}] {prop}')

                for prop, o in new_type.properties.items():
                    if prop not in old_type.properties and not self.is_internal_property(prop, new_type.spg_type_enum) and not o.inherited:
                        old_type.add_property(new_type.properties[prop])
                        need_update = True
                        print(f'Create property: [{new_type.name}] {prop}')

                    elif old_type.properties[prop].object_type_name != \
                            new_type.properties[prop].object_type_name:
                        assert not old_type.properties[prop].inherited, \
                            self.error_msg(f'{new_type.name}] {prop} is inherited property, deny modify')
                    ...
                    elif old_type.properties[prop].sub_properties != new_type.properties[prop].sub_properties:
                        need_update = self.diff_sub_property(new_type.properties[prop].sub_properties,
                                                             old_type.properties[prop].sub_properties,
                                                             old_type.name, old_type.properties[prop],
                                                             new_type.properties[prop])
                        if need_update:
                            old_type.properties[prop].alter_operation = AlterOperationEnum.Update
                ...
                for relation in new_type.relations:
                    ...
                    if relation not in old_type.relations or old_type.relations[relation].object_type_name != \
                    ...
                    elif old_type.relations[relation].sub_properties != new_type.relations[relation].sub_properties:
                        need_update = self.diff_sub_property(new_type.relations[relation].sub_properties,
                                                             old_type.relations[relation].sub_properties,
                                                             old_type.name, old_type.relations[relation],
                                                             new_type.relations[relation])
                        if need_update:
                            old_type.relations[relation].alter_operation = AlterOperationEnum.Update

                    elif old_type.relations[relation] != new_type.relations[relation]:
                        assert not old_type.relations[relation].inherited, \
                            self.error_msg(f'{new_type.name}] {p_name} is inherited relation, deny modify')

                        old_type.relations[relation].overwritten_by(new_type.relations[relation])
                        old_type.relations[relation].alter_operation = AlterOperationEnum.Update
                        need_update = True
                ...
                for relation, o in old_type.relations.items():
                    ...
                    if o.inherited or p_name in new_type.properties or o.is_dynamic:
                        ...
                        continue
                    if relation not in new_type.relations and not o.inherited and not o.is_dynamic and not (
                            new_type.spg_type_enum == SpgTypeEnum.Concept and p_name in ["isA", "locateAt"]):

                        old_type.relations[relation].alter_operation = AlterOperationEnum.Delete
                        need_update = True
    ...
    def export_schema_python(self, filename):
        ...
        inner = ""
        content = "# ATTENTION!\n"
        content += "# This file is generated by Schema automatically, it will be refreshed after schema has been committed\n"
