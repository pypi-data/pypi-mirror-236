...
from knext.core.builder.operator import KnowledgeExtractOp, Vertex, EvalResult
...
class DiseaseExtractor(KnowledgeExtractOp):
    ...
    def get_re_prompt(self) -> str:
        ...
        for k, v in self.spg_type.properties.items():
            if v.name in ["id", "description"]:
                continue
            spo = "\"subject\":\"{}\",\"predicate\":\"{}\",\"object\":\"{}\"".format(self.spg_type.name_zh, v.name_zh, v.object_type_name_zh)
            spo = "{" + spo + "}\n"
    ...
    def get_ner_prompt(self) -> str:
        ...
        获取NER的Prompt
        """
        prompt = prompt_template["ner"].replace("${schema}", f'[{self.spg_type.name}:{self.spg_type.name_zh}]')
        return prompt
    ...
    def infer(self, prompt, extract_input):
        ...
        print(response)
        parsed_json = self.chat(prompt_template["convert"], history=[(llm_input, response)])
        import re
        ...
        for spo_item in re_obj:
            ...
            if spo_type != "Text":
                ...
                if len(ner_object):
                    ...
                    if spo_en_name in subject_properties and len(subject_properties[spo_en_name]):
                        subject_properties[spo_en_name] = subject_properties[spo_en_name] + "," + ",".join(ner_values)
                    else:
                        subject_properties[spo_en_name] = ",".join(ner_values)
            else:
                if spo_en_name in subject_properties and len(subject_properties[spo_en_name]):
                    subject_properties[spo_en_name] = subject_properties[spo_en_name] + "," + spo_item["object"]
                else:
    ...
    def eval(self, record: Vertex) -> List[Vertex]:
        # try:
        #     content = record.get_property("content")
        #     output = self.infer(self.get_re_prompt(), content)
        #     vertexes = self.ner_to_vertex(output)
        #     return EvalResult(vertexes)
        # except Exception as e:
        #     print(e)
        #     if self.retry < 3:
        #         self.retry = self.retry + 1
        #         self.eval(record)
        sub_v = Vertex("1", "Medical.Disease", {"commonSymptom": "2", "applicableDrug": "3", "complication": "4"})
        obj_v2 = Vertex("2", "Medical.Symptom", {"name": "2"})
        obj_v3 = Vertex("3", "Medical.Drug", {"name": "3"})
        obj_v4 = Vertex("4", "Medical.Disease", {"name": "4"})
        vertexes = [sub_v, obj_v2, obj_v3, obj_v4]
        return vertexes
