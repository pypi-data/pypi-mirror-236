...
def submit_job(job_names):
    ...
    for job in job_list:
        ...
        click.secho(
            ...
            f" Use ` knext builder get --id {inst.building_job_inst_id} ` to check job status.", fg="bright_green"
        )
...
def get_job(id):
    ...
    if res and len(res) > 0:
        ...
        click.secho(f"ERROR: Cannot find BuilderJob instance. Please check if the [--id {id}] is correct.", fg="bright_red")
        sys.exit()

    if res[0].status in ["SUCCESS", "FAILURE"] and res[0].result and res[0].result.error_table_file:
        confirm = click.style(f"BuilderJob instance with id [{res[0].job_inst_id}] execution completed. "
                              f"Do you need to download error_table_file?", fg="bright_green")
        if click.confirm(confirm):
            ...
            file_path = os.path.join(os.environ["KNEXT_ROOT_PATH"], os.environ['KNEXT_BUILDER_RECORD_DIR'], error_table_file.split('/')[-1])
