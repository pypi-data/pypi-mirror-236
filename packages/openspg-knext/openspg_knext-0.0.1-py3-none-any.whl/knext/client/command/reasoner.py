...
#  is distributed on an "AS IS" BASIS, WIT`HOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
#  or implied.
...
def submit_reasoner_job(file, dsl):
    ...
    click.secho(
        ...
        f" Use ` knext reasoner get --id {inst.reasoner_job_inst_id} ` to check job status.", fg="bright_green"
    )
...
def query_reasoner_job(id):
    ...
    if res and len(res) > 0:
        ...
        click.secho(f"ERROR: Cannot find ReasonerJob instance. "
                    f"Please check if the [--id {id}] is correct.", fg="bright_red")
        sys.exit()

    if res[0].status in ["SUCCESS", "FAILURE"] and res[0].result and res[0].result.table_name:
        confirm = click.style(f"ReasonerJob instance with id [{res[0].job_inst_id}] execution completed. "
                              f"Do you need to download result_file?", fg="bright_green")
        if click.confirm(confirm):
            ...
            file_path = os.path.join(os.environ["KNEXT_ROOT_PATH"], os.environ['KNEXT_REASONER_RESULT_DIR'], table_name.split('/')[-1])
            if not Path(file_path).parent.exists():
                os.mkdir(Path(file_path).parent.resolve())
            with open(file_path, 'w') as file:
                file.write(content)
            click.secho(f"Download successful. The file path is [{file_path}].", fg="bright_green")
        else:
            sys.exit()
