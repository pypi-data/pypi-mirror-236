...
def _register_operators():
    builder_operator_path = os.path.join(os.environ["KNEXT_ROOT_PATH"], os.environ["KNEXT_BUILDER_OPERATOR_DIR"])
    if builder_operator_path in sys.path:
        return
    sys.path.append(builder_operator_path)
    import inspect

    for root, dirs, files in os.walk(builder_operator_path):
        for file_name in files:
            if file_name.endswith(".py"):
                ...
                for class_name, class_obj in classes:
                    if issubclass(class_obj, BaseOp) and inspect.getmodule(class_obj) == module:
                        name = (
                            class_obj.name if hasattr(class_obj, "name") else class_name
                        )
                        BaseOp.register(name=name, local_path=os.path.join(root, file_name))(class_obj)


class Operator(object):
    """
    Declare a local operator or platform operator to load into the pipeline.
    At the same time, some methods are provided to realize the registration of operators
    and the interaction between local operators and platform operators.
    Args:
        name: The unique identifier of the operator,
                usually declared in the annotation above the operator implementation.
        params: Parameters of operator.
    """
    ...
    def publish(self, op_name: str):
        """Publish a new operator version.
    ...
    def _generate_op_config(self, op_name: str, version: int = None, params: Dict[str, str] = None):
        overviews = self._client.operator_overview_get(op_name)
        if not overviews:
            raise ValueError(f"Operator [{op_name}] is not published."
        ...
        if version:
            ...
            for operator_version in operator_versions:
                if operator_version.version == version:
                    ...
                    break
            if not op:
                raise ValueError(f"Operator [{op_name}] with Version [{version}] is not published."
                                 f" Use ` knext operator publish {op_name} ` to publish this operator.")
        else:
