...
def dispatch_results_by_target_type(
    ...
    if not gl_params.get('issue') and not gl_params.get('mr'):
        _error('Cannot post results to GitLab: missing merge request or issue iid.')
...
def publish_results(
    ...
    if not all(gl_params.get(x) for x in MANDATORY_PARAMETERS):
        _error(
            'Cannot post results to GitLab: one of the mandatory parameters is missing or cannot be retrieved from the environment. Mandatory parameters: %s.',
            ', '.join(MANDATORY_PARAMETERS),
        )
        return
    ...
    try:
        ...
        msg = str(err)
        if token:
            msg = msg.replace(token['private_token'], '***')
