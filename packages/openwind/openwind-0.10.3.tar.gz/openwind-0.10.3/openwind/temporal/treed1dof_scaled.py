...
class TemporalReed1dofScaled(TemporalComponentExit):
    ...
    def one_step(self):
        ...
        theta = self.theta
        gamma, zeta, kappa, Qr, omegar, Kc, alpha_c, epsilon = self.reed1dof.get_dimensionless_values(self.t, self.Zc, self.rho)
        Pclosed = self.reed1dof.get_Pclosed(self.t) # necessary to rescale the quantities from/toward the pipe
        ...
        # coef from the pipe
        p_no_flow = self.pipe_end.get_p_no_flow()/Pclosed # the pressure from the pipe needs to be scaled
        A_lambda = self.Zc / self.pipe_end.get_alpha()  # alpha = dt /(2* m_end)
        ...
        # update values
        self.pipe_end.update_flow(flow_lambda*Pclosed/self.Zc) # in the pipe, the equations are not scaled
        self._this_y = next_y
        self._last_y = this_y
    ...
    def energy(self):
        ...
        gamma, zeta, kappa, Qr, omegar, Kc, alpha_c, epsilon = self.reed1dof.get_dimensionless_values(self.t, self.Zc, self.rho)
        Pclosed = self.reed1dof.get_Pclosed(self.t)
        ...
        # rescaling
        scaling_coef =  Pclosed**2*kappa/(omegar *self.Zc)
    ...
    def dissipated_last_step(self):
        ...
        return (spring_damping + bernoulli_dissip + NL_dissip + forcing_term) * self.dt*Pclosed**2/self.Zc
