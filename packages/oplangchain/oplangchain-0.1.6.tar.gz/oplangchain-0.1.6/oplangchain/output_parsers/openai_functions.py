...
class FunctionCallNotExecuted(Exception):
    def __init__(self, generation_text):
        self.generation_text = generation_text
        super().__init__(self.generation_text)

    def to_json(self):
        return json.dumps(
            {
                "error": "FunctionCallNotExecuted",
                "message": "The function call was not executed.",
...
class OutputFunctionsParser(BaseGenerationOutputParser[Any]):
    ...
    def parse_result(self, result: List[Generation]) -> Any:
        ...
        try:
            # if function_call in message.additional_kwargs then assign it to func_call else raise an error that contains generation.text
            if "function_call" in message.additional_kwargs:
                func_call = message.additional_kwargs["function_call"]
            else:
                raise OutputParserException(f"Could not parse function call")
