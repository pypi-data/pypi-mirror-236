...
def fit_estimator(
    ...
    X_train: np.ndarray,
    y_train: np.ndarray,
) -> EstimatorType | MemoryError | List[List[Union[EstimatorType, Exception, str]]]:
    """
...
def check_estimator_list(
    ...
    if estimator_list is None:
        ...
        if use_sklearn_estimators_aside_estimator_list:
            ...
            estimator_list = list(set(estimator_list) | set(sklearn_estimators))
...
def pretty_print_errors(returned_errors: List[List[Union[EstimatorType, Exception, str]]]) -> None:
    ...
    for est, err, traceback in returned_errors:
