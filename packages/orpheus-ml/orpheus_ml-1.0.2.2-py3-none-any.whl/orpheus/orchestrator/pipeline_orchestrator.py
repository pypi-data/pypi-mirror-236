...
from orpheus import ComponentService, PerformanceService, PipelineEvolverService, PreparatorService
...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        validation_size: float | int = 0.05,
        shuffle: bool = False,
        stratify: bool = False,
        ...
        categorical_features_pca_variance_threshold: Optional[float] = None,
        ...
        exclude_estimators: Optional[List[str]]
            The list of estimators to exclude. Does not need to match the name of the estimator exactly.
        ...
        categorical_features_pca_variance_threshold : float = None
            Variance threshold to be used for PCA on categorical features.
            Only active if categorical_features are provided.
        ...
        self.component_service = ComponentService(
            ...
            categorical_features_pca_variance_threshold=categorical_features_pca_variance_threshold,
        )
    ...
    def __repr__(self):
        return f"PipelineOrchestrator(metric={get_obj_name(self.metric)}, config_path={self.config_path}, verbose={logger.get_verbose()}, n_jobs={self.n_jobs}, test_size={self.test_size}, validation_size={self.validation_size}, random_state={self.random_state}, shuffle={self.shuffle}, stratify={self.stratify}, cv_obj={self.cv_obj})"
    ...
    def register_metric(
        ...
        SCORE_TYPES[modeling_type][optimization_direction].append(get_obj_name(metric))
    ...
    def build(
        ...
        if "base" in generations:
            self.pipelines["base"] = self.component_service.generate_pipeline_for_base_models(
                top_n_per_tuner=top_n_per_tuner_base, timeout_duration=timeout_duration
            )
    ...
    def _stress_test_pipeline(
        self,
        pipeline_name: str,
        pipeline_to_test: MultiEstimatorPipeline | None,
        threshold_score: Optional[float] = None,
        reg_trials: int = 5,
        clf_max_occurance_pct: float = 0.8,
        fraction_to_explain: float = 1.0,
            ...
            if fraction_to_explain:  # if more than 0.0
                try:
                    ...
                    explained_features = performance_service.get_explained_features(
                        fraction=fraction_to_explain, shuffle=shuffle_explaining, plot=plot_explaining
                    )
