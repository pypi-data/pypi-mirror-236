...
class GradeNotebooks():
    async def copySubDirNotebooksToParentDir(self, parent_dir):
    ...
    async def grade(self, p, notebooks):
        try:
            path = uh.handle_upload(notebooks)
            print(path)
            # zip_folder = notebooks_zip.split(".")[0]
            # with ZipFile(notebooks_zip, 'r') as zObject:
            #     zObject.extractall(path=zip_folder)

            # self.copySubDirNotebooksToParentDir(zip_folder)
