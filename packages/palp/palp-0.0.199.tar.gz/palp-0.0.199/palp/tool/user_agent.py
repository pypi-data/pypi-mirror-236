...
# 返回一个 user-agent
def random_ua(ua_type: str = None) -> str:
    ua_type = ua_type or settings.RANDOM_USERAGENT_TYPE

    if ua_type == COMPUTER:
        return random.choice(USER_AGENTS['computer'][random.choice(list(USER_AGENTS['computer'].keys()))])
    elif ua_type == CHROME:
        return random.choice(USER_AGENTS['computer']['chrome'])
    elif ua_type == OPERA:
        return random.choice(USER_AGENTS['computer']['opera'])
    elif ua_type == FIREFOX:
        return random.choice(USER_AGENTS['computer']['firefox'])
    elif ua_type == IE:
        return random.choice(USER_AGENTS['computer']['ie'])
    elif ua_type == SAFARI:
        return random.choice(USER_AGENTS['computer']['safari'])
    elif ua_type == MOBILE:
        return random.choice(USER_AGENTS['mobile'])
    else:
        ua_type_list = []
        ua_type_list.extend(list(USER_AGENTS.keys()))
        ua_type_list.extend(list(USER_AGENTS['computer'].keys()))
        raise ValueError(
            f"请输入正确的 UA 值类型，UA 值类型应该在该列表中 {ua_type_list}，输入类型：{ua_type}"
        )


if __name__ == '__main__':
    print(random_ua(ua_type=COMPUTER))
