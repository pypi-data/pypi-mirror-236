...
from numpy import count_nonzero, isnan, inf, linalg, arange, repeat, nan, mean
...
def __fit_fixed_p_model__(pfd, flevel, ro, bounds=False, sig_lims=None, method='trf', loss='soft_l1', f_scale=0.1, max_nfev=None, xtol=1e-9):
	...
	if (fo > fm) | (fo <= 0):
		...
		try:
			...
			return fo, fm, sigma, ro, bias, rmse, nrmse, fo_err, fm_err, sigma_err, snr_raw, nfl, nfev, flag, success
...
def __fit_calc_p_model__(pfd, flevel, bounds=False, sig_lims=None, ro_lims=None, method='trf', loss='soft_l1', f_scale=0.1, max_nfev=None, xtol=1e-9):
	...
	if (fo > fm) | (fo <= 0):
		...
		try:
			...
			return fo, fm, sigma, ro, bias, rmse, nrmse, fo_err, fm_err, sigma_err, ro_err, snr_raw, nfl, nfev, flag, status
		
		except linalg.LinAlgError as err:
			if str(err) == 'Singular matrix':
				print('Unable to calculate fitting errors, skipping sequence.'),
				fo, fm, sigma, ro, bias, rmse, nrmse, fo_err, fm_err, sigma_err, ro_err, snr_raw, nfl, nfev = repeat(nan, 13)
				flag = -3
				success = 'False'
...
def __fit_no_p_model__(pfd, flevel, ro=None, bounds=False, sig_lims=None, method='trf', loss='soft_l1', f_scale=0.1, max_nfev=None, xtol=1e-9):
	...
	if (fo > fm) | (fo <= 0):
		...
		try:
			...
			return fo, fm, sigma, bias, rmse, nrmse, fo_err, fm_err, sigma_err, snr_raw, nfl, nfev, flag, success
		
		except linalg.LinAlgError as err:
			if str(err) == 'Singular matrix':
				print('Unable to calculate fitting errors, skipping sequence.'),
				fo, fm, sigma, bias, rmse, nrmse, fo_err, fm_err, sigma_err, snr_raw, nfl, nfev = repeat(nan, 11)
				flag = -3
				success = 'False'
				return fo, fm, sigma, bias, rmse, fo_err, fm_err, sigma_err, snr_raw, nfl, nfev, flag, success
				pass
