...
def correct_fire_instrument_bias(df, pos=1, sat_len=100):
    ...
    # Replace fluorescence level in DataFrame with bias corrected data 
...
def calculate_blank_FastOcean(file_, seq_len=100, delimiter=','):
    ...
    df = read_csv(file_, skiprows=26, nrows=2, header=None, delimiter=delimiter, encoding='latin1')
    df = df.iloc[:,2:].T
    df.columns = ['date', 'time']
    df['datetime'] = to_datetime(df.date.values+' '+df.time.values)
    df = df.drop(columns=['date','time'])

    res = read_csv(file_, skiprows=43, nrows=seq_len, header=None, delimiter=delimiter, encoding='latin1')
    res = res.iloc[:,2:]
