...
if TYPE_CHECKING:
    from pingsafe_cli.psgraph.common.pingsafe.platform_integration import FileToPersist
    from pingsafe_cli.psgraph.common.typing import _SkippedCheck, _ScaSuppressions

CVE_COMMENT_PATTERN = re.compile(r"(pingsafe:skip=|pingsafe:skip=) *(CVE-[\-\d]+)(:[^\n]+)?")
PACKAGE_COMMENT_PATTERN = re.compile(r"(pingsafe:skip=|pingsafe:skip=) *([^\n]+)?")
