...
from pingsafe_cli.psgraph.common.util.parser_utils import find_var_blocks
import pingsafe_cli.psgraph.terraform.graph_builder.variable_rendering.renderer as renderer
from pingsafe_cli.psgraph.terraform.graph_builder.variable_rendering.safe_eval_functions import evaluate
...
PINGSAFE_RENDER_MAX_LEN = force_int(os.getenv("PINGSAFE_RENDER_MAX_LEN", "10000"))
...
def evaluate_terraform(input_str: Any, keep_interpolations: bool = True) -> Any:
    ...
    if isinstance(input_str, str) and PINGSAFE_RENDER_MAX_LEN and 0 < PINGSAFE_RENDER_MAX_LEN < len(input_str):
        logging.debug(f'Rendering was skipped for a {len(input_str)}-character-long string. If you wish to have it '
                      f'evaluated, please set the environment variable PINGSAFE_RENDER_MAX_LEN '
