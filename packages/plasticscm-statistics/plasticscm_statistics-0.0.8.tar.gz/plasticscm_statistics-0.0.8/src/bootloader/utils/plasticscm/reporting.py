...
def simplify_timestamp(timestamp: ISO8601DateTime) -> str:
    simplified_timestamp = \
        f'{timestamp.year:>04}-{timestamp.month:>02}-{timestamp.day:>02} ' \
        f'{timestamp.hour:>02}:{timestamp.minute:>02}:{timestamp.second:>02}'
    return simplified_timestamp
...
def get_changeset_history(
    ...
    if start_date or end_date:
        filters = []
        if start_date:
            filters.append(f"date >= '{simplify_timestamp(start_date)}'")
        if end_date:
            filters.append(f"date < '{simplify_timestamp(end_date)}'")
