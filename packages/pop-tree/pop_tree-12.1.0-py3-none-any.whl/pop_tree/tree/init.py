...
async def parse_base(hub, base, root: dict, ref: str, parts: list):
    ...
    if isinstance(base, pop.hub.Sub):
        ...
        hub.log.trace(f"Parsing a single function")
        func = base
        try:
            # IF the function is an instance of "Contracted", then get the underlying function
            func = func.func
        except AttributeError:
            ...
        func_doc = hub.tree.mod.format_func(func)
        func_doc["ref"] = ref
        ...
        raise TypeError(f"Unknown base: {base.__class__.__name__}")
