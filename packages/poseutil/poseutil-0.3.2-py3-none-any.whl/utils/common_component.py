...
def getHipCenter(pose):
    ...
    hipCenter = pose_measure.getHipPoint(X, AVG)
    return hipCenter

def getPoseHeight(pose):
    pose_measure = PoseMeasure(pose)
    nose_shoulder = pose_measure.getNoseShoulderDistance()
    shoulder_hip = pose_measure.getShoulderHipDistance()
    hip_knee = pose_measure.getHipKneeDistance()
    knee_ankle = pose_measure.getKneeAnkleDistance()
    return (nose_shoulder*2) + shoulder_hip + hip_knee + knee_ankle