...
def configure_dropout(model, config, parameters):
    ...
    model.config.hidden_dropout_prob = get("hidden_dropout_prob")
    model.config.attention_probs_dropout_prob = get("attention_probs_dropout_prob")
    model.config.classifier_dropout = get("classifier_dropout")

    listify = lambda parameter: parameter if type(parameter) == list else [parameter]
    if "dropout_type" in parameters.keys():

        id2label = config["variable_parameters"]["search_field"]["dropout_type"]["id2label"]
        choice = listify(id2label[get("dropout_type")])
        if "hidden" in choice:
            model.config.hidden_dropout_prob = get("dropout_prob")
        if "attention" in choice:
            model.config.attention_probs_dropout_prob = get("dropout_prob")
        if "classifier" in choice:
            model.config.classifier_dropout = get("dropout_prob")
...
def init_run(trial, config):
    ...
    return parameters, trial_checkpoint, trial_complete, training_args
