...
class Workspace(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if admin_portal is not None and not isinstance(admin_portal, WorkspaceAdminPortalArgs):
                admin_portal = admin_portal or {}
                def _setter(key, value):
                    admin_portal[key] = value
                WorkspaceAdminPortalArgs._configure(_setter, **admin_portal)
            ...
            if auth_policy is not None and not isinstance(auth_policy, WorkspaceAuthPolicyArgs):
                auth_policy = auth_policy or {}
                def _setter(key, value):
                    auth_policy[key] = value
                WorkspaceAuthPolicyArgs._configure(_setter, **auth_policy)
            ...
            if bulk_tenants_invites_email is not None and not isinstance(bulk_tenants_invites_email, WorkspaceBulkTenantsInvitesEmailArgs):
                bulk_tenants_invites_email = bulk_tenants_invites_email or {}
                def _setter(key, value):
                    bulk_tenants_invites_email[key] = value
                WorkspaceBulkTenantsInvitesEmailArgs._configure(_setter, **bulk_tenants_invites_email)
            __props__.__dict__["bulk_tenants_invites_email"] = bulk_tenants_invites_email
            if captcha_policy is not None and not isinstance(captcha_policy, WorkspaceCaptchaPolicyArgs):
                captcha_policy = captcha_policy or {}
                def _setter(key, value):
                    captcha_policy[key] = value
                WorkspaceCaptchaPolicyArgs._configure(_setter, **captcha_policy)
            ...
            if facebook_social_login is not None and not isinstance(facebook_social_login, WorkspaceFacebookSocialLoginArgs):
                facebook_social_login = facebook_social_login or {}
                def _setter(key, value):
                    facebook_social_login[key] = value
                WorkspaceFacebookSocialLoginArgs._configure(_setter, **facebook_social_login)
            ...
            if github_social_login is not None and not isinstance(github_social_login, WorkspaceGithubSocialLoginArgs):
                github_social_login = github_social_login or {}
                def _setter(key, value):
                    github_social_login[key] = value
                WorkspaceGithubSocialLoginArgs._configure(_setter, **github_social_login)
            __props__.__dict__["github_social_login"] = github_social_login
            if google_social_login is not None and not isinstance(google_social_login, WorkspaceGoogleSocialLoginArgs):
                google_social_login = google_social_login or {}
                def _setter(key, value):
                    google_social_login[key] = value
                WorkspaceGoogleSocialLoginArgs._configure(_setter, **google_social_login)
            __props__.__dict__["google_social_login"] = google_social_login
            if hosted_login is not None and not isinstance(hosted_login, WorkspaceHostedLoginArgs):
                hosted_login = hosted_login or {}
                def _setter(key, value):
                    hosted_login[key] = value
                WorkspaceHostedLoginArgs._configure(_setter, **hosted_login)
            __props__.__dict__["hosted_login"] = hosted_login
            if lockout_policy is not None and not isinstance(lockout_policy, WorkspaceLockoutPolicyArgs):
                lockout_policy = lockout_policy or {}
                def _setter(key, value):
                    lockout_policy[key] = value
                WorkspaceLockoutPolicyArgs._configure(_setter, **lockout_policy)
            __props__.__dict__["lockout_policy"] = lockout_policy
            if magic_code_email is not None and not isinstance(magic_code_email, WorkspaceMagicCodeEmailArgs):
                magic_code_email = magic_code_email or {}
                def _setter(key, value):
                    magic_code_email[key] = value
                WorkspaceMagicCodeEmailArgs._configure(_setter, **magic_code_email)
            __props__.__dict__["magic_code_email"] = magic_code_email
            if magic_link_email is not None and not isinstance(magic_link_email, WorkspaceMagicLinkEmailArgs):
                magic_link_email = magic_link_email or {}
                def _setter(key, value):
                    magic_link_email[key] = value
                WorkspaceMagicLinkEmailArgs._configure(_setter, **magic_link_email)
            __props__.__dict__["magic_link_email"] = magic_link_email
            if mfa_authentication_app is not None and not isinstance(mfa_authentication_app, WorkspaceMfaAuthenticationAppArgs):
                mfa_authentication_app = mfa_authentication_app or {}
                def _setter(key, value):
                    mfa_authentication_app[key] = value
                WorkspaceMfaAuthenticationAppArgs._configure(_setter, **mfa_authentication_app)
            __props__.__dict__["mfa_authentication_app"] = mfa_authentication_app
            if mfa_policy is not None and not isinstance(mfa_policy, WorkspaceMfaPolicyArgs):
                mfa_policy = mfa_policy or {}
                def _setter(key, value):
                    mfa_policy[key] = value
                WorkspaceMfaPolicyArgs._configure(_setter, **mfa_policy)
            ...
            if microsoft_social_login is not None and not isinstance(microsoft_social_login, WorkspaceMicrosoftSocialLoginArgs):
                microsoft_social_login = microsoft_social_login or {}
                def _setter(key, value):
                    microsoft_social_login[key] = value
                WorkspaceMicrosoftSocialLoginArgs._configure(_setter, **microsoft_social_login)
            __props__.__dict__["microsoft_social_login"] = microsoft_social_login
            __props__.__dict__["name"] = name
            if new_device_connected_email is not None and not isinstance(new_device_connected_email, WorkspaceNewDeviceConnectedEmailArgs):
                new_device_connected_email = new_device_connected_email or {}
                def _setter(key, value):
                    new_device_connected_email[key] = value
                WorkspaceNewDeviceConnectedEmailArgs._configure(_setter, **new_device_connected_email)
            __props__.__dict__["new_device_connected_email"] = new_device_connected_email
            if oidc is not None and not isinstance(oidc, WorkspaceOidcArgs):
                oidc = oidc or {}
                def _setter(key, value):
                    oidc[key] = value
                WorkspaceOidcArgs._configure(_setter, **oidc)
            ...
            if password_policy is not None and not isinstance(password_policy, WorkspacePasswordPolicyArgs):
                password_policy = password_policy or {}
                def _setter(key, value):
                    password_policy[key] = value
                WorkspacePasswordPolicyArgs._configure(_setter, **password_policy)
            if password_policy is None and not opts.urn:
                raise TypeError("Missing required property 'password_policy'")
            __props__.__dict__["password_policy"] = password_policy
            if pwned_password_email is not None and not isinstance(pwned_password_email, WorkspacePwnedPasswordEmailArgs):
                pwned_password_email = pwned_password_email or {}
                def _setter(key, value):
                    pwned_password_email[key] = value
                WorkspacePwnedPasswordEmailArgs._configure(_setter, **pwned_password_email)
            __props__.__dict__["pwned_password_email"] = pwned_password_email
            if reset_password_email is not None and not isinstance(reset_password_email, WorkspaceResetPasswordEmailArgs):
                reset_password_email = reset_password_email or {}
                def _setter(key, value):
                    reset_password_email[key] = value
                WorkspaceResetPasswordEmailArgs._configure(_setter, **reset_password_email)
            __props__.__dict__["reset_password_email"] = reset_password_email
            if reset_phone_number_email is not None and not isinstance(reset_phone_number_email, WorkspaceResetPhoneNumberEmailArgs):
                reset_phone_number_email = reset_phone_number_email or {}
                def _setter(key, value):
                    reset_phone_number_email[key] = value
                WorkspaceResetPhoneNumberEmailArgs._configure(_setter, **reset_phone_number_email)
            __props__.__dict__["reset_phone_number_email"] = reset_phone_number_email
            if saml is not None and not isinstance(saml, WorkspaceSamlArgs):
                saml = saml or {}
                def _setter(key, value):
                    saml[key] = value
                WorkspaceSamlArgs._configure(_setter, **saml)
            __props__.__dict__["saml"] = saml
            if user_activation_email is not None and not isinstance(user_activation_email, WorkspaceUserActivationEmailArgs):
                user_activation_email = user_activation_email or {}
                def _setter(key, value):
                    user_activation_email[key] = value
                WorkspaceUserActivationEmailArgs._configure(_setter, **user_activation_email)
            __props__.__dict__["user_activation_email"] = user_activation_email
            if user_invitation_email is not None and not isinstance(user_invitation_email, WorkspaceUserInvitationEmailArgs):
                user_invitation_email = user_invitation_email or {}
                def _setter(key, value):
                    user_invitation_email[key] = value
                WorkspaceUserInvitationEmailArgs._configure(_setter, **user_invitation_email)
            __props__.__dict__["user_invitation_email"] = user_invitation_email
            if user_used_invitation_email is not None and not isinstance(user_used_invitation_email, WorkspaceUserUsedInvitationEmailArgs):
                user_used_invitation_email = user_used_invitation_email or {}
                def _setter(key, value):
                    user_used_invitation_email[key] = value
                WorkspaceUserUsedInvitationEmailArgs._configure(_setter, **user_used_invitation_email)
