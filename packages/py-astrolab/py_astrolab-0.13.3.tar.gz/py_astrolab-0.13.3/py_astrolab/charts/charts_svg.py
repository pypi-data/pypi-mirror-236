...
class MakeSvgInstance:
    ...
    def __transitRing(self, r):
        ...
        outer_ring = '<circle class="transitRing" cx="%s" cy="%s" r="%s" style="fill: none; stroke: white; stroke-width: 35px; stroke-opacity: .8; filter: url(#%s);"/>' % (
            r, r, r, filter_id)
    ...
    def __makePlanetGrid(self):
        out = '<g transform="translate(-100,350)">'

        # Aggiungi un rettangolo nero come sfondo del box
        # out += '<rect x="-200" y="-200" width="1000" height="400" fill="black" style="opacity: .5"/>'

        # loop over all planets
        li = 150
        offset = 60

        # out += '<g transform="translate(65, -15)">'
        # out += f'<text text-anchor="end" style="fill:white; font-size: 14px;">{self.language_settings["planets_and_house"]} {self.name}:</text>'
        # out += '</g>'
        ...
        for i in range(len(self.planets_settings)):
            ...
            if self.planets_settings[i]['visible'] == 1:
                ...
                if self.planets_retrograde[i]:
                    out += f'<g class="retrograde" transform="translate(25,-6)"><use transform="scale(.6)" xlink:href="#retrogradeWhite" /></g>'
                out += '<g transform="translate(40,-8)"><use transform="scale(0.5)" xlink:href="#' + self.zodiac[self.planets_sign[i]]+'" /></g>'
                out += '<text text-anchor="start" x="60" style="fill:white; font-size: .8em; font-family: var(--bs-body-font-family);">%s</text>' % (self.__dec2deg(self.planets_degree[i]))
                out += '<g transform="translate(135,-8)"><use transform="scale(0.5)" xlink:href="#' + self.planets_settings[i]['name']+'White" /></g>'
                out += f'<text text-anchor="start" x="150" style="fill:white; font-size: .8em; font-family: var(--bs-body-font-family);">{self.language_settings["planets"][self.planets_settings[i]["label"]]}</text>'
                out += '</g>'
                li = li + 20

        if self.chart_type == "Transit" or self.chart_type == "Composite":
            # if self.chart_type == "Transit":
            #     out += '<g transform="translate(255, -15)">'
            #     out += f'<text text-anchor="end" style="fill:white; font-size: 14px;">{self.t_name}:</text>'
            # else:
            #     out += '<g transform="translate(370, -15)">'
            #     out += f'<text text-anchor="end" style="fill:white; font-size: 14px;">{self.language_settings["planets_and_house"]} {self.t_user.name}:</text>'
            # out += '</g>'
            ...
            for i in range(len(self.planets_settings)):
                ...
                if self.planets_settings[i]['visible'] == 1:
                    ...
                    out += f'<text text-anchor="end" style="fill:white; font-size: .8em;">{self.language_settings["planets"][self.planets_settings[i]["label"]]}</text>'
                    out += f'<g transform="translate(5,-8)"><use transform="scale(0.5)" xlink:href="#{self.planets_settings[i]["name"]}White" /></g>'
                    out += '<text text-anchor="start" x="25" style="fill:white; font-size: .8em;">%s</text>' % (self.__dec2deg(self.t_planets_degree[i]))
                    out += '<g transform="translate(95,-8)"><use transform="scale(0.5)" xlink:href="#' + self.zodiac[self.t_planets_sign[i]]+'" /></g>'
                    if self.t_planets_retrograde[i]:
                        out += '<g transform="translate(115,-6)"><use transform="scale(.6)" xlink:href="#retrogradeWhite" /></g>'
                    out += '</g>'
                    t_li = t_li + 20
    ...
    def makeTemplate(self):
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            td['transitRingAxes'] = self.__makeAxis(r, r-self.c1+54.5, self.t_user.axis_list, 9.7, 'black', transit_axis, composite_axis)
