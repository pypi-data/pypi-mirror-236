...
from hs.common import protobuf_utils
...
def notify_callback(pb_notify):
    """
    定义处理消息推送的callback
    :param pb_notify  参考 PBNotify.proto
    """
    print(f"trading notify_callback，payload：{json_format.MessageToJson(pb_notify)}")
...
class StockTradeDemo(object):
    ...
    def query_max_buying_power_and_qty(self, args: str = '') -> ModelResult:
    ...
    def cancel_oder(self, args: str = '') -> ModelResult:
        ...
        model_result = self._trading_api.cancel_order(order_id=param.get('order_id', ''),
                                                      ...
                                                      exchange_type=param.get('exchange_type', ''))
        if model_result.is_success:
            print(f"Cancel order success, order id：{model_result.get_model()}")
        else:
            print(f"fail to query, code: {model_result.code}, msg: {model_result.msg}")
        return model_result
    ...
    def query_today_order_list(self, args: str = '') -> ModelResult:
        ...
        model_result = self._trading_api.query_today_order_list(exchange_type=param.get('exchange_type', ''),
                                                                ...
                                                                offset=param.get("page_query_key", ''))
        if model_result.is_success:
    ...
    def add_notify_callback(self):
        """增加消息推送回调函数"""
        self._trading_api.add_notify_callback(notify_callback)
        return self
    ...
    def trade_login(self, trade_passwd: str):
        """交易解锁"""
        return self._trading_api.trading_login_with_result(trade_passwd)
...
class TradeInterfaceTestCmd(cmd.Cmd):
    ...
    def do_query_max_buying_power_and_qty(self, args: str = ''):
        """query_max_buying_power_and_qty {"exchange_type":"S", "portfolio":"138926816", "order_type":"3", "stock_code":"2222.SA", "order_price":"10", "order_id":"123"}"""
        if not args:
            print("Please enter your arguments or help <command> to query")
            return
        self.invoker_instance.query_max_buying_power_and_qty(args)
    ...
    def do_trade_login(self, trade_passwd: str):
        """trade_login <trade_passwd>"""
        model_result = self.invoker_instance.trade_login(trade_passwd)
        if model_result.is_success:
            print("Trade login success")
            return
        print(f"Trade login failed, message:{model_result.msg}")
