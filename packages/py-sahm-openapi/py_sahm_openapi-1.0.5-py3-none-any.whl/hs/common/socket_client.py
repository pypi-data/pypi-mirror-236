...
class SocketClient(object):
    ...
    def start(self, p_token, p_ip, p_port):
        ...
        for i in range(1):
            request_id, _ = self.trading_login(self._trading_passwd)
    ...
    def init_connection(self):
        ...
        try:
            ...
            request_msg_header.body_sha1 = rsa_utils.rsa_sign(init_connection_request.SerializeToString(),
                                                              self._rsa_private_key)
            # 把数据对象打包为字节对象
            ...
            if request_msg_header.msg_type == RequestMsgTypeEnum.REQUEST.value \
                ...
                # Socket登录初始化RSA
                encrypt_payload = rsa_utils.encrypt_data(init_connection_request.SerializeToString(),
    ...
    def trading_login(self, trade_passwd: str):
        # build payload
        temp_trade_passwd = self._trading_passwd
        if trade_passwd is not None:
            temp_trade_passwd = trade_passwd
        passwd = rsa_utils.encrypt_data(temp_trade_passwd, self._rsa_public_key)
    ...
    def trading_relogin(self):
        if not self.is_alive():
            return False
        # 执行平台登录获取新的token
        token = self._token_client.reconnect_get_token(self._get_server_key, self._login_country_code,
                                                       self._login_mobile)
        self._token = token
        ...
        if request_id is not None and request_id == '':
            ...
            trading_passwd_result = self.sync_get_result_direct(request_id,
                                                                timeout_in_seconds=SYNC_GET_RESULT_SECONDS_TIMEOUT)
            if trading_passwd_result is not None:
                trading_passwd_payload = parse_payload(trading_passwd_result)
                self._logging.debug(f"check trading password: {trading_passwd_payload}")
                ...
                if trading_passwd_payload and trading_passwd_payload.success:
                    self._last_keep_alive_seconds_int = now_to_int()
                    self._logging.info(
                        f"trading relogined success, heartbeat request self._last_keep_alive_seconds_int：{self._last_keep_alive_seconds_int}")
                    self.heartbeat(self._heartbeat_interval_sec)
                    return True
            else:
        ...
        def wait_notify(_socket_client_self):
            while True:
                ...
                if callable(_socket_client_self._handle_notify_callback):
                    ...
                    self._logging.error(
                        f"Notify callback is not callable：{_socket_client_self._handle_notify_callback}")
                    # break 不跳出循环，持续消费
        ...
        while len(self._readbuf) > 0:
            ...
            if pb_response.DESCRIPTOR.name == PBNotify.DESCRIPTOR.name:
                ...
                if pb_response.payload.Is(InitConnectResp.DESCRIPTOR):
                    ...
                    self._logging.info(
                        f"received init connection message, update set encrypted_key, init connection has done.")
                # 接口响应结果
