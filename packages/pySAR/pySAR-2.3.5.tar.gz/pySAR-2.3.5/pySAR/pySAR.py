...
class PySAR():
    ...
    def encode_aai_descriptor(self, aai_indices=None, descriptors=None, show_plot=False, print_results=True, output_folder=""):
        ...
        for index in self.aai_indices:
            aai_encoding = pd.concat([aai_encoding, pd.DataFrame(self.get_aai_encoding(index))], axis=1)
        ...
        X = pd.concat([aai_encoding, descriptor_encoding_df], axis=1)
        ...
        aai_desc_df.loc[0] = [aai_indices, str(index_cat).strip(), ', '.join(self.descriptors), str(desc_group), eval.r2, 
    ...
    def output_results(self, results):
        ...
        print('# Algorithm: {}\n# Model Parameters: {}\n# Test Split: {}'.format(
            repr(self.model), self.model.model.get_params(), self.test_split))
        print('\n#############################################################')
        print('######################## Results ############################\n')
