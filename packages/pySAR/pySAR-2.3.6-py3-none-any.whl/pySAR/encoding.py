...
class Encoding(PySAR):
    ...
    aai_descriptor_encoding(aai_list=None, descriptors=None, desc_combo=1, sort_by='R2', output_folder=""):
    ...
    def aai_encoding(self, aai_list=None, sort_by='R2', output_folder=""):
        ...
        elif (isinstance(aai_list, str)):   #if single aai index input, cast to list
            if (',' in aai_list):
                all_indices = aai_list.replace(' ', '').split(',')
            else:
        ...
        print('# Dataset: {}\n# Number of Sequences/Sequence Length: {} x {}\n# Target Activity: {}\n# Algorithm: {}\n# Test Split: {}'.
              format(os.path.basename(self.dataset), self.num_seqs, self.sequence_length, self.activity_col, repr(self.model), self.test_split))
        if (self.model_parameters == "" or self.model_parameters is None or self.model_parameters == {}):
            print('# Model Parameters: {}'.format("\n\t".join(tw.wrap(', '.join(self.model.model.get_params())))))        
        else:
        ...
        for index in tqdm(all_indices[:int(len(all_indices))], unit=" indices", position=0, 
            desc="AAI Indices", disable=tqdm_disable, ncols=87):
    ...
    def descriptor_encoding(self, descriptors=None, desc_combo=1, sort_by='R2', output_folder=""):
        ...
        print('# Dataset: {}\n# Number of Sequences/Sequence Length: {} x {}\n# Target Activity: {}\n# Algorithm: {}\n# Test Split: {}'.format(
            os.path.basename(self.dataset), len(self.data), self.data[self.sequence_col].str.len().max(), self.activity_col, repr(self.model), self.test_split))
        if (self.model_parameters == "" or self.model_parameters is None or self.model_parameters == {}):
            print('# Model Parameters: {}'.format("\n\t".join(tw.wrap(', '.join(self.model.model.get_params())))))        
        else:
            print('# Model Parameters: {}'.format("\n\t".join(tw.wrap(', '.join(self.model_parameters)))))       
        ...
        for descr in tqdm(all_descriptors[:int(len(all_descriptors))], unit=" descriptor", position=0, 
            desc="Descriptors", mininterval=30, disable=tqdm_disable, ncols=87):
        ...
        print('\n####################################################')
        print('Elapsed Time for Descriptor Encoding: {0:.2f} seconds.\n'.format(elapsed))
    ...
    def aai_descriptor_encoding(self, aai_list=None, descriptors=None, desc_combo=1, sort_by='R2', output_folder=""):
        """
        ...
        for index in tqdm(all_indices[:int(len(all_indices))], unit=" indices", desc="AAI Indices", disable=tqdm_disable, ncols=87):
            ...
            for descr in tqdm(all_descriptors, leave=False, unit=" descriptor", desc="Descriptors", disable=tqdm_disable, ncols=87):
