...
class PySAR():
    ...
    :**kwargs : dict
        keyword arguments of the same name as in the configuration files. The value 
        of the kwarg arguments take precedence if they are input.
    ...
    def encode_aai_descriptor(self, aai_indices=None, descriptors=None, show_plot=False, print_results=True, output_folder=""):
        ...
        #if multiple descriptors input as str, split into comma seperated list
        if isinstance(self.descriptors , str):
            self.descriptors = self.descriptors.split(',')
        ...
        if (isinstance(aai_indices, str)):   #if single descriptor input, cast to list
            if (',' in aai_indices):
                self.aai_indices = aai_indices.replace(' ', '').split(',')
            else:
                self.aai_indices = [aai_indices]
        ...
        aai_desc_df.loc[0] = [', '.join(aai_indices), str(index_cat).strip(), ', '.join(self.descriptors), str(desc_group), eval.r2, 
    ...
    def output_results(self, results):
        ...
        print('# Dataset: {}\n# Number of Sequences/Sequence Length: {} x {} \
            \n# Target Activity: {}'.format(self.dataset, self.num_seqs, self.sequence_length, self.activity_col))
        # if (self.aai_indices is not None and len(self.aai_indices) <= 10):
        if (self.aai_indices is not None and len(self.aai_indices) <= 10):
            print('# AAI Indices: {}'.format("\n\t".join(tw.wrap(', '.join(self.aai_indices)))))
            if (self.use_dsp):
                print('# DSP Parameters:\n  # Spectrum: {}\n  # Window Function: {} \
                \n  # Filter Function: {}'.format(self.spectrum, self.window_type, self.filter_type))  
        if (self.descriptors is not None):
            print('# Descriptors: {}'.format("\n\t".join(tw.wrap(', '.join(self.descriptors)))))  
        print('# Algorithm: {}\n# Model Parameters: {}\n# Test Split: {}\n# Feature Space: {}'.format(
            repr(self.model), self.model.model.get_params(), self.test_split, self.feature_space))
        print('\n#############################################################')
        print('######################## Results ############################\n')
