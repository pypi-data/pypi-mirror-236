...
class AstroWeather:
    ...
    async def retrieve_data_uptonight(self):
        ...
        if os.path.isfile(os.getcwd() + "/www/uptonight-report.json"):
            _LOGGER.debug(f"Uptonight report found")
            with open(os.getcwd() + "/www/uptonight-report.json") as json_file:
                dataseries = json.load(json_file)
                _LOGGER.debug(f"Uptonight imported")
        else:
            _LOGGER.debug(f"uptonight-report.json not found. Current path: {os.getcwd()}")
