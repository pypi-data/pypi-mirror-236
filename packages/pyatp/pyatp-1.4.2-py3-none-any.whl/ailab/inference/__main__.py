...
def chatglm_pipeline(args):
    ...
    if args.full_finetuned_model_path is not None:
        model,tokenizer = load_chatglm_model_tokenizer(args.full_finetuned_model_path,args.full_finetuned_model_path)

    if args.fintuned_weights is not None:
...
def chinese_llama_alpaca_pipeline(args):
    if args.full_finetuned_model_path is not None:
        raise SystemExit(f'chinese_llama_alpaca not support full yeat')
...
def LLaMA_Efficient_Tuning_pipeline(args):
    ...
    if args.full_finetuned_model_path is not None:
        model,tokenizer = load_efficient_model_tokenizer(args.full_finetuned_model_path,args.full_finetuned_model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--full_finetuned_model_path", type=str, default='None')
    ...
    if args.fintuned_weights == 'None':
        args.fintuned_weights = None
    if args.full_finetuned_model_path == 'None':
        args.full_finetuned_model_path = None
        
    if (args.fintuned_weights is None) != (args.full_finetuned_model_path is None):
        print('check lora or full')
    else:
        raise SystemExit(f'should only lora or full')
    ...
    elif args.full_finetuned_model_path is not None and not os.path.exists(args.full_finetuned_model_path):
        raise SystemExit(f'args.full_finetuned_model_path {args.full_finetuned_model_path} not exist')
