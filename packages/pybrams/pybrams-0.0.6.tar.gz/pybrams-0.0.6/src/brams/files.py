...
class File:
    ...
    def __init__(self, year, month, day, hours, minutes, sample_rate, pps_count, duration, precise_start, precise_end, system_code, location_code, location_url, system_url, wav_url, wav_name, png_url, png_name, corrected_wav_name=None, beacon_frequency=None):
        ...
        self.corrected_wav_name: str | None = corrected_wav_name
        self.system: systems.System | None = None
        ...
        self.signal: Signal = Signal(beacon_frequency)
    ...
    def __json__(self):
        ...
        return {
            ...
            "corrected_wav_name": self.corrected_wav_name,
            "beacon_frequency": self.signal.beacon_frequency
        }
    ...
    def load(self):
        ...
        if not corrected_wav_content:
            ...
            self.signal.set(self.wav.data.signal_short, self.wav.header.sample_rate)
            self.signal.process(clean=True)

            self.pps = PPS(self.wav.pps.index, self.wav.pps.time)
            self.corrected_pps = self.pps.to_corrected_PPS(self.type)

            self.wav.data.signal_short = self.signal.signal_short
            ...
            self.corrected_pps = PPS(self.wav.pps.index, self.wav.pps.time)
...
def get(start: str, system: systems.System | str | list[systems.System] | list[str] | Dict[str, systems.System] | None = None) -> File | Dict[str, File]:
    ...
    if isinstance(system, (str, systems.System)):
        ...
        cached_file = Cache.get(f"{system_code}.{year}{month}{day}_{hours}{minutes}")
        ...
        system_codes = [system.system_code for system in systems.all().values()]
