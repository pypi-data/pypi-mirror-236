...
class HiModule:
    ...
    def readAllTagFilesIn(self):
        f = open(self.path_file,  'rb')
        self.readAllTagFiles(f)
        f.close()


    def readAllTagFiles(self, f):
        for i in range(self.moduleHeader.ResourceIndex):
            self.readFileEntry(f,i)

    def readFileEntryIn(self, index=-1) -> HiModuleFileEntry:
        f = open(self.path_file,  'rb')
        result = self.readFileEntry(f,index)
        f.close()
        return result
    
    def readFileEntry(self, f, index=-1) -> HiModuleFileEntry:
