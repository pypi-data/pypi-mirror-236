...
################## Ban Status 🚫 ##################
    ...
    async def account_add(self, user_id, app_id, app_hash, session):
        await self.get_user(user_id)
        account = dict(
            app_id=app_id,
            app_hash=app_hash,
        ...
        self.cache[user_id]["account"] = account
        await self.col.update_one(
            {"id": user_id}, {"$set": {"account": account}}
        )
    async def get_account(self, id):
        ...
        user = await self.get_user(id)
        return user.get("account", default)

    async def get_all_account(self):
        account = self.col.find({"account"})
        return account
db_access = Database()
