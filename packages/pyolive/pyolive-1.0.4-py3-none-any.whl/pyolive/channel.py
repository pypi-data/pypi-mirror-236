...
class Channel(threading.Thread):
    def __init__(self, logger, namespace, alias, devel=False):
        super().__init__()
        self.daemon = True
        self.que = Queue()
    ...
    def publish_heartbeat(self, worker_name):
        ...
        data['timestamp'] = 0
        routing_key = 'sys.' + self.namespace + '.heartbeat'
    ...
    def publish_job(self, context:JobContext):
        ...
        if context.timestamp == 0:
            routing_key = 'job.' + self.namespace + '.early.' + context.topic
        else:
            routing_key = 'job.' + self.namespace + '.now.' + context.topic
