...
def lr_finder(net,loss_function,training_loader,savepath=None,optimizer_type="sgd",base_lr=1e-7,max_lr=10,num_iter=100,batch_size=4):
    n = 0
    learning_rate = []
    losses = []
    optimizer = get_optimizer(net, optimizer_type, base_lr)
    lr_scheduler = FindLR(optimizer, max_lr=max_lr, num_iter=num_iter)
    epoches = int(num_iter / len(training_loader)) + 1
    ...
    if savepath is None:
        savepath = "result.jpg"
    savelr_finder(savepath, learning_rate=learning_rate, losses=losses)

def savelr_finder(path='result.jpg', learning_rate=None, losses=None):
