...
import qgate_graph.file_format as cns
import qgate_graph
    ...
    def generate_from_file(self, input_file: str, output_dir: str="output"):
        ...
        with open(input_file, "r") as f:
            while True:
                ...
                if input_dict[cns.FileFormat.PRF_TYPE]==cns.FileFormat.PRF_HDR_TYPE:
                    # header
                    report_date=datetime.datetime.fromisoformat(input_dict[cns.FileFormat.PRF_HDR_NOW]).strftime("%Y-%m-%d %H-%M-%S")
                    label=input_dict[cns.FileFormat.PRF_HDR_LABEL]
                    bulk=input_dict[cns.FileFormat.PRF_HDR_BULK]
                    file_name=self._unife_file_name(f"{label}-{report_date}-bulk-{bulk[0]}x{bulk[1]}")
                    title=f"'{label}', {report_date}, bulk {bulk[0]}/{bulk[1]}"

                elif input_dict[cns.FileFormat.PRF_TYPE]==cns.FileFormat.PRF_CORE_TYPE:
                    # core
                    if input_dict[cns.FileFormat.PRF_CORE_GROUP] in executors:
                        executors[input_dict[cns.FileFormat.PRF_CORE_GROUP]].append(input_dict[cns.FileFormat.PRF_CORE_REAL_EXECUTOR])
                        total_performance[input_dict[cns.FileFormat.PRF_CORE_GROUP]].append(input_dict[cns.FileFormat.PRF_CORE_TOTAL_CALL_PER_SEC])
                        avrg_time[input_dict[cns.FileFormat.PRF_CORE_GROUP]].append(input_dict[cns.FileFormat.PRF_CORE_AVRG_TIME])
                        std_deviation[input_dict[cns.FileFormat.PRF_CORE_GROUP]].append(input_dict[cns.FileFormat.PRF_CORE_STD_DEVIATION])
                    else:
                        executors[input_dict[cns.FileFormat.PRF_CORE_GROUP]] = [input_dict[cns.FileFormat.PRF_CORE_REAL_EXECUTOR]]
                        total_performance[input_dict[cns.FileFormat.PRF_CORE_GROUP]] = [input_dict[cns.FileFormat.PRF_CORE_TOTAL_CALL_PER_SEC]]
                        avrg_time[input_dict[cns.FileFormat.PRF_CORE_GROUP]] = [input_dict[cns.FileFormat.PRF_CORE_AVRG_TIME]]
                        std_deviation[input_dict[cns.FileFormat.PRF_CORE_GROUP]]=[input_dict[cns.FileFormat.PRF_CORE_STD_DEVIATION]]
