...
def hamilton2quafu(i: list, reverse=True):
    ...
    # 2. 拼接 ansatz
    # qiskit_meas_circuits = [ansatz.compose(circ).measure_all(inplace=False) for circ in meas_circuits]
...
def quafuVQE(H_op, ansatz, api_token, params, random_param=False, backend = "ScQ-P10", cpPath='checkpoint.txt', logJson='log.json'):
    # Initialize Quafu
