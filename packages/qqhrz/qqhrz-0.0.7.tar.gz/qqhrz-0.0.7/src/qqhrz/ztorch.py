...
if __name__ == '__main__':
    ...
    # all_sentences = [sentence for paragraph in paragraphs
    #                  for sentence in paragraph]
    # vocab = d2l.Vocab(all_sentences, min_freq=5, reserved_tokens=[
    #     '<pad>', '<mask>', '<cls>', '<sep>'])
    # dataset = WikiTextDataset(vocab, all_sentences[:2000], 64)
    # batch_size = 512
    # data_iter = DataLoader(dataset, batch_size)
    # net = BERTModoul(len(vocab), 100, 3, 2, [100], 200, dropout=0.1)
    # loss_function = BERTLoss()
    # for batch in data_iter:
    #     print([data.shape for data in batch])
    #     _, mlm_Y_hat, nsp_Y_hat = net(batch[0], batch[1], batch[3], batch[2])
    #     print(loss_function(nsp_Y_hat, batch[5], mlm_Y_hat, batch[6], batch[4]))
    #     break
    # print([data.shape for data in get_bert_input(vocab, all_sentences[:10], 64)])
    # print(sentences)
    # vocab = d2l.Vocab([['i', 'like', 'it'], ['you', 'are', 'a', 'good', 'person']], reserved_tokens=['<pad>', '<cls>', '<seq>'])
    # print(vocab.token_to_idx)
    # print(get_maskLM([1, 3, 4, 5, 2, 6, 7, 8, 9, 10, 2], vocab))
    pass
    # ['i', 'like', 'it'], ['you', 'are', 'a', 'good', 'person']
    # [1, 3, 5, 6, 2], [4, 7, 8, 11, 15, 2]
    # tokens = torch.tensor([[1, 3, 5, 6, 2], [4, 7, 8, 11, 15]], dtype=torch.long)
    # segments = torch.tensor([[0, 0, 0, 1, 1], [0, 0, 1, 1, 1]], dtype=torch.long)
    # vl = torch.tensor([4, 3])
    # predict_positions = torch.tensor([[3, 2], [2, 3]])
    # bm = BERTModoul(20, 100, 3, 2, [100], 200, 0.1)
    # print([b.shape for b in bm(tokens, segments, predict_positions, vl)])
    # x = torch.randn(2, 4, 3)
    # predict_positions = torch.tensor([[1, 3, 2], [2, 0, 1]])
    # print(x)
    # print(predict_positions)
    # mlm = MaskLM(10, 3, 12)
    # print(mlm(x, predict_positions).shape)
    # print(get_tokens_and_segments(['i', 'like', 'it']))
    # print(get_tokens_and_segments(['i', 'like', 'it'], ['you', 'are', 'a', 'good', 'person']))
