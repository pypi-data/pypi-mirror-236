...
from quadratic_programs.exceptions import QPError
...
class QuadraticProgram:
    ...
    def _add_variables(
        ...
        if isinstance(keys, int) and keys < 1:
            raise QPError(f"Cannot create non-positive number of variables: {keys}")
        if not name:
            name = "x"
        if "{{}}" in key_format:
            raise QPError(
        ...
        def _find_name(name, key_format, k):
            ...
            while True:
                ...
                if new_name == prev:
                    raise QPError(f"Variable name already exists: {new_name}")
        ...
        for key in lst:
            ...
            if indexed_name in self._variables_index:
                raise QPError(f"Variable name already exists: {indexed_name}")
    ...
    def _var_dict(
        ...
        Raises:
            QPError: if the variable name is already taken.
            QPError: if less than one variable instantiation is attempted.
            QPError: if `key_format` has more than one substitution or a
                                     nested substitution.
        """
        return dict(
    ...
    def continuous_var(
        ...
        Raises:
            QPError: if the variable name is already occupied.
        """
    ...
    def linear_constraint(
        ...
        Raises:
            QPError: if the constraint name already exists or the sense is not
                valid.
        """
        if name:
            if name in self.linear_constraints_index:
                raise QPError(f"Linear constraint's name already exists: {name}")
    ...
    def quadratic_constraint(
        ...
        Raises:
            QPError: if the constraint name already exists.
        """
        if name:
            if name in self.quadratic_constraints_index:
                raise QPError(f"Quadratic constraint name already exists: {name}")
    ...
    def substitute_variables(
        ...
        Raises:
            QPError: if the substitution is invalid as follows.
    ...
    def get_feasibility_info(
        ...
        Raises:
            QPError: If the input `x` is not same len as total vars
        """
    ...
    def prettyprint(self, wrap: int = 80) -> str:
        ...
        Raises:
            QPError: if there is a non-printable name.
        """
        # pylint: disable=cyclic-import
