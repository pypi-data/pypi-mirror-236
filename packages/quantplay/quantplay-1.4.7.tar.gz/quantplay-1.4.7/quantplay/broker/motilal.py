...
class Motilal(Broker):
    ...
    def cancel_order(self, unique_order_id):
        ...
        Constants.logger.info(f"[CANCEL_ORDER] {unique_order_id}")
        response = requests.post(
        ...
        if "errorcode" in response and response["errorcode"] in ["MO1066"]:
            ...
            raise RetryableException(
                f"[ORDER_CANCELLATION_FAILED] {unique_order_id} due to network error"
            )
        elif "errorcode" in response and response["errorcode"] in ["16273"]:
            Constants.logger.info(
                f"[CANCEL_ORDER_RESPONSE] [{unique_order_id}] record doesn't exist"
            )
            return
