...
try:
    ...
    if 37 <= version < 42:
        ...
        raise GAMSVersionError(
            'The REMix Python API is not compatible with GAMS versions below 37, \
        ...
        try:
            ...
            raise GAMSVersionError(
                "It seems that you are using a GAMS version higher than 41, \
                please install the API as instructed in \
                https://www.gams.com/latest/docs/API_PY_GETTING_STARTED.html#PY_PIP_INSTALL_BDIST"
            )
...
def read_gdx_symbol(gdx_path: Union[str, dict], gdx_symbol: str, sel_dict=None) -> pd.DataFrame:
...
def fill_missing_values_in_timeseries(df: pd.DataFrame, timemodel: pd.DataFrame) -> pd.DataFrame:
    ...
    mi = pd.MultiIndex.from_product(
        [timemodel.index, df.index.droplevel(lvl_time).unique().to_list()]
    )
...
class GDXEval:
    ...
    def get_symbol(self, key: str, sel_dict=None) -> pd.DataFrame:
        ...
        if (
            self._fill_ts
            and "timeModel" in df.index.names
            and key not in ["timeModel", "timeModelToCalc"]
        ):
            if "timeModelToCalc" in self._symbols:
                df = fill_missing_values_in_timeseries(df, self["timeModelToCalc"])
            elif "timeModel" in self._symbols:
                df = fill_missing_values_in_timeseries(df, self["timeModel"])
            else:
    ...
    def to_xlsx(self, filename="results.xlsx"):
        ...
        with pd.ExcelWriter(filename, engine="openpyxl") as writer:
            for i in symbols_without_time:
                if i in self._symbols:
                    self[i].to_excel(writer, sheet_name=i, merge_cells=False)
