...
class PolarsDataset(DirectoryDataset):
        ...
        if len(df_list) > 0:
            ...
            if len(value_columns) != 0:
                if "t0001" not in df_out.columns:
                    df_out.groupby(by=index_columns).agg(value_columns)
                else:
    ...
    def write_map_file(self, data, parent, name, extension, **kwargs):
        ...
        if map_format == "dat":
            ...
            data.columns = [""]
        data.write_csv(join(parent, filename), has_header=True)
        ...
        if "map_" in _input:
            ...
            if not (set([id.lower() for id in index_columns]) == set([id.lower() for id in dataframe.columns])):
                ...
                dataframe = dataframe.with_columns([
                    pl.col('map'),
                    *[pl.col('map').apply(lambda s, i=i: s.split(' . ')[i]).alias(col_name)
