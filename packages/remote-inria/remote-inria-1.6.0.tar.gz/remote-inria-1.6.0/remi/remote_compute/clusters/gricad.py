...
def _sync_container_from_inria() -> None:
    ...
    print_info(
        to_bold('Syncing container image ') + '`' + colored(inria_sif_name, 'yellow') + '`'
            + to_bold(' to GRICAD: ') + '`' + colored(path_to_sif_gricad, 'yellow') + '`'
    )
    ...
    if return_code == 0:
        ...
        subprocess.run(
            ['cp', '-f', last_built_recipe_path, last_synced_recipe_path],
            check=True
        )
    else:
