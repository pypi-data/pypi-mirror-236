...
class Container(BaseContainer):
    ...
    def start_pipeline(project:str, name:str, pl_conf:BaseModel, log:bool=False, store:bool=True, overwrite:bool=False):
        ...
        PL_CLAZZ = PIPELINE[pl_conf._clazz]
        pipeline = PL_CLAZZ(pl_conf, project, name)
        pipeline.start()
    ...
    def init_wandb_logger(project:str, name:Dict[str, Any], config:Dict[str, Any]=None):
        if Container.global_config.log:
            wandb.init(project=project, name=name, settings=wandb.Settings(start_method="fork"), config=config)
