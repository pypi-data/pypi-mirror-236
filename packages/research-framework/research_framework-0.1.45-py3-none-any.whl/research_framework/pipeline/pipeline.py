...
class FitPredictPipeline:
    def __init__(self, doc:PipelineModel, project:str, name:str):
        print("\n* Pipeline: ")
        ...
        for f in self.pipeline.filters:
            for k,v in f.params.items():
                Container.send_to_logger(message={ f'{filter.clazz}.{k}': str(v)})
        ...
        try:
            ...
            for combi in combis:
                ...
                pipeline_model = PipelineModel(
                    ...
                    filters=list(map(lambda clazz, params: FilterModel(
                        clazz=clazz, 
                        params=params, 
                        overwrite=filter_map[clazz].overwrite), combi.items()
                    )),
                ...
                pipeline = FitPredictPipeline(pipeline_model)
                pipeline.start()
