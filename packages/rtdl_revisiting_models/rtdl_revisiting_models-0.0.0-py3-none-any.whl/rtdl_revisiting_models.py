    ...
    **Examples**
    ...
    def __init__(
        ...
        if any(x <= 0 for x in cardinalities):
            i, value = next(iter(enumerate(cardinalities)))
            raise ValueError(
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.embeddings(x + self._category_offsets)
        if self.bias is not None:
            x = x + self.bias
        return x
    ...
    **Examples**
    ...
    def __init__(
        ...
        *,
        ...
        ffn_d_hidden_multiplier: Optional[float] = None,
        ffn_dropout: float,
        # NOTE: DIFF
        # In the paper, FT-Transformer uses the ReGLU actvation. However, as mentioned
        # in the appendix of the paper, there is no significant difference in
        # performance between ReGLU and ReLU.
        # So here, ReLU is used by default because of its simplicity.
        ffn_use_reglu: bool = False,
        ...
        Args:
            ...
            ffn_use_reglu: if True, the ReGLU activation is used in feed-forward blocks
                instead of ReLU.
                ...
                nn.ModuleDict(
                    {
                        ...
                        'ffn': delu.nn.named_sequential(
                            (
                                'linear1',
                                # ReGLU divides dimension by 2,
                                # so multiplying by 2 to compensate fot this
                                nn.Linear(
...
class FTTransformer(nn.Module):
    ...
    def get_default_kwargs(
        cls, n_blocks: int = 3, ffn_use_reglu: bool = False
    ) -> Dict[str, Any]:
        """Get the default hyperparameters.

        Args:
            n_blocks: the number of blocks. The supported values are: 1, 2, 3, 4, 5, 6.
            ffn_use_reglu: if `True`, the ReGLU activation will be used in
                the feed-forward blocks instead of ReLU.
        Returns:
        ...
        return {
            ...
            'ffn_d_hidden': None,
            'ffn_d_hidden_multiplier': 4 / 3 if ffn_use_reglu else 2.0,
