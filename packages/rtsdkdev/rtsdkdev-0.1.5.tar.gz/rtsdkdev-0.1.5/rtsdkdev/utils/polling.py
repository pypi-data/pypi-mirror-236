...
from .types import AgentServerStatus
...
async def loop_with_backoff(
    ...
    logger: Logger,
    fn: Callable[[], Awaitable[AgentServerStatus]],
) -> AgentServerStatus:
    ...
    while True:
        try:
            ...
            logger.error(f"Error running RPC agent: {err}")
            sleep(delay_time_ms)
            delay_time_ms *= 2
            delay_time_ms = min(delay_time_ms, CONNECTION_ERROR_RETRY_MAX_MS)
