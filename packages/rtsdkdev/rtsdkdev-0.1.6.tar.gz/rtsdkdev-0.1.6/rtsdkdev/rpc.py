...
from rtsdkdev.utils.api import RetoolAPI
from rtsdkdev.utils.errors import FunctionNotFoundError, create_agent_server_error
from rtsdkdev.utils.helpers import is_client_error
from rtsdkdev.utils.logger import Logger
from rtsdkdev.utils.polling import loop_with_backoff
from rtsdkdev.utils.schema import parse_function_arguments
from rtsdkdev.utils.types import (
    AgentServerError,
    AgentServerStatus,
...
from rtsdkdev.version import __version__
...
class RetoolRPC:
    ...
    async def execute_function(
        ...
        fn_spec = self._functions.get(function_name)
        if not fn_spec:
            raise FunctionNotFoundError(function_name)

        parsed_arguments = parse_function_arguments(
            function_arguments, fn_spec["arguments"]
        )
        self._logger.debug("Parsed arguments: ", parsed_arguments)

        if asyncio.iscoroutinefunction(fn_spec["implementation"]):
            result = await fn_spec["implementation"](parsed_arguments, context)
        else:
            result = fn_spec["implementation"](parsed_arguments, context)
