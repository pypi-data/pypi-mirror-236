...
describe('GetColumnConfig - cellRenderer', () => {
  ...
  it('should correctly render the "status" cell', () => {
    const columnConfig = GetColumnConfig();
  ...
  it('should correctly render the "accountId" cell', () => {
    const columnConfig = GetColumnConfig();
    const rowData = { clusterArn: 'arn:aws:elasticmapreduce:us-west-2:331110439030:cluster/j-3CLVK7LUVU18O' };

    const renderedValue = columnConfig[4].cellRenderer({ row: rowData });

    expect(renderedValue).toBe('331110439030');
  });
