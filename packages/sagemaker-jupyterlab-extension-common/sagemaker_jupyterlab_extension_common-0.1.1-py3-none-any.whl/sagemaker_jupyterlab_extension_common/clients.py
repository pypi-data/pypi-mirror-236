...
class SageMakerAsyncBoto3Client(BaseAysncBoto3Client, SingletonConfigurable):
    ...
    def __init__(self, region_name, partition_name):
        ...
        self.default_user_profile = get_user_profile_name()
    ...
    async def describe_user_profile(self, domain_id=None, user_profile_name=None):
        try:
            ...
            userProfile = (
                ...
                else user_profile_name
            )
            logging.info(
                f"Received request to describe user-profile with DomainId ({domainId}) and profile-name ({userProfile}) "
            )
            async with self._create_sagemaker_client() as sm_client:
                response = await sm_client.describe_user_profile(
                    DomainId=domainId, UserProfileName=userProfile
                )
                logging.info(
                    f"Successfuly described user-profile ({userProfile}) in domain with Id ({domainId})"
                )
        except botocore.exceptions.ClientError as error:
            logging.error(
                f"Failed to describe user-profile ({userProfile}) in domain ({domainId}). {error.response}"
            )
            raise error
        return response
