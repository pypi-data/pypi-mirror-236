...
async def test_describe_domain_success(mock_stage):
    # Given
...
async def test_describe_domain_called_with_default_domain_success(
    ...
    inner_client.describe_domain.assert_called_with(DomainId="default_domain")
    assert result == DESCRIBE_DOMAIN_TEST_RESPONSE
...
async def test_describe_user_profile_success(mock_stage):
    # Given
    sagemaker_client, inner_client = mock_sagemaker_client(
        {
...
async def test_describe_user_profile_called_with_default_profile_success(
    mock_stage, mock_user_profile
):
    # Given
    sagemaker_client, inner_client = mock_sagemaker_client(
        {
    ...
    # When
    result = await sagemaker_client.describe_user_profile(
        domain_id="d-jfffdjjjvhy", user_profile_name="default_profile"
    )
    ...
    inner_client.describe_user_profile.assert_called_with(
        DomainId="d-jfffdjjjvhy", UserProfileName="default_profile"
    )
    assert result == DESCRIBE_USER_PROFILE_TEST_RESPONSE
...
async def test_create_sagemaker_client_with_devo_stage(mock_stage):
    sagemaker_client, inner_client = mock_sagemaker_client(
        {
            "describe_domain.return_value": get_future_results(
                DESCRIBE_DOMAIN_TEST_RESPONSE
