...
def get_http_request_event_logger():
    """
    This function builds a eventlogger for http request logging.
...
"""
This is list of regex pattern that define filter pattern for the APIs that 
we want to log to CW log-groups. 

For example - filters currently supported APIs for
- resource-plugin: aws/sagemaker/api/instance/metrics
- git pluging:  aws/sagemaker/api/git/list/repositories

It is necessary to gate these APIS so we dont blowup metrics emitted into cloud watch.
...
allowed_uri_patterns = [
    {
        "regex": re.compile(r"/default/aws/sagemaker/api/(emr|instance|git)*"),
    }
]
