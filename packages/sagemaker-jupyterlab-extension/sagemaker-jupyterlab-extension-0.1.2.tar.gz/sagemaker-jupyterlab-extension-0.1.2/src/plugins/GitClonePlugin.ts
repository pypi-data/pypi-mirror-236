...
import { JUPYTER_COMMAND_IDS, pluginIds } from '../constants';
...
const executeGitCloneCommand = async (
  ...
  try {
    ...
    //   message: ClientErrorMessage.GitDialogError,
    //   error,
    // });
    return;
  }
  ...
  try {
    ...
    // studioLogger.error({
    //   schema: ClientSchemas.ClientError,
    //   message: ClientErrorMessage.ValidRepoPathError,
...
const GitClonePlugin: JupyterFrontEndPlugin<void> = {
  ...
  requires: [IFileBrowserFactory, IDefaultFileBrowser, IMainMenu, ITranslator],
  autoStart: true,
  activate: async (
    app: JupyterFrontEnd,
    factory: IFileBrowserFactory,
    ...
    commands.addCommand(JUPYTER_COMMAND_IDS.gitClone, {
      ...
      caption: '',
      execute: () => executeGitCloneCommand(factory, defaultFileBrowser, contents, commands),
