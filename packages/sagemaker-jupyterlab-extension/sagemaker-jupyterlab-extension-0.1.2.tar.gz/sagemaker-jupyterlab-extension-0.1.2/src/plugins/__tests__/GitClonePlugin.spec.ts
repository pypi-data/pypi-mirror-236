...
describe('Git clone plgin suite', () => {
  ...
  it('Should test the activate function of the git clone plugin', async () => {
    ...
    await GitClonePlugin.activate(appMock, mockFactory, mockDefaultFileBrowser, mainMenuMock, transMock);
  ...
  xit('should be able to run the Execute command', async () => {
    ...
    try {
      await executeGitCloneCommand(mockFactory, contentMock, commands);
    } catch (error) {
