...
class AUTH:
    def __init__(self, _login=None, _password=None, token=None, show_token=False):
        ...
        self.user_firstName = user_data['first_name']
        self.user_lastName = user_data['last_name']
        self.user_birthD = user_data['birth_date']
        self.user_class_level = user_data['class_level_id']
        self.user_class_name = user_data['class_name']
        self.user_parents = user_data['representatives'][0]['last_name']
        self.subjects = {item['subject_id']: item['name'] for item in user_data['groups']}
        # print(f'{self.user_firstName}, {self.user_birthD}, {self.user_class_name} класс\n')
...
class _Marks:
    ...
    def get_per_trimester(self, trimester: int = 0):
        data = get(
            ...
            f'{self.user.user_class_level}&student_profile_id={self.user.user_id}',
            headers={
                'Auth-Token': self.token,
                'Authorization': self.token
...
class _Homework:
    ...
    def get_by_date(self, date_offset: int = 0):
        ...
        if not payload:
            raise NullFieldError
        for item in payload:
            material = item.get('additional_materials', [{}])[0].get('items', [{}])[0] if item[
                'additional_materials'] else {}
            test_url = material.get('urls', [])
            result.append(
                HouseworkType(
                    subject_name=item.get('subject_name', ''),
                    description=sub(r'\n{2,}', '\n', item["description"]),
                    attached_file=material.get('link', None),
                    attached_test=test_url[-1].get('url') if test_url else None
                )
            )
        return result
