...
height, width = 0, 0
mvn = ''
menu_dict = config.previous_menu_map_dict
...
def do_install_maven(stdscr, skip_press_anykey: bool = False):
    ...
    '''
    update_mvn_path()
    exe_shell_curses(command, stdscr, skip_press_anykey)
...
def get_parent_menu(current_menu):
    return menu_dict.get(current_menu)
...
def display_menu(stdscr, current_menu) -> bool:
    ...
    try:
        while not break_flag:
            ...
            if is_move_key(sub_key):
                menu_current_row = do_move_key(sub_key, menu_current_row, current_menu)
            elif is_ctrl_c(sub_key):
                say_goodbye(stdscr)
            elif is_enter_key(sub_key):
                parent_menu = get_parent_menu(current_menu[0].replace(" ", ""))
                if is_exit(current_menu[menu_current_row]):
                    say_goodbye(stdscr)
                if parent_menu is None:
                    display_menu(stdscr, util.get_main_menu(menu_current_row))
                    continue
                if is_back(current_menu[menu_current_row]):
...
def main_menu(stdscr) -> bool:
    ...
    try:
        while not break_flag:
            key = stdscr.getch()
            if is_move_key(key):
                current_row = do_move_key(key, current_row, config.menu_items)
                show_menu(stdscr, current_row, config.menu_items)
            elif is_ctrl_c(key):
                say_goodbye(stdscr)
