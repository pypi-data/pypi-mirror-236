...
class SenSpot(Resensys):
    ...
    def timeStream(self,df_name,time=[],t_s="",t_e="",calibrate=True,sample_int=""):
        ...
        if cat == 2: # check if the data stream is high-rate (e.g. HPA or strain gauge)
            query = f'SELECT UNIX_TIMESTAMP(Time)*1000 - Optional AS OrderingTime, Time as MY_UTC_TIME, Value, SeqNo, Optional FROM {table} WHERE Time BETWEEN "{t_start}" AND "{t_end}" group by OrderingTime'
            ...
            query = f'SELECT Time as MY_UTC_TIME, Value, SeqNo, Optional FROM {table} WHERE Time BETWEEN "{t_start}" AND "{t_end}"'
            # apply downsample interval
            if sample_int in ['','M1','M6','M15','M30','H1','H4','H6','H12','D1','W1','W2','MN1','MN3','MN6','YR']:
                if sample_int == "":
                    query += " order by MY_UTC_TIME asc"
                elif sample_int == "M1":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (60) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (60) asc"
                elif sample_int == "M6":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (360) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (60) asc"
                elif sample_int == "M15":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (900) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (900) asc"
                elif sample_int == "M30":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (1800) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (1800) asc"
                elif sample_int == "H1":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600) asc"
                elif sample_int == "H4":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*4) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*4) asc"
                elif sample_int == "H6":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*6) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*6) asc"
                elif sample_int == "H12":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*12) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*12) asc"
                elif sample_int == "D1":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24) asc"
                elif sample_int == "W1":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*7) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*7) asc"
                elif sample_int == "W2":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*14) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*14) asc"
                elif sample_int == "MN1":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*30) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*30) asc"
                elif sample_int == "MN3":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*90) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*90) asc"
                elif sample_int == "MN6":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*180) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*180) asc"
                elif sample_int == "YR":
                    query += " group by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*365) order by UNIX_TIMESTAMP(MY_UTC_TIME) div (3600*24*365) asc"
        ...
        data_df['MY_UTC_TIME'] = pd.to_datetime(data_df['MY_UTC_TIME'])
        # Subtract the retarded time from the base time
        data_df['MY_UTC_TIME'] = data_df['MY_UTC_TIME'] - data_df['Time Added']
