...
import sigan.src.api.api as slack_api
import sigan.src.utils.general_utils as general_utils
import sigan.src.utils.time_utils as time_utils
import sigan.src.utils.display as display

from sigan.src.config import get_config
...
def delete_alarm(alarm_id: int = typer.Argument(..., help="[yellow]Alarm ID[/yellow] to be [red]deleted[/red]")):
    try:
        user_info = general_utils.read_team_id(cfg.app_dir)
    except FileNotFoundError:
        raise typer.BadParameter("Registration is not complete.")
...
def change_deadline(
    alarm_id: int = typer.Argument(..., help="[yellow]Alarm ID[/yellow] to be [purple]changed[/purple]"),
    deadline: str = typer.Option(..., "-d", "--deadline", help="deadline, [yellow]yyyy:mm:dd or mm:dd[/yellow]", callback=deadline_callback),
    ):
...
def change_date(
    alarm_id: int = typer.Argument(..., help="[yellow]Alarm ID[/yellow] to be [purple]changed[/purple]"),
    date: str = typer.Option(..., "-t", "--time", help="Alarm sending date or time, [yellow]yyyy/mm/dd or mm/dd or yyyy/mm/dd hh:mm or mm/dd hh:mm", callback=change_alarm_date_callback),
    ):
    try:
        user_info = general_utils.read_team_id(cfg.app_dir)
    except FileNotFoundError:
...
def change_interval(
    alarm_id: int = typer.Argument(..., help="[yellow]Alarm ID[/yellow] to be [purple]changed[/purple]"),
    interval: str = typer.Option(..., "-i", "--interval" ,help="[yellow]New interval[yellow]", callback=interval_callback)
    ):
    try:
        user_info = general_utils.read_team_id(cfg.app_dir)
    except FileNotFoundError:
...
@app.callback()
def app_setting():
    general_utils.config_setup(cfg.app_dir_path, cfg.team_id_path)