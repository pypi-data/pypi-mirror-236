...
from snb_node.tools.transutils import trans, _

# check for tornado 3.1.0
try:
    import tornado
except ImportError as e:
    raise ImportError(_("The SmartNoteBook requires tornado >= 5.0")) from e

try:
    version_info = tornado.version_info
except AttributeError as e:
    raise ImportError(_("The SmartNoteBook requires tornado >= 5.0, but you have < 1.1.0")) from e
if version_info < (5, 0):
    raise ImportError(_("The SmartNoteBook requires tornado >= 5.0, but you have %s") % tornado.version)
...
from snb_node.config.Config import  BASE_PATH
...
try:
    from snb_node.terminal import TerminalManager
...
from snb_node.config.manager import ConfigManager
from snb_node._version import __version__
from snb_node.kernels.kernelmanager import MappingKernelManager, AsyncMappingKernelManager
from snb_node.sessions.sessionmanager import SessionManager
from snb_node.tools.tz import utcnow
from snb_node.tools.utils import (
    check_pid,
    # pathname2url,
    run_sync,
    unix_socket_in_use,
...
from snb_node.gateway.managers import GatewayClient
from snb_node.tools.log import log_request
from snb_node.base.handlers import RedirectWithParams
from snb_node.tools.traittypes import TypeFromClasses
from snb_node.contents.manager import ContentsManager

DEFAULT_SmartNOTEBOOK_IP = "0.0.0.0"
DEFAULT_SmartNOTEBOOK_PORT = 8888
# -----------------------------------------------------------------------------
# Module globals
# -----------------------------------------------------------------------------

_examples = """
smart notebook                       # start the notebook
"""
...
class NbserverListApp(JupyterApp):
    ...
    flags = dict(
        jsonlist=({"NbserverListApp": {"jsonlist": True}}, _("Produce machine-readable JSON list output.")),
        json=({"NbserverListApp": {"json": True}}, _("Produce machine-readable JSON object on each line of output.")),
    )
...
flags["allow-root"] = ({"SmartNotebookApp": {"allow_root": True}}, _("Allow the notebook to be run from root user."))

flags["autoreload"] = (
    {"SmartNotebookApp": {"autoreload": True}},
...
aliases.update(
    {
        "ip": "SmartNotebookApp.ip",
        "port": "SmartNotebookApp.port",
        "port-retries": "SmartNotebookApp.port_retries",
        "sock": "SmartNotebookApp.sock",
        "sock-mode": "SmartNotebookApp.sock_mode",
        "transport": "KernelManager.transport",
        "notebook-dir": "SmartNotebookApp.notebook_dir",
        "pylab": "SmartNotebookApp.pylab",
...
def load_handlers(name, permission_config):
    """Load the (URL pattern, handler) tuples for each component."""
    try:
        mod = __import__(name, fromlist=["default_handlers"])
        for hanlers in mod.default_handlers:
            name = hanlers[1].__module__ + "." + hanlers[1].__name__
            name = name.replace("snb_node.node.", "")
            name = name.replace(".", "_")
            name = name.replace("_handlers_", "_")
            id = hanlers[0]
            # print(id)
            permi = hanlers[1].post.__annotations__.get("return", None)
            if permi:
                key = name + "_" + "post"
                if key in permission_config:
                    permission_config[key]["url"].append(id)
                else:
                    permission_config[key] = {"url": [id], "config": permi}

            permi = hanlers[1].get.__annotations__.get("return", None)
            if permi:
                key = name + "_" + "get"
                if key in permission_config:
                    permission_config[key]["url"].append(id)
            ...
            permi = hanlers[1].delete.__annotations__.get("return", None)
            if permi:
                key = name + "_" + "delete"
                if key in permission_config:
                    permission_config[key]["url"].append(id)
            ...
            permi = hanlers[1].put.__annotations__.get("return", None)
            if permi:
                key = name + "_" + "put"
                if key in permission_config:
                    permission_config[key]["url"].append(id)
...
def shutdown_server(server_info, timeout=5, log=None):
    ...
    *server_info* should be a dictionary as produced by list_running_servers().
    ...
    req = HTTPRequest(
        url + "api/shutdown", method="POST", body=b"", headers={"Authorization": "token " + server_info["token"]}
    )
    if log:
...
class NotebookWebApplication(web.Application):
    def __init__(
        ...
        term_h = jupyter_app.init_terminals()
        handlers.append(term_h[0])
        ...
        from snb_node.snb_faas.function import init_SNB_FAAS
    ...
    def init_settings(
        ...
        # Use the SmartNotebookApp logger and its formatting for tornado request logging.
        log_function = functools.partial(log_request, log=log, log_json=jupyter_app.log_json)
        print(f"config={jupyter_app.config}")
        print(f"config_dir={jupyter_app.config_dir}")
        settings = dict(
            # basics
    ...
    def init_handlers(self, settings):
        ...
        # load extra snb_node specified by users before default handlers
        for service in settings["extra_services"]:
            handlers.extend(load_handlers(service, permission_config))
        handlers.extend(load_handlers("snb_node.config.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.contents.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.sessions.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.kernels.handlers", permission_config))

        handlers.extend(load_handlers("snb_node.pyls.lsphandlers", permission_config))
        # handlers.extend(load_handlers('snb_node.pyls.lshandlers'))

        handlers.extend(load_handlers("snb_node.file_sync.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.db_metadata.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.exec_snb.handlers", permission_config))

        handlers.extend(load_handlers("snb_node.snb_faas.service.FaaSAdmin", permission_config))
        handlers.extend(load_handlers("snb_node.chatgptapi.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.permission.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.heartbeat.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.chatgptv2.handlers", permission_config))
        handlers.extend(load_handlers("snb_node.ireport.IReportKernel", permission_config))
        ...
        if GatewayClient.instance().gateway_enabled:
            ...
            gateway_handlers = load_handlers("gateway.handlers", permission_config)
        ...
        handlers.extend(load_handlers("base.handlers", permission_config))
...
class NbserverStopApp(JupyterApp):
    ...
    port = Integer(
        DEFAULT_SmartNOTEBOOK_PORT,
        config=True,
        help=f"Port of the server to be killed. Default {DEFAULT_SmartNOTEBOOK_PORT}",
    )
    ...
    def start(self):
        ...
        for server in servers:
            ...
            print(f"There is currently no server running on {current_endpoint}", file=sys.stderr)
...
class SmartNotebookApp(JupyterApp):
    name = "smart-notebook"
    version = __version__
    description = _("Smart HTML Notebook.这将启动一个基于Tornado的HTML笔记本服务器，提供一个HTML5/Javascript笔记本客户端")

    examples = _examples
    aliases = aliases
    flags = flags

    classes = [KernelManager, Session, MappingKernelManager, KernelSpecManager, NotebookNotary]
    ...
    allow_credentials = Bool(False, config=True, help=_("Set the Access-Control-Allow-Credentials: true header"))

    allow_root = Bool(False, config=True, help=_("Whether to allow the user to run the notebook as root."))
    ...
    autoreload = Bool(False, config=True, help=("Reload the webapp when changes are made to any Python src files."))

    default_url = Unicode("/tree", config=True, help=_("The default URL to redirect to from `/`"))

    ip = Unicode("localhost", config=True, help=_("The IP address the notebook server will listen on."))

    cookie_secret = b"eb890ae6203c8cba141ddfe91644beddb25974b2e7502e95"

    @default("ip")
    def _default_ip(self):
        return DEFAULT_SmartNOTEBOOK_IP
    ...
    custom_display_url = Unicode("", config=True, help=_("""Override URL shown to users."""))

    port_env = "JUPYTER_PORT"
    port_default_value = DEFAULT_SmartNOTEBOOK_PORT
    port = Integer(
        port_default_value, config=True, help=_("The port the notebook server will listen on (env: JUPYTER_PORT).")
    )
    ...
    sock = Unicode("", config=True, help=_("The UNIX socket the notebook server will listen on."))

    sock_mode = Unicode("0600", config=True, help=_("The permissions mode for UNIX socket creation (default: 0600)."))
    ...
    token = Unicode(
        ...
        with the JUPYTER_TOKEN environment variable. When no password is enabled, the default is to generate a new, random token.
    ...
    allow_remote_access = Bool(config=True, help=""" """)
    ...
    tornado_settings = Dict(
        config=True, help=_("Supply overrides for the tornado.web.Application that the " "Jupyter notebook uses.")
    )
    ...
    jinja_environment_options = Dict(
        config=True, help=_("Supply extra arguments that will be passed to Jinja environment.")
    )
    ...
    enable_mathjax = Bool(
        True,
        config=True,
        help="""Whether to enable MathJax for typesetting math/TeX   When disabled, equations etc. will appear as their untransformed TeX source.  """,
    )
    ...
    base_url = Unicode(
        "/",
        config=True,
        help="""The base URL for the notebook server.Leading and trailing slashes can be omitted, and will automatically be added.""",
    )
    ...
    extra_template_paths = List(
        ...
        config=True,
        help=_(
            """Extra paths to search for serving jinja templates. Can be used to override templates from notebook.templates."""
        ),
    )

    extra_nbextensions_path = List(
        Unicode(), config=True, help=_("""extra paths to look for Javascript notebook extensions""")
    )

    extra_services = List(
        Unicode(),
        config=True,
        help=_("""handlers that should be loaded at higher priority than the default snb_node"""),
    )
    ...
    websocket_url = Unicode(
        "",
        config=True,
        help="""The base URL for websockets, if it differs from the HTTP server (hint: it almost certainly doesn't). Should be in the form of an HTTP origin: ws[s]://hostname[:port] """,
    )

    mathjax_url = Unicode(
        "",
        config=True,
        help="""A custom url for MathJax.js. Should be in the form of a case-sensitive url to MathJax, for example:  /static/components/MathJax/MathJax.js """,
    )
    ...
    contents_manager_class = TypeFromClasses(
        ...
        default_value=ContentsManager,
        # default_value=jupyter_server.snb_node.contents.manager.ContentsManage,
        klasses=[
            ContentsManager,
            ...
            # will be ignored.
            "jupyter_server.contents.snb_node.managers.ContentsManager",
        ],
        config=True,
    ...
    session_manager_class = Type(default_value=SessionManager, config=True, help=_("The session manager class to use."))

    config_manager_class = Type(default_value=ConfigManager, config=True, help=_("The config manager class to use"))
    ...
    def _update_pylab(self, change):
        ...
        self.log.error(_("Please use `%pylab{0}` or `%matplotlib{0}` in the notebook itself.").format(backend))
        self.exit(1)

    notebook_dir = Unicode(config=True, help=_("The directory to use for notebooks and kernels."))
    ...
    server_extensions = List(Unicode(), config=True, help=(_("DEPRECATED use the nbserver_extensions dict instead")))
    ...
    def parse_command_line(self, argv=None):
        ...
        if self.extra_args:
            ...
            c = Config()
            if os.path.isdir(f):
                c.SmartNotebookApp.notebook_dir = f
            elif os.path.isfile(f):
                c.SmartNotebookApp.file_to_run = f
    ...
    def init_logging(self):
        ...
        for log in app_log, access_log, gen_log:
            # consistent log output name (SmartNotebookApp instead of tornado.access, etc.)
    ...
    def init_webapp(self):
        ...
        if self.sock:
            if self.port != DEFAULT_SmartNOTEBOOK_PORT:
                self.log.critical(
            ...
            if self.file_to_run:
                self.log.critical(
                    _("Options --SmartNotebookApp.file_to_run and --sock are mutually exclusive."),
                )
                sys.exit(1)
    ...
    def _bind_http_server_tcp(self):
        ...
        if not success:
            if self.port_retries:
                self.log.critical(
                    _("ERROR: the notebook server could not be started because " "no available port could be found.")
                )
            else:
                self.log.critical(
                    _("ERROR: the notebook server could not be started because " "port %i is not available.") % port
                )
            self.exit(1)
        return success
    ...
    def init_terminals(self):
        ...
        try:
            from snb_node.terminal import initialize

            self.terminals_available = True
            return initialize(nb_app=self)
        except ImportError as e:
    ...
    def init_server_extension_config(self):
        ...
        extensions = section.get("SmartNotebookApp", {}).get("nbserver_extensions", {})

        for modulename, enabled in sorted(extensions.items()):
            if modulename not in self.nbserver_extensions:
                self.config.SmartNotebookApp.nbserver_extensions.update({modulename: enabled})
            ...
            if enabled:
                try:
                    ...
                    if self.reraise_server_extension_failures:
                        raise
                    self.log.warning(_("Error loading server extension %s"), modulename, exc_info=True)
    ...
    def shutdown_no_activity(self):
        ...
        if seconds_since_active > self.shutdown_no_activity_timeout:
            self.log.info("No kernels or terminals for %d seconds; shutting down.", seconds_since_active)
            self.stop()

    def init_shutdown_no_activity(self):
        if self.shutdown_no_activity_timeout > 0:
            self.log.info(
                "Will shut down after %d seconds with no kernels or terminals.", self.shutdown_no_activity_timeout
            )
    ...
    def notebook_info(self, kernel_count=True):
        ...
        info += _(
            "SmartNoteBook node {version} is running at:\n{url}".format(
                version=SmartNotebookApp.version, url=self.display_url
            )
        )
        # if self.gateway_config.gateway_enabled:
    ...
    def write_server_info_file(self):
        ...
        try:
            ...
            self.log.error(_("Failed to write server-info to %s: %s"), self.info_file, e)
    ...
    def start(self):
        ...
        if "dev" in __version__:
            with open(os.path.join(BASE_PATH, "ascii_logo.txt"), 'r') as logo:
                info(_("\n" + logo.read()))

            info(_("Welcome to SmartNoteBook! resources section at http://wwww.smartnotebook.tech/"))
...
main = launch_new_instance = SmartNotebookApp.launch_instance
