...
def wrangle_masking_policy(self, database_name:str, schema_name:str, env_database_prefix:str, env_role_prefix:str, deploy_db_name:str, ignore_roles_list:str, deploy_tag_list:list[str], current_role:str, available_roles:list[str], handle_ownership)->dict:
    if env_database_prefix is None:
        env_database_prefix = ''
    if env_role_prefix is None:
        env_role_prefix = ''

    polices = self._sf.masking_policies_get(database_name, schema_name)  
    
    data = []
    for d in polices:
        full_policy_name = database_name + '.' + schema_name + '.' + d['MASKING_POLICY_NAME']
        d['OWNER'] = self._handle_ownership(handle_ownership, d['OWNER'], 'masking policy', full_policy_name, current_role, available_roles)
        ...
        tags = []
        tags_raw = self._sf.tag_references_get(database_name, full_policy_name, 'masking policy') #task tag references live within Snowflake db
        for t in tags_raw:
        ...
        grants_raw = self._sf.grants_get(full_policy_name, 'masking policy')
        grants = []
        for g in grants_raw:
