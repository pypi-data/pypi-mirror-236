...
def convert_2_fits():
    ...
    for field in fields:
        cat_name = f'{field}_mag_gaiaxpsp.cat'
        cat_step = 'gaiaXPSP'
        ...
        save_name = f'{field}_mag_gaiaxpsp.fits'
...
def fix_XY_rotation():
    ...
    for field in fields:
        ...
        cat_name = f'{field}_mag_gaiaxpsp.fits'
        cat_file = os.path.join(save_path, cat_name)

        save_name = f'{field}_mag_gaiaxpsp_xycorr.fits'
        save_file = os.path.join(save_path, save_name)
        ...
        # In case of shorts, single images are used, so the XY rotation fix
        # should be skipped
        if conf['shorts']:
            if not os.path.exists(save_file):
                cmd = f"cp {cat_file} {save_file}"
                ut.printlog(cmd, log_file)
                os.system(cmd)
            else:
                ut.printlog(f"File {save_file} already exists.", log_file)
...
def concatenate_calib_catalogs():
    ...
    cat_name = '{field}_mag_gaiaxpsp_xycorr.fits'
    cat_file =  os.path.join(save_path, cat_name)

    # Start save name
    save_name = 'concat_mag_gaiaxpsp_xycorr.cat'
    save_file = os.path.join(save_path, save_name)
...
def compute_xy_maps():
    ...
    cat_name = 'concat_mag_gaiaxpsp_xycorr.cat'
    cat_file = os.path.join(save_path, cat_name)

    for filt in conf["gaiaxpsp_sed_pred"]:
        save_name = f"{filt}_offsets_grid.npy"
        save_file = os.path.join(save_path, save_name)
...
def plot_xy_maps():
    ...
    for filt in conf["gaiaxpsp_sed_pred"]:
        ...
        grid_name = f"{filt}_offsets_grid.npy"
