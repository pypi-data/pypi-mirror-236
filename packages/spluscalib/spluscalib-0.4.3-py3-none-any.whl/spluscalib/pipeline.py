...
cprint(conf_file, color = "white", attrs=["dark"])
...
for field in fields:
    ...
    ************************************************************************
    ...
    if 'photometry_aperimages' in conf['run_steps']:
        ...
        final_file = os.path.join(field_path, 'Photometry', 'dual', 'detection',
    ...
    if 'calibration_gaiaXPSP' in conf['run_steps']:
        ...
        script = os.path.join(pipeline_path, 'steps', 'calibration_gaiaXPSP.py')
        cmd = f'python3 {script} {field} {conf_file}'
        ut.printlog(cmd, log_field_file,
                    color = "blue", on_color = "on_white")
        os.system(cmd)
    ...
    if 'plot_mosaic_calib' in conf['run_steps']:
        ...
        if plot_mosaic:
            ...
            ut.printlog("Mosaic plot already exists and configuration is set"
                        " not to overwrite.", log_field_file)
