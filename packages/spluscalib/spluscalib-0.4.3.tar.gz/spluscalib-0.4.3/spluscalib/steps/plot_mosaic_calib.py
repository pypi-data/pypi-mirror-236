...
obs_database = pd.read_csv(obs_database_file)
...
filters_splus     = conf["internal_sed_pred"]
...
ref_xmatch = fits.open(ref_xmatch_file )[1].data
gaia_xmatch = fits.open(gaia_xmatch_file )[1].data

########

ref_catalog_file = os.path.join(field_path, "Crossmatch", ref_xmatch_name)
ref_catalog = fits.open(ref_catalog_file)[1].data

gaia_catalog_file = os.path.join(field_path, "Crossmatch",
                                 f"{field}_GAIADR3.fits")
gaia_catalog =  fits.open(gaia_catalog_file)[1].data

########
calib_phot = conf["calibration_photometry"]
xy_corr_file = os.path.join(field_path, "Photometry", calib_phot,
                            "xy_correction",
                            f"sex_{field}_R_{calib_phot}_xycorr.fits")

x0y0l_file = os.path.join(field_path, "Photometry", calib_phot,
                          "xy_correction",
                          f"{field}_R_{calib_phot}_x0y0l.csv")

try:
    xy_corr_data = fits.open(xy_corr_file)[1].data
...
zp_int_data_file = os.path.join(field_path, f"Calibration_{calib}", "internal",
                                f"{field}_splus_sed_fit.cat")
...
zp_ext_data = pd.read_csv(zp_ext_data_file,
                          delim_whitespace = True, escapechar = "#")
zp_int_data = pd.read_csv(zp_int_data_file,
                          delim_whitespace = True, escapechar = "#")
...
zp_int_file = os.path.join(field_path, f"Calibration_{calib}", "internal",
                           f"{field}_internal_tmp.zp")
zp_tmp_file = os.path.join(field_path, f"Calibration_{calib}", "internal",
                           f"{field}_external_and_internal.zp")
...
zp_int = ut.zp_read(zp_int_file)
zp_tmp = ut.zp_read(zp_tmp_file)
zp_gsc = ut.zp_read(zp_gsc_file)
zp_gsc_mean = ut.zp_read(zp_gsc_mean_file)
zp_final = ut.zp_read(zp_final_file)

try:
    zp_stl_file = os.path.join(field_path, f"Calibration_{calib}", "stlocus",
                               f"{field}_stlocus_tmp.zp")
    zp_stl = ut.zp_read(zp_stl_file)
except:
    pass

try:
    zp_stl_data_file = os.path.join(field_path, f"Calibration_{calib}", "external",
                               f"{field}_mag_ext.cat")
    zp_stl_data = pd.read_csv(zp_stl_data_file,
                          delim_whitespace = True, escapechar = "#")
except:
    pass
...
mosaic = [['00', '00', '00', '03', '03', '03', '03', '03', '08', '09', '09', '09', '09', '09', '09'], # 0
          ['10', '10', '10', '03', '03', '03', '03', '03', '08', '09', '09', '09', '09', '09', '09'], # 1
          ['20', '20', '20', '23', '24', '25', '26', '27', '28', '29', '29', '29', '2C', '2C', '2C'], # 2
          ['30', '30', '30', '33', '34', '35', '36', '37', '38', '39', '3A', '3B', '3C', '3D', '3E'], # 3
          ['40', '40', '40', '43', '44', '45', '46', '47', '48', '49', '4A', '4B', '4C', '4D', '4E'], # 4
          ['40', '40', '40', '53', '54', '55', '56', '57', '58', '59', '5A', '5B', '5C', '5D', '5E'], # 5
          ['40', '40', '40', '63', '64', '65', '66', '67', '68', '69', '6A', '6B', '6C', '6D', '6E'], # 6
          ['70', '70', '70', '73', '73', '75', '76', '76', '78', '79', '79', '7B', '7C', '7C', '7C'], # 7
          ['80', '80', '80', '83', '84', '85', '86', '87', '88', '89', '8A', '8B', '8C', '8D', '8E'], # 8
          ['80', '80', '80', '93', '93', '95', '95', '97', '97', '99', '99', '9A', '9A', '9D', '9D'], # 9
          ['80', '80', '80', '93', '93', '95', '95', '97', '97', '99', '99', '9A', '9A', '9D', '9D']#, # A
         ]
...
# Expand external zp fitting plot when there is space available
line = 4
col0 = 3
for filt in filters_splus:
    i = filters_splus_order[filt]
    if (filt in filters_splus_ext) and (filt not in filters_splus_stlocus):
        mosaic[line][col0+i] = mosaic[line-1][col0+i]

    if (filt not in filters_splus_ext) and (filt not in filters_splus_stlocus):
        mosaic[line][col0+i] = mosaic[line+1][col0+i]

    #if (filt in filters_splus_stlocus) and (filt not in filters_splus_ext):
    #    mosaic[line-1][col0+i] = mosaic[line][col0+i]
...
def plot_filters(filter_list, filters_colors, ax, path_to_filters = "",
    ...
    # Plot labels
    ax.text(1000, -0.2, r"1000 $\AA$", ha = "center", color = label_color,
            fontsize = 14, alpha = 0.5)

    for x in range(4000, 12000, 3000):
        ax.text(x, -0.2, x, ha = "center", color = label_color, fontsize = 14,
                alpha = 0.5)
...
def plot_xy_pos(xy_corr_data, x0y0l_data, ax, conf, line_color = "#00AAFF"):

    x = xy_corr_data["X_IMAGE"]
    y = xy_corr_data["Y_IMAGE"]
...
def plot_detections(detections,  ax, label_color, ref_xmatch,
                    xmatch_color = "#880066"):
    N = len(detections["X"])
    ...
    ax.scatter(ref_xmatch["X"], ref_xmatch["Y"], color = xmatch_color, s = 15,
               marker = 'o', alpha = 0.8,
               label = f"{ref_label} x-match (N = {N})")

    ax.legend(loc = 2, frameon=False, labelcolor = color2, fontsize = 14)
...
def plot_Alambda_dist(ebv_data, ax, filters_splus, alambda_av,
    ...
    for y in yticks_major:
        ax.plot([-1, 12], [y, y], lw=0.5, alpha=0.2, zorder=-5,
                color=label_color)
        ax.text(10.5, y, y, ha="center", va="bottom", rotation=0,
    ...
    ax.set_xlim(-0.5, len(filters_splus) - 0.5)
...
def plot_zp_fitting(filt, zp_fitting_data, zp_data="", ax=plt, mag_low=14,
    ...
    ax.scatter(x[f & fg], y[f & fg], color=filt_color, marker='.', s=5,
               alpha=0.7, zorder=3)
    ...
    if zoom:
        ...
        if zp_data != "":
            ...
            ax.plot([12, 22], [zp_data[filt], zp_data[filt]],
                    ls='--', lw=2, color=filt_color, zorder=2)
            ax.text(21.7, zp_data[filt] - 0.15, f"{zp_data[filt]:.3f}",
                    va="center", ha="right", color=filt_color,
                    fontsize=17, alpha=0.8)

        # Plot ticks
        for y in np.arange(-0.5, 1, 0.5):
            ax.text(12.2, y, f"{y:0.1f}", va="center", ha="left",
        ...
        for x in [13, 15, 17, 19]:
            ax.text(x, -1 + 0.1, x, ha="center", color=label_color, fontsize=10,
                    alpha=0.5)

        ax.text(21.5, -1 + 0.1, r"mag$_{\mathrm{AB}}$", ha="right",
                color=label_color, fontsize=9, alpha=0.5)
        ax.text(12.4, 1 - 0.12, r"$\Delta$mag", ha="left", va="top",
                rotation=90, color=label_color, fontsize=9,
                alpha=0.5)

        for x in np.arange(13, 22, 2):
            ax.plot([x, x], [-1, -1 + 0.05], color=label_color, lw=1, alpha=0.5)

        for x in np.arange(14, 22, 1):
        ...
        if zp_data != "":
            ...
            ax.plot([12, 22], [zp_data[filt], zp_data[filt]], ls='--', lw=2, color=filt_color, zorder=2)
            ax.text(21.7, zp_data[filt] - 0.6, f"{zp_data[filt]:.3f}", va="center", ha="right", color=filt_color,
                    fontsize=17, alpha=0.8)
        ...
        for y in np.arange(0, 6, 2):
            ax.text(12.2, y, f"{y:0d}", va="center", ha="left", color=label_color, fontsize=10, alpha=0.5, rotation=90)

        for y in np.arange(0, 6, 2):
            ax.plot([12, 23], [y, y], color=label_color, lw=1, alpha=0.05, zorder=-5)

        for y in np.arange(-3, 6, 2):
            ax.plot([12, 23], [y, y], color=label_color, lw=1, alpha=0.02, zorder=-5)

        for y in np.arange(-3.5, 6, 1):
            ax.plot([12, 23], [y, y], color=label_color, lw=1, alpha=0.01, zorder=-5)

        for x in [13, 15, 17, 19]:
            ax.text(x, -2 + 0.2, x, ha="center", color=label_color, fontsize=10, alpha=0.5)

        ax.text(21.5, -2 + 0.2, r"mag$_{\mathrm{AB}}$", ha="right", color=label_color, fontsize=9, alpha=0.5)
        ax.text(12.4, 6 - 0.12, r"$\Delta$mag", ha="left", va="top", rotation=90, color=label_color, fontsize=9,
                alpha=0.5)
...
fig, axs = plt.subplot_mosaic(mosaic,
                              layout='constrained',
                              figsize = (2*1960*px, 2*1440*px),
                              facecolor=color0,
                              per_subplot_kw={"10": {"projection": ccrs.Robinson()}})
...
ax = axs['40']
ax.fill_between([0.94,1], [2./3, 2./3], [3./3, 3./3], color = color0,
                transform = ax.transAxes)
if 'calibration_stloc' in conf['run_steps']:
    ax.text(0.96, 5./6, "Stellar Locus", va = "center", color = "#b4008c",
            fontsize = 16, rotation = 90, transform = ax.transAxes, alpha = 0.8)

ax = axs['40']
ax.fill_between([0.94,1], [1./3, 1./3], [2./3, 2./3], color = color0,
                transform = ax.transAxes)
ax.text(0.96, 3./6, "Internal", va = "center", color = "#b4008c",
        fontsize = 16, rotation = 90, transform = ax.transAxes, alpha = 0.8)

ax = axs['40']
ax.fill_between([0.94,1], [0, 0], [1./3, 1./3], color = color0,
                transform = ax.transAxes)
ax.text(0.96, 1./6, "XY-check", va = "center", color = "#b4008c",
        fontsize = 16, rotation = 90, transform = ax.transAxes, alpha = 0.8)
...
newax = fig.add_axes([0.025,0.9,0.15,0.1], anchor = "NE", zorder=1)
...
ax.text(0.00, 0.97, field, fontsize = 22, color = field_name_color, ha='left',
        va='top', transform = ax.transAxes)

ax.text(0.98, 0.97, conf["calibration_name"], fontsize = 12,
...
ax = axs["20"]
ax.fill_between([0.,1.], [0.90, 0.90], [1, 1], color = color0, transform = ax.transAxes)
...
plot_detections(detections = detections, ax = ax, label_color = color2,
                ref_xmatch = ref_xmatch, xmatch_color = "#AA5533")

if xy_corr_data is not None:
    plot_xy_pos(xy_corr_data = xy_corr_data,
                x0y0l_data   = x0y0l_data,
                ax = ax,
                conf = conf,
                line_color = "#888888")
...
#####################################
#### Plot zp-fitting ref
ax_line = ['23', '24', '25', '26', '27', '28']
for i in range(len(filters_ref)):
    filt = filters_ref[i]
    ax = axs[ax_line[i]]
    plot_zp_fitting(filt, zp_ext_data, ax = ax, mag_low = 14, mag_up = 18,
...
#####################################
#### Plot zp-fitting stlocus

ax_line = ['43', '44', '45', '46', '47', '48', '49', '4A', '4B', '4C', '4D', '4E']

if filters_splus_stlocus:
    for filt in filters_splus_stlocus:
            i = filters_splus_order[filt]
            ax = axs[ax_line[i]]
            plot_stloc_zp_fitting(filt, zp_stl_data, stlocus_reference, conf = conf, zp_data = zp_stl,
                                  ax=ax, label_color="#FFFFFF",filters_colors = filters_colors)
#####################################
#### Plot zp-fitting internal

ax_line = ['53', '54', '55', '56', '57', '58', '59', '5A', '5B', '5C', '5D', '5E']

mag_low_list = conf["internal_zp_fitting_mag_low"]
mag_up_list = conf["internal_zp_fitting_mag_up"]

for filt, mag_low, mag_up in zip(filters_splus, mag_low_list, mag_up_list):
    i = filters_splus_order[filt]
    ax = axs[ax_line[i]]
    if (filt in filters_splus_ext) or (filt in filters_splus_stlocus):
        zoom = True
    else:
        zoom = False
        
    plot_zp_fitting(filt, zp_int_data, zp_int, ax = ax, mag_low = mag_low,
...
ax_line = ['73', '73', '75', '76', '76', '78', '79', '79', '7B', '7C', '7C', '7C'] # 7
...
#####################################
#### xy-check
ax_line = ['63', '64', '65', '66', '67', '68', '69', '6A', '6B', '6C', '6D', '6E'] # 5

for filt in filters_splus_ext:
    i = filters_splus_order[filt]
    ax = axs[ax_line[i]]
    plot_xycheck(xychecks[filt], filt, ax = ax, filters_colors = filters_colors,
                 label_color = color2, bg_color = color0, clim = [-0.1, 0.1])
