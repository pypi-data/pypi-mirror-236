...
struct CoordinateMolecularMap
{
    ...
    CPP_ATOM_GROUP molecule_atoms;
    std::vector<int> periodic_molecules;

    int threads_per_block = 256;
    int blocks_per_grid = 20;
    //注意传入的crd是device上地址，一般初始化的时候总是有这个东西的
    void Initial(CONTROLLER *controller, int atom_numbers, VECTOR box_length, VECTOR *crd, 
        CONECT connectivity, const char *module_name = NULL);
    //清除内存
    void Clear();
