...
class Price_Logic(CustomBaseModel):
    ...
    def unpack(details: dict[str, Any]) -> "Price_Logic":
        ...
        print ("--------------------------------------")
        print(pricefinding_class_string)
        if pricefinding_class_string == "Interrupted_On_Time":
            pricefinding = Interrupted_On_Time()
        elif pricefinding_class_string == "Continuous_On_Time":
            pricefinding = Continuous_On_Time()

        else:
            raise PricefindingError

        return Price_Logic(market=market, nr_of_hours_on=nr_of_hours_on, timeframes=timeframes, pricefinding=pricefinding)
