...
class BuilderModel:
    ...
    def buildPySIGMA(self):
        """
        Building a pySIGMA input files
        """

        magnet_name = self.model_data.GeneralParameters.magnet_name
        builder_SIGMA = BuilderPySIGMA(model_data=self.model_data, roxie_data=self.roxie_data)
        self.parser_SIGMA = ParserPySIGMA(builder_SIGMA)
        self.parser_SIGMA.writeSIGMA2yaml(output_path=self.output_path, simulation_name=magnet_name,
                                          append_str_to_magnet_name='_SIGMA')
        coordinate_file_path = self.parser_SIGMA.coordinate_file_preprocess(model_data=self.model_data, input_folder_path=self.path_magnet, output_folder_path=self.output_path)

        # Call mainPySIGMA which generates the Java files.
        input_file_path = os.path.join(self.output_path, f'{magnet_name}_SIGMA.yaml')
        mps = MainPySIGMA(model_folder=self.output_path)
        mps.build(input_yaml_file_path=input_file_path, input_coordinates_path=coordinate_file_path, results_folder_name=self.results_folder_name)
