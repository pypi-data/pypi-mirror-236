...
class ParserRoxie:
    ...
    def getCoilDataFromDataFile(self, coil_content: str = None, verbose: bool = False):
        ...
        for key in keywords:
            ...
            if key == 'block':
                layers = list(set([float(row.split()[3 if int(row.split()[1]) == 1 else 4]) for row in parameters]))
                layers.sort()
                poles = list(set([int(row.split()[11]) for row in parameters]))
                poles.sort()
                poles_flipped = poles.copy()
                poles_flipped.reverse()
            ...
            for row in parameters:
                ...
                if key == 'group':
                    ...
                    if not data.groups:
                        coil = 1
                        pole = poles.index(turn) + 1 if imag == 0 else poles_flipped.index(turn) + 1
                        if imag == 1:
                            for blk_name, blk in data.blocks.items():
                                if blk.pole == pole and blk.phi == phi and blk.current == - current:
                                    winding = blk.winding
                                    break
                    else:
                        for group in data.groups:
                            ...
                            if block_nr in data.groups[group].blocks:
                                group_nr = int(group)
                                if data.groups[group].symm == 0:
                                    pole = poles.index(turn) + 1 if imag == 0 else poles_flipped.index(turn) + 1
                                else:
                                    pole = floor(turn * data.groups[group].symm / 360 + 1)
                    ...
                    layer = layers.index(radius if block_type == 1 else phi) + 1
                    ...
                    data.blocks[rowSplitStr[0]] = pd.Block(type=block_type, nco=int(rowSplitStr[2]),
                                                           ...
                                                           winding=winding, shift2=pd.Coord(x=0., y=0.),
    ...
    def findConductorPositions(self, block: pd.Block = None, conductor: pd.CondPar = None, verbose: bool = False):
        ...
        for c in range(1, self.block.nco + 1):
            ...
            if self.block.type == 2 and self.block.alpha < 0:  # block-coil
                                    ...
                                    for block_key, block in winding.blocks.items():
                                        ...
                                        if self.symmetric_coil:
                                            if blocks.index(block_key) == 0:
                                                ...
                                                wedge.corners = pd.Corner(iH=block.block_corners.iL, iL=adj_winding.blocks[
                                                    adj_blocks[blocks.index(block_key)]].block_corners.iH,
                                                                          oH=block.block_corners.oL, oL=adj_winding.blocks[
                                                        adj_blocks[blocks.index(block_key)]].block_corners.oH)
                                                wedge.corners_ins = pd.Corner(iH=block.block_corners_ins.iL,
                                                                              iL=adj_winding.blocks[adj_blocks[blocks.index(
                                                                                  block_key)]].block_corners_ins.iH,
                                                                              oH=block.block_corners_ins.oL,
                                                                              oL=adj_winding.blocks[adj_blocks[blocks.index(
                                                                                  block_key)]].block_corners_ins.oH)
                                                wedge.order_h = pd.Order(coil=coil_nr, pole=pole_nr, layer=layer_nr,
                                                                         winding=winding_key, block=block_key)
                                                wedge.order_l = pd.Order(coil=coil_nr, pole=pole_nr, layer=layer_nr,
                                            ...
                                            if anticlockwise:
                                                wedge.corners = pd.Corner(iH=adj_winding.blocks[
                                                    adj_blocks[blocks.index(block_key)]].block_corners.iL,
                                                                          iL=block.block_corners.iH,
                                                                          oH=adj_winding.blocks[adj_blocks[
                                                                              blocks.index(block_key)]].block_corners.oL,
                                                                          oL=block.block_corners.oH)
                                                wedge.corners_ins = pd.Corner(iH=adj_winding.blocks[
                                                    adj_blocks[blocks.index(block_key)]].block_corners_ins.iL,
                                                                              iL=block.block_corners_ins.iH,
                                                                              oH=adj_winding.blocks[adj_blocks[blocks.index(
                                                                                  block_key)]].block_corners_ins.oL,
                                                                              oL=block.block_corners_ins.oH)
                                                wedge.order_l = pd.Order(coil=coil_nr, pole=pole_nr, layer=layer_nr,
                                                                         winding=winding_key, block=block_key)
                                                wedge.order_h = pd.Order(coil=coil_nr, pole=pole_nr, layer=layer_nr,
    ...
    def getData(self, dir_iron: Path = None, dir_data: Path = None, dir_cadata: Path = None,
        ...
        if path_to_yaml_model_data is not None:
            try:
