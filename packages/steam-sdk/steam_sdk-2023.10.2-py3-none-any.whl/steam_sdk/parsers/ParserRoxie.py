    ...
    def getCoilDataFromDataFile(self, coil_content: str = None, verbose: bool = False):
        ...
        for key in keywords:
            ...
            if key == 'block':
                ...
                layers.sort()
                poles = list(set([float(row.split()[11]) for row in parameters]))
                poles.sort()
            ...
            for row in parameters:
                ...
                if key == 'group':
                    ...
                    if not data.groups:
                        coil = 1
                        pole = poles.index(turn) + 1 if imag == 0 else poles[::-1].index(turn) + 1
                        if imag == 1:
                            winding_correction_blocks.append(int(rowSplitStr[0]))
                    else:
