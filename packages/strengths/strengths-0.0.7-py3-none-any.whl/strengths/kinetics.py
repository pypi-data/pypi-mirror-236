...
def compute_reaction_rates(system, 
    ...
    :param position: position of the mesh in which the rates should be computed
    ...
    position_index = system.get_mesh_index(position)
    ...
    volume = system.space.mesh_vol.copy()
    
    if not r.environments==None and not system.network.environments[system.mesh_env[position_index]] in r.environments :
...
def compute_diffusion_rates(system, 
    ...
    given species of a given system at a given state from a given source mesh to another destination mesh.
    Those are computed from the diffusion coefficient and mesh size according to the method presented by David Bernstein (Bernstein, 2015) [#Bernstein2015]_.
    ...
    :param src_position: position of the diffusion source mesh.
    :type src_position: number, tuple or coord like
    :param dst_position: position of the diffusion destination mesh.
    ...
    src_position_index = system.get_mesh_index(src_position)
    dst_position_index = system.get_mesh_index(dst_position)
    ...
    Di, Dj = 0, 0
    if isdict(s.D):
        Di = s.D.get(system.network.environments[system.space.mesh_env[src_position_index]], UnitValue(0, "µm2/s"))
        Dj = s.D.get(system.network.environments[system.space.mesh_env[dst_position_index]], UnitValue(0, "µm2/s"))
    else :
        Di = s.D
        Dj = s.D

    h = system.space.mesh_vol**(1/3)
    k = UnitValue("0 s-1")
    if Di!=0 and Dj!=0 : 
        k = 2/(h**2 * (1/Di + 1/Dj))
...
def compute_dspeciesdt(system, 
    ...
    p = system.space.get_mesh_coordinates(system.space.get_mesh_index(position))
