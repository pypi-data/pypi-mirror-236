...
class RDOutput :
    """
    Store the output of a reaction-diffusion system trajectory simulation.
    ...
    :type engine_description: str
    :param engine_option: option used with the engine used for the simulation.
    :type engine_option: str
    :param rng_seed: pseudo random number generator seed used for the simulation.
    :type rng_seed: int
    """
    ...
    def __init__ (self, 
        ...
        self._system = script.system.copy()
    ...
    def get_state(self, species, sample) :
        ...
        if isnone(species) :
            ...
            return UnitArray(
                self.data.value.reshape((self.nsamples(), self.nspecies()*self.nmeshes()))[sample_index, : ], self.data.units, check_value=False)
        else:
            species_index = self.system.network.get_species_index(species)
            if isnone(species_index) :
                raise Exception("Undefined species \""+species+"\".")
    ...
    def get_trajectory_point(self, species, sample, position) :
        ...
        return self.data.get_at(sample_index*self.nspecies()*self.nmeshes() + species_index*self.nmeshes() + mesh_index)
...
def save_rdoutput(so, path, separate_data=True) :
    """
