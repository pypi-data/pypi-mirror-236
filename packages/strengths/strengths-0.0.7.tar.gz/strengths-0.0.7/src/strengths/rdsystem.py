from strengths.rdspace import RDSpace, rdspace_from_dict, rdspace_to_dict, load_rdspace, save_rdspace
...
def generate_species_state(species, network, space, units_system) :
    ...
    for i in range(space.size()) :
        mesh_species_density = valproc.get_value_in_env(
            value = species.density, 
            environment = network.environments[mesh_env[i]], 
            default = UnitValue(0, "molecule/µm3"))
        
        state[i] = (mesh_species_density * mesh_vol).convert(units_system).value
...
def generate_species_chemostats(species, network, space) : 
    ...
    for i in range(space.size()) :
        mesh_species_cstt = valproc.get_value_in_env(
            value = species.chstt, 
            environment = network.environments[mesh_env[i]], 
            default = 0)
        chstt[i] = int(mesh_species_cstt)
...
class RDSystem : 
    ...
    def __init__(self, network, space=RDSpace(), state=None, chemostats=None, units_system=UnitsSystem()) :
        """
        constructor
        """
    ...
    def space(self, v) :
        if type(v) != RDSpace :
            raise ValueError("space must be a :py:class:`RDSpace`.")
        self._space = v
