    ...
    def _getTaurusPlotDataItemConfigs(self):
        ...
        for curve in self.plot_item.listDataItems():
            if isinstance(curve, self.itemClass):
                ...
                if (
                    name is None
                    and hasattr(curve, "base_name")
                    and hasattr(curve, "_getCurveNames")
                ):
                    name = [curve.base_name()] + curve._getCurveNames()
...
def _demo_ModelChooser():
    ...
    from taurus_pyqtgraph import TaurusModelChooserTool
