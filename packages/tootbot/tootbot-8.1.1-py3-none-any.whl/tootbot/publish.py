...
class MastodonPublisher:
    ...
    async def initialise(
        ...
        print(
            ...
            f"@{user_info['username']} on {config.mastodon_config.domain}"
        )
        logger.debug(
    ...
    async def cross_post(  # noqa: C901 PLR0913
        ...
        logger.debug(
            "MastodonPublisher.cross_post(posts=%s, "
            "reddit_helper, "
            "media_helper, "
            "duplicate_checks=%s)",
            posts,
        ...
        for sub_id, tags, submission in posts:
            ...
            try:
                ...
                logger.debug(
                    "MastodonPublisher.make_post"
                    " - self.mastodon_config.media_always_sensitive=%s",
    ...
    async def _remove_posted_earlier(
        ...
        non_duplicates = []
        for attachment in attachments:
            if attachment.checksum and await post_recorder.duplicate_check(
                attachment.checksum
            ):
                logger.debug(
