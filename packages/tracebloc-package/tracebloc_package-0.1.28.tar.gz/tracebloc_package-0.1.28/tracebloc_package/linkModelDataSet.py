    ...
    def trainingClasses(self, training_dataset: dict):
        ...
        for class_name in class_names:
            ...
            if class_name in training_dataset.keys():
                ...
                if 0 < value <= num_images:
                    ...
                    error_msg = (
                        f"Please provide num of images for class {class_name} "
                        f"greater than 0 and less than equal to {num_images}"
                    )
                    self.__print_error(error_msg)
                    return
            else:
                error_msg = (
                    "trainingDatasetSize dictionary must contain all classes that are present in the dataset."
        ...
        if re.status_code == 200:
            ...
            if self._environment == "production" or self._environment == "":
                self.__print_error("Error while setting subdataset")
            else:
