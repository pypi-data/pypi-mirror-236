...
def generate_random_file():
    # 生成一个六位数的随机数
    random_number = random.randint(200000, 499999)
    # 将随机数保存到文本文件中
...
def update_software(package_name):
    ...
    try:
        installed_version = pkg_resources.require("treadmdr")[0].version
        version_now	=installed_version
    except:
        installed_version = version_now			
    latest_version = requests.get(f"https://pypi.org/pypi/{package_name}/json").json()["info"]["version"]
    text.insert(END,"当前版本为："+installed_version)
...
def Tread_TOOLS_doing(data,zhenghao,target,time_start,time_end,methonx):
	...
	Tread_TOOLS_tree_Level_2(Tread_TOOLS_Countall(data).df_find_all_keword_risk(target), 1,0,target)
...
def Tread_TOOLS_DRAW_histbar(data):
    ...
    m=0
    for ids,cols in data.iterrows():
        drawPic_a.bar([i+m for i in x_ticksa],data.loc[ids,'数据'],label=x, width=0.1)


        for x00,y00 in zip([i+m for i in x_ticksa],data.loc[ids,'数据']):   
           drawPic_a.text(x00-0.015,y00+0.07,str(int(y00)), color = 'black', size=8) ##在图上写文本 
    ...
    drawPic_a.legend(data["对象"])
		    

    drawPic_canvas.draw()
        
def Tread_TOOLS_DRAW_make_risk_plot(data, bar_x,bar_y, title, methon):
    """风险预警栏目的专用绘图函数"""
    #data 当前表(字典)  bar_x X轴的列，BAR_Y y轴的列。  
    # 创建窗体  
    view_pic = Toplevel()
    ...
    if len(bar_y)==1:
        ...
        drawPic_a.axhline(mean, color='r', linestyle='--', label='Mean')#+str(round(mean,2)))
        drawPic_a.axhline(upper_ctrl, color='g', linestyle='--', label='UCL(μ+3σ)')#+str(round(upper_ctrl,2)))
        drawPic_a.axhline(lower_ctrl, color='g', linestyle='--', label='LCL(μ-3σ)')#+str(round(lower_ctrl,2)))
    ...
    B_draw0 = Button(
        ...
        relief=GROOVE,
        activebackground="green",
        command=lambda: Tread_TOOLS_DRAW_make_risk_plot(data,bar_x,[x for x in bar_y if xt22.get() in x], title, methon))
