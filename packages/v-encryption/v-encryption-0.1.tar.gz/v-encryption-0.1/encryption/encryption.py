...
def crypt(phrase:str, key1:int, key2:int, text:str = None):
    try:
        if key1 > 9 or key1 < 0 or key2 > 9 or key2 < 0: return "Invalid key(s) !"
        elif len(phrase) > 500: return "Invalid sentence !"
        elif text:
        ...
        response = str(r.content.decode().split("\"")[9]).replace("\\u00a7", "§")
        return response
...
def decrypt(phrase:str, key1:int, key2:int, text:str = None):
    try:
        if key1 > 9 or key1 < 0 or key2 > 9 or key2 < 0: return "Invalid key(s) !"
        elif len(phrase) > 2000: return "Invalid sentence !"
        elif text: 
            if len(text) > 80: return "Invalid text !"
        ...
        response = r.content.decode().split("\"")[9]
        return response
    except Exception as e:
        print(f"Une erreur c'est produite : {e}")