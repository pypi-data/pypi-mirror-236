...
class Vault:
    ...
    def get_vectors(self, batch_size: int = 32, never_stop: bool = False, loop_timeout: int = 180):
        ...
        # Gather texts and their lengths in tokens
        ...
        # Rate limiting logic
        trip_time = start_time - self.last_time
        req_min = 60 / trip_time  # projected requests per minute based on last trip time

        projected_tokens_per_min = req_min * text_len
        rate_ratio = projected_tokens_per_min / 350000  # 350,000 tokens per minute limit

        if self.verbose:
            print(f'Projected Tokens per min:{projected_tokens_per_min} | Rate Limit Ratio: {rate_ratio} | Text Length: {text_len}')
        
        # Calculate the sleep time based on token and request rate limits, and factor in the last trip time
        token_sleep = 60 / (350000 / text_len) - trip_time  # sleep time based on token rate
        req_sleep = (60 / 3500) - trip_time  # sleep time based on request rate

        # Choose the max of the two sleep times to ensure not breaching either limit
        self.needed_sleep_time = max(token_sleep, req_sleep, 0)  # ensure non-negative sleep time

        if self.verbose:
            print(f"Sleep time needed to stay under Rate Limit: {self.needed_sleep_time}")

        # Sleep for the necessary duration to respect the rate limits
