...
class Vault:
    ...
    def get_vectors(self, batch_size: int = 32, never_stop: bool = False, loop_timeout: int = 180):
        ...
        text_len = sum(self.ai.get_tokens(text) for text in texts)
        ...
        # Introducing an internal function to include rate limiting inside each batch processing
        def process_with_rate_limit(batch_text_chunks):
            # This logic is similar to what was outside, but now it's applied to each batch
            trip_time = time.time() - self.last_time
            req_min = 60 / trip_time
            projected_tokens_per_min = req_min * len(batch_text_chunks)
            token_sleep = 60 / (1000000 / len(batch_text_chunks)) - trip_time
            req_sleep = (60 / 3000) - trip_time
            sleep_time = max(token_sleep, req_sleep, 0)
            time.sleep(sleep_time)

            # After sleeping, now you can process the batch
            return self.process_batch(batch_text_chunks, never_stop, loop_timeout)

        # Limit the number of concurrent threads, e.g., to 10 (you can adjust this number)
        # with ThreadPoolExecutor(max_workers=10) as executor:
        #     batch_embeddings_list = list(executor.map(process_with_rate_limit, batches_text_chunks))
