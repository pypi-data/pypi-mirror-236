...
def assign_node_size(node:NodeTemplate):
    if type(node) in [InputNode, OutputNode, GitLoaderNode, SerpAPILoaderNode]:
        return 250, 155
    elif type(node) in [URLLoaderNode, FileLoaderNode, WikipediaLoaderNode, ArXivLoaderNode, ChatMemoryNode]:
        return 200, 100
    elif type(node) in [NotionLoaderNode, OpenAILLMNode, VectorDBLoaderNode, VectorDBReaderNode, LogicMergeNode]:
        return 275, 175
    else:
