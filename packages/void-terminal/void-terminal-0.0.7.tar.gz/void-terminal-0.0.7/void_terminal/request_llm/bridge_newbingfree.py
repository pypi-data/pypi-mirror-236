...
========================================================================
"""
from void_terminal.request_llm.edge_gpt_free import Chatbot as NewbingChatbot
...
class NewBingHandle(Process):
    ...
    def check_dependency(self):
        try:
            ...
            self.info = "Missing dependencies，If you want to use Newbing，In addition to the basic pip dependencies，You also need to run`pip install -r request_llm/requirements_newbing.txt`Install the dependencies for Newbing。"
            self.success = False
    ...
    def run(self):
        ...
        if (self.newbing_model is None) or (not self.success):
            ...
            try:
                ...
                self.success = False
                tb_str = '\n```\n' + trimmed_format_exc() + '\n```\n'
                self.child.send(f'[Local Message] Cannot load Newbing components。{tb_str}')
                self.child.send('[Fail]')
                self.child.send('[Finish]')
                raise RuntimeError(f"Cannot load Newbing components。")
