...
from volstreet.utils.core import currenttime
...
class BaseStrategy(ABC):
    ...
    def __init__(
        ...
        self.strategy_tags = self.set_strategy_tags(strategy_tags)
        self.indices_to_trade = self.initialize_indices(indices)
        self.client_data = {} if client_data is None else client_data
        self.parameters, self.special_parameters = self.set_parameters(
            parameters, special_parameters
        )
    ...
    def __repr__(self):
        return (
            ...
            f"parameters={self._truncate_or_str(self.parameters)}, "
            f"indices={[index.name for index in self.indices_to_trade]}, "
            f"tags={self.strategy_tags}, "
    ...
    def run(self):
        ...
        while currenttime().time() < time(*self.start_time):
            sleep(5)
...
class IntradayStrangle(BaseStrategy):
    ...
    def initialize_parameters(self, parameters, special_parameters) -> dict:
        ...
        logger.info(f"Initializing intraday strangle with parameters: {parameters}")
        return parameters
