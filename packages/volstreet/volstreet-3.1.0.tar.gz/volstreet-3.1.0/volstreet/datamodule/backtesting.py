...
from volstreet.utils import findstrike
...
class BackTester:
    ...
    def _build_option_chain_skeleton(
        ...
        df = df.reset_index()
        df["atm_strike"] = df.close.apply(lambda x: findstrike(x, base))
    ...
    def _build_overnight_backtest_skeleton(
        ...
        df["call_strike"] = df.close.apply(
            lambda x: findstrike(x * (1 + call_strike_offset), base)
        )
        df["put_strike"] = df.close.apply(
            lambda x: findstrike(x * (1 - put_strike_offset), base)
        )
    ...
    def _refined_fetch_option_prices_from_skeleton(
        self,
        skeleton: pd.DataFrame,
        index: str,
        timestamp_col: str = "timestamp",
        square_up_timestamp_col: str = "square_up_timestamp",
    ) -> pd.DataFrame:
        melt_columns = [timestamp_col]
        if square_up_timestamp_col:
            melt_columns.append(square_up_timestamp_col)

        # Melt the skeleton DataFrame to bring timestamps into a single column
        melted_skeleton = pd.melt(
            skeleton,
            id_vars=["call_strike", "put_strike", "expiry"],
            value_vars=melt_columns,
            var_name="time_type",
            value_name="timestamp_",
        )

        # Sort the melted skeleton by timestamp
        melted_skeleton_sorted = melted_skeleton.sort_values("timestamp_")

        # Assume we have a constant MAX_PARAMS to indicate how many rows TimescaleDB can handle in a single query
        MAX_PARAMS = 1000

        # Break melted_skeleton_sorted into chunks of size MAX_PARAMS
        chunks = [
            melted_skeleton_sorted.iloc[i : i + MAX_PARAMS]
            for i in range(0, len(melted_skeleton_sorted), MAX_PARAMS)
        ]

        option_prices_full = pd.DataFrame()

        for chunk in chunks:
            timestamps = chunk["timestamp_"].tolist()
            expirys = chunk["expiry"].tolist()
            strikes = chunk[["call_strike", "put_strike"]].values.tolist()

            query = self.db_manager.generate_query_for_option_prices(
                index=index,
            ...
            option_prices_chunk = self.db_manager.fetch_option_prices_from_tsdb(query)
            option_prices_full = pd.concat([option_prices_full, option_prices_chunk])

        option_prices = (
            option_prices_full.groupby(["timestamp", "expiry", "strike", "option_type"])
            .close.first()
            .unstack()
            .reset_index()
        )
    ...
    def _fetch_option_prices_from_skeleton(
        ...
        for timestamp_chunk in timestamp_chunks:
            query = self.db_manager.generate_sql_query_for_option_prices(
                cols_to_return=[
                    "timestamp",
                    "expiry",
                    "strike",
    ...
    def backtest_option_buying_strategy_on_expiry(
        ...
        day_info["strike"] = day_info.open.apply(lambda x: findstrike(x, strike_base))
    ...
    def backtest_intraday_strangle(
        ...
        day_info["traded_call_strike"] = (day_info.open * (1 + call_offset)).apply(
            lambda x: findstrike(x, strike_base)
        )
