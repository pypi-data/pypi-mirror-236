...
def intraday_strangle(
    ...
    def process_stop_loss(info_dict, sl_type):
        ...
        while all([current_time().time() < time(*exit_time)]):
            check_for_stop_loss(
    ...
    if not call_sl and not put_sl:  # Both stop losses not hit
        if time_to_expiry(expiry, in_days=True) < (
            5 / (365 * 24 * 60)
        ):  # less than 5 mins to expiry so
            # dont place exit order
...
def intraday_trend(
    ...
    while entries < max_entries and current_time().time() < exit_time:
        ...
        stop_loss_hit = False
        notifier(
            f"{underlying.name} {position} trender triggered with {movement:0.2f} movement. {underlying.name} at {price}. "
