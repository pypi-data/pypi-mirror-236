...
class BackTester:
    ...
    def _build_option_chain_skeleton(
        ...
        df = df.reset_index()
        df["atm_strike"] = df.close.apply(lambda x: find_strike(x, base))
    ...
    def _build_overnight_backtest_skeleton(
        ...
        df["call_strike"] = df.close.apply(
            lambda x: find_strike(x * (1 + call_strike_offset), base)
        )
        df["put_strike"] = df.close.apply(
            lambda x: find_strike(x * (1 - put_strike_offset), base)
        )
