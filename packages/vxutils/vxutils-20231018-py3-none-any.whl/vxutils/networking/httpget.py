...
class vxHttpGet:
    ...
    def __init__(
        self,
        parser: Callable[[httpx.Response], Any] = None,
    ...
    async def _task(self, method: str, url: str) -> str:
    ...
    async def _get(self, *urls, **params):
        """执行连接"""
    ...
    def __call__(self, *urls, **params):
        ...
        if loop and loop.is_running():
            ...
            for i in range(0, len(urls), self.workers):
                _urls = urls[i : i + self.workers]
                results.extend(asyncio.run(self._get(*_urls, **params)))
                if i > 0:
                    vxtime.sleep(0.1)
