...
def create(package, verbose=False):
    ...
    found = False
    for sched in scheds:
        if sched.description == 'VersionChecker(develop)':
            found = True

    if not found:
        sched = project.pipelineschedules.create({
            'ref': 'develop',
            'description': 'VersionChecker(develop)',
    ...
    found = False
    for sched in scheds:
        if sched.description == 'VersionChecker(master)':
            found = True

    if not found:
        sched = project.pipelineschedules.create({
            'ref': 'master',
            'description': 'VersionChecker(master)',
            'cron_timezone': 'Europe/Paris',
            'cron': '{} 5 * * *'.format(random.randint(0, 59)),
            'active': True
        })
