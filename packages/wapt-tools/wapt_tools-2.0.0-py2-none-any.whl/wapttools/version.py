...
def latestVersion(force=False):
    ...
    # log.debug(url_info)
    if 'Content-Type' not in url_info.headers or url_info.headers['Content-Type'] != 'application/octet-stream':
        ...
        if config['html'] is True:
            ...
            latest_version = soup.select(config['selector'])[index].contents[0].strip().encode('utf-8')
            log.debug('selected[{}].contents[0] = {}'.format(index, latest_version))
        else:
