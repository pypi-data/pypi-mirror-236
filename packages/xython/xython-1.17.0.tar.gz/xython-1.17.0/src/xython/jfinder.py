...
class jfinder:
    ...
    def change_jfsql_to_resql (self, jf_re):
        """
    ...
    def delete_by_jfsql (self, jf_re, input_text):
        """
		jfsql형식에 맞는 자료들 삭제

        :param jf_re:
        :param input_text:
        :return: [[결과값, 시작순서, 끝순서, [그룹1, 그룹2...], match결과].....]
		"""
        re_sql = self.jfinder(jf_re)

        result = self.search_all_by_resql(re_sql, input_text)
        self.delete_by_resql (re_sql, input_text)
        return result

    def delete (self, jf_re, input_text):
        """
        입력자료중 jfsql에 맞는 형식을 삭제하는것
    ...
    def replace_by_jfsql (self, jf_re, replace_word, input_text):
        """
        입력자료를 원하는 문자로 바꾸는것
    ...
    def run_by_jfsql (self, jf_re, input_text):
        """
    ...
    def search_between_word1_word2_by_jfsql(self, word_a, word_b, input_text):
        ...
        :return:
        """
        jf_re = "(?<=\\" + str(word_a) + ")(.*?)(?="+str(word_b) + ")"
        result = self.search_all_by_resql(jf_re, input_text)
        return result
    ...
    def search_by_jfsql (self, jf_re, input_text):
        """
    ...
    def search_all_by_jfsql (self, jf_re, input_text):
        """
