...
class pcell:
	...
	def paste_range(self, sheet_name="", xyxy=""):
		...
		sheet_object.Cells(x1, y1).Select()
		sheet_object.Paste()
		# sheet_object.Cells(x1, y1 ).PasteSpecial(-4104)
		#sheet_object.Cells(x1, y1).PasteSpecial(-4163)  # 값 붙여 넣기
	...
	def add_all_pictures_in_folder_at_sheet(self, sheet_name, folder_name, ext_list, xywh, link=0J, image_in_file=1):
		# 특정폴다안이 모든 사진을 전부 불러오는 것이다
		# def add_al.l_pictures_in_fol.der_at_sheet(selfsheet_name) file_path, xywh, link=0J, image_in_file=l):
		# sh.Shapes.A、Picture卜호[일이를' "링크가있나", .■문人[에저장' "x좌표", "y좌표", "넓이", "높Op)
	...
	def insert_picture_at_cell_as_same_size(self, sheet_name, xyxy, file_name, space = 1):
