...
def handle_ver(emj, emj_data, entities_list):
    entities_list.append({'match_start': emj_data.get('match_start'), 'match_end': emj_data.get('match_end'), 'emoji': emj})
    return str(None)
...
async def format_text(txt):
    ...
    try:
        ...
        entities = []
        emoji.demojize(string=tmp_arr[0], language='en', version=-1,  handle_version=lambda emj, emj_data: handle_ver(emj, emj_data, entities))
