...
def Entity(cls):
    ...
    def new_getattribute(self, name):
        ...
        return get_value_from_property(value)
...
def get_constraint_from_property(property_value):
    """从属性值中解析出约束条件"""
    if not isinstance(property_value, tuple):
        return []
...
def get_value_from_property(property_value):
    """从属性值中解析出是基本类型的唯一属性默认值"""
    # 属性值就是基本类型
    if isinstance(property_value, (int, str, float, bytes)) or property_value is None:
