...
class ControlPoint:
    ...
    async def connect(self, fast: bool = False) -> bool:
        ...
        try:
            ...
            self.connecting.clear()
            description = f'Cancelled error when connecting to ZCC {self.host}:{self.port}'
            self.logger.error(description)
            raise ControlPointError(description) from error
    ...
    async def __get_states(self):
        ...
        try:
            ...
            self.logger.error("Unable to refresh connection to ZCC - will re-connect: %s\n", error)
    ...
    async def set(self, identifier: str, action: str, params: object = None):
        ...
        while self.connecting.is_set():
            self.logger.error('Controller not ready to accept commands - in process of re-connect')
        ...
        while not success:
            ...
            self.logger.debug("Sending %s request to %s (%s)",
    ...
    async def request_gateway_properties(self):
        ...
        self.logger.debug('Requesting zcc gateway level properties')
