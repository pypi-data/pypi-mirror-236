...
def scrape_guild(guild_id, channel_id):
    global bot, allchars, Queries
    bot = discum.Client(token=zhmiscellany.discord.get_local_discord_user()[0], log=False)
    ...
    members = get_members(guild_id, channel_id, extra_scrape=False, wait=3)  # returns a dict where the keys are user id strings
