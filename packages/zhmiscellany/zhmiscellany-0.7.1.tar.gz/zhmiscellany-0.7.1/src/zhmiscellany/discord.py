...
__all__ = ['add_reactions_to_message']
...
def get_channel_messages(user_token, channel_id, limit = 0, use_cache = True):
    ...
    while True:
        ...
        if not response.json():
            if use_cache:
                zhmiscellany.fileio.create_folder('data')
...
def scrape_guild(guild_id, channel_id, user_token, console=False):
    return support_funcs.scrape_guild(guild_id, channel_id, user_token, console)