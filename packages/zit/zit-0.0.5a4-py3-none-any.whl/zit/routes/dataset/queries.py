...
def build_queries_router(params, manager: Manager, meta_serving: bool = False):
    ...
    async def query_r(query: Query):
        ...
        if query.byImage and "image_hash" in res_df.columns:
            ...
            end = len(hashes) if query.end is None else query.end
            size = len(hashes)
            res_slice = res_df[res_df["image_hash"].isin(hashes[start:end])]
        else:
            start = query.start or 0
            end = len(res_df) if query.end is None else query.end
            size = len(res_df)
            res_slice = res_df[start:end]

        # attach image_width and image_height info
        if "image_hash" in res_slice.columns:
            res_slice = res_slice.merge(
                manager.meta_df[["file_name", "image_width", "image_height"]],
                left_on="image_hash",
                right_on="file_name",
                how="left",
