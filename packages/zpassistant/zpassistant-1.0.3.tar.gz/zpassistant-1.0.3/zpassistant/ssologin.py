...
class SsoLoginUtil:
    ...
    def ssoLoginByUserNamePassword(self,url, userName, password,type = None):
        # 获取 session cookie
        session_cookie_jar, expire = self.session_cookie_expired_check(type)
        if not expire:
            self.session.cookies = session_cookie_jar
        ...
        if response.status_code == 200:
            response = session.get(get_protocol_and_domain(response.url) +  redirect(response.url), allow_redirects=True)
            if response.status_code == 200:
                return session
...
if (__name__ == "__main__"):
    def checkSsoLogin(browser):
        if browser.title == '工作台 [Jenkins]':
            ...
            return True
        return False
    SsoLoginUtil("https://zpsso.zhaopin.com/login").ssoLoginByUserNamePassword( "https://apollo.dev.zhaopin.com/","zhitao.mao","hK7m6568")